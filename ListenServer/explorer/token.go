...
func GetWalletValue(category string, account string) ([]*TokenValue, error) {
	...
	if category == "DATX" {
		tokenValue, err := GetTokenValue("DATX", account)
		if err != nil {
			return tokens, err
		}
		tokens = append(tokens, tokenValue)
		tokenValue, err = GetTokenValue("DBTC", account)
		if err != nil {
			return tokens, err
		}
		tokens = append(tokens, tokenValue)
		tokenValue, err = GetTokenValue("DETH", account)
		if err != nil {
			return tokens, err
		}
		tokens = append(tokens, tokenValue)
		tokenValue, err = GetTokenValue("DEOS", account)
		if err != nil {
			return tokens, err
		}
		tokens = append(tokens, tokenValue)
...
func GetDATXBalance(token string, account string) (float64, error) {
	...
	// 转换成发币合约
	if token == "DATX" {
		formData["code"] = "datxio.token"
	} else {
		formData["code"] = "datxio." + strings.ToLower(token)
	}
