...
def monitor_exchange():
    ...
    results = cldatx('get actions tokenxchange {} 0 -j'.format(action_num))
...
def transfer(to, quantity):
    if quantity[:-4] != ' SYS':
        quantity += ' SYS'
    results = cldatx('transfer tokenxchange {} "{}" {} -j'.format(to, quantity, KEY_TO_INTERNAL_ACCOUNT))
