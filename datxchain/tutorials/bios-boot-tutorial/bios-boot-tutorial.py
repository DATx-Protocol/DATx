...
fastUnstakeSystem = './fast.refund/datxos.system/datxos.system.wasm'
...
def claimRewards():
    table = getJsonOutput(args.cldatx + 'get table datxos datxos producers -l 100')
    times = []
...
def msigProposeReplaceSystem(proposer, proposalName):
    ...
    trxPermissions = [{'actor': 'datxos', 'permission': 'active'}]
    with open(fastUnstakeSystem, mode='rb') as f:
        setcode = {'account': 'datxos', 'vmtype': 0, 'vmversion': 0, 'code': f.read().hex()}
    run(args.cldatx + 'multisig propose ' + proposalName + jsonArg(requestedPermissions) + 
        jsonArg(trxPermissions) + 'datxos setcode' + jsonArg(setcode) + ' -p ' + proposer)
...
def msigReplaceSystem():
    run(args.cldatx + 'push action datxos buyrambytes' + jsonArg(['datxos', accounts[0]['name'], 200000]) + '-p datxos')
    sleep(1)
...
def stepInstallSystemContracts():
    run(args.cldatx + 'set contract datxos.token ' + args.contracts_dir + 'datxos.token/')
    run(args.cldatx + 'set contract datxos.msig ' + args.contracts_dir + 'datxos.msig/')
def stepCreateTokens():
    run(args.cldatx + 'push action datxos.token create \'["datxos", "10000000000.0000 %s"]\' -p datxos.token' % (args.symbol))
    totalAllocation = allocateFunds(0, len(accounts))
    run(args.cldatx + 'push action datxos.token issue \'["datxos", "%s", "memo"]\' -p datxos' % intToCurrency(totalAllocation))
    sleep(1)
def stepSetSystemContract():
    retry(args.cldatx + 'set contract datxos ' + args.contracts_dir + 'datxos.system/')
    sleep(1)
