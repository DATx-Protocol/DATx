...
namespace datxos { namespace client { namespace http {
   ...
   std::string do_txrx(T& socket, boost::asio::streambuf& request_buff, unsigned int& status_code) {
      ...
      DATX_ASSERT( status_code != 400, invalid_http_request, "The server has rejected the request as invalid!");

      std::string status_message;
      std::getline(response_stream, status_message);
      DATX_ASSERT( !(!response_stream || http_version.substr(0, 5) != "HTTP/"), invalid_http_response, "Invalid Response" );
      ...
      DATX_ASSERT(response_content_length >= 0, invalid_http_response, "Invalid content-length response");
   ...
   parsed_url parse_url( const string& server_url ) {
      ...
      if(res.scheme != "http" && res.scheme != "https")
         DATX_THROW(fail_to_resolve_host, "Unrecognized URL scheme (${s}) in URL \"${u}\"", ("s", res.scheme)("u", server_url));
      if(res.server.empty())
         DATX_THROW(fail_to_resolve_host, "No server parsed from URL \"${u}\"", ("u", server_url));
      if(res.port.empty())
   ...
   resolved_url resolve_url( const http_context& context, const parsed_url& url ) {
      ...
      if (ec) {
         DATX_THROW(fail_to_resolve_host, "Error resolving \"${server}:${url}\" : ${m}", ("server", url.server)("port",url.port)("m",ec.message()));
      }
      ...
      for(const auto& r : result) {
         ...
         if (resolved_port) {
            DATX_ASSERT(*resolved_port == port, resolved_to_multiple_ports, "Service name \"${port}\" resolved to multiple ports and this is not supported!", ("port",url.port));
         } else {
            resolved_port = port;
         }
      }
...
#elif defined( _WIN32 )
         DATX_THROW(http_exception, "HTTPS on Windows not supported");
#else
   ...
   if( status_code == 200 || status_code == 201 || status_code == 202 ) {
      ...
      if (url.path.compare(0, chain_func_base.size(), chain_func_base) == 0) {
         throw chain::missing_core_api_accessory_exception(FC_LOG_MESSAGE(error, "Chain API accessory is not enabled"));
      } else if (url.path.compare(0, wallet_func_base.size(), wallet_func_base) == 0) {
         throw chain::missing_wallet_api_accessory_exception(FC_LOG_MESSAGE(error, "Wallet is not available"));
      } else if (url.path.compare(0, history_func_base.size(), history_func_base) == 0) {
         throw chain::missing_history_api_accessory_exception(FC_LOG_MESSAGE(error, "History API accessory is not enabled"));
      } else if (url.path.compare(0, net_func_base.size(), net_func_base) == 0) {
         throw chain::missing_p2p_net_api_accessory_exception(FC_LOG_MESSAGE(error, "Net API accessory is not enabled"));
      }
   } else {
      auto &&error_info = response_result.as<datxos::error_results>().error;
   ...
   DATX_ASSERT( status_code == 200, http_request_fail, "Error code ${c}\n: ${msg}\n", ("c", status_code)("msg", re) );
   return response_result;
   }
}}}
