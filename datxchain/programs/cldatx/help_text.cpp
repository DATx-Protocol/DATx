...
Please check the account and table name, and verify that the account has the expected code using:
  cldatx get code ${1})text");

const char* failed_to_find_transaction_text = _("Failed to fetch information for transaction: \033[1m${1}\033[0m from the history accessory\n\n"
...
const char* failed_to_find_transaction_with_block_text = _("Failed to fetch information for transaction: \033[1m${1}\033[0m from the history accessory and the transaction was not present in block \033[1m${2}\033[0m\n");

const char* history_accessory_advice_text = _("\033[32mPlease ensure that the \033[2mdatxos::history_accessory\033[22m is enabled on the RPC node you are connecting to and that an account involved in this transaction was configured in the \033[2mfilter-on\033[22m setting.\033[0m\n");
...
const std::vector<std::pair<const char*, std::vector<const char *>>> error_help_text {
   ...
   {"Transaction ([^ ]{8})[^ ]* not found in history and no block hint was given", {failed_to_find_transaction_text, history_accessory_advice_text}},
   {"Transaction ([^ ]{8})[^ ]* not found in history or in block number ([0-9]*)", {failed_to_find_transaction_with_block_text, history_accessory_advice_text}},
};
...
const char* error_advice_public_key_type_exception = R"=====(Public key should be encoded in base58 and starts with DATX prefix)=====";
...
const char* error_advice_authority_type_exception = R"=====(Ensure that your authority JSON is valid follows the following format!
{
  ...
  "keys": [         <keys must be alpha-numerically sorted by their string representations and unique>
    ...
    {
      "key":        <STRING: DATX.IO compatible Public Key>,
...
You can refer to contracts/DatxosLib/transaction.hpp for reference)=====";
...
const char* error_advice_tx_duplicate =  "You can try embedding datxos nonce action inside your transaction to ensure uniqueness.";

const char* error_advice_invalid_action_args_exception = R"=====(Ensure that your arguments follow the contract abi!
You can check the contract's abi by using 'cldatx get code' command.)=====";
...
const char* error_advice_missing_core_api_accessory_exception =  "Ensure that you have \033[2mdatxos::core_api_accessory\033[0m\033[32m added to your node's configuration!";
const char* error_advice_missing_wallet_api_accessory_exception =  "Ensure that you have \033[2mdatxos::wallet_api_accessory\033[0m\033[32m added to your node's configuration!\n"\
                                    "Otherwise specify your wallet location with \033[2m--wallet-url\033[0m\033[32m argument!";
const char* error_advice_missing_history_api_accessory_exception =  "Ensure that you have \033[2mdatxos::history_api_accessory\033[0m\033[32m added to your node's configuration!";
const char* error_advice_missing_p2p_net_api_accessory_exception =  "Ensure that you have \033[2mdatxos::p2p_net_api_accessory\033[0m\033[32m added to your node's configuration!";
...
const std::map<int64_t, std::string> error_advice = {
   ...
   { missing_core_api_accessory_exception::code_value, error_advice_missing_core_api_accessory_exception },
   { missing_wallet_api_accessory_exception::code_value, error_advice_missing_wallet_api_accessory_exception },
   { missing_history_api_accessory_exception::code_value, error_advice_missing_history_api_accessory_exception },
   { missing_p2p_net_api_accessory_exception::code_value, error_advice_missing_p2p_net_api_accessory_exception },
...
namespace datxos { namespace client { namespace help {

bool print_recognized_errors(const fc::exception& e, const bool verbose_errors) {
   // datx recognized error code is from 3000000
   // refer to libraries/chain/include/datxos/chain/exceptions.hpp
