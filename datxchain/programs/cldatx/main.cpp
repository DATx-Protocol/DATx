 ...
 *  @defgroup datxclienttool DATXOS Command Line Client Reference
 *  @brief Tool for sending transactions and querying state from @ref noddatx
 *  @ingroup datxclienttool
 */

/**
  @defgroup datxclienttool
  ...
  have a local copy of `noddatx` running and configured to load the 'datxos::core_api_accessory'.
...
```
$ ./cldatx
Command Line Interface to DATXOS Client
Usage: programs/cldatx/cldatx [OPTIONS] SUBCOMMAND

Options:
  -h,--help                   Print this help message and exit
  -u,--url TEXT=http://localhost:8888/
                              the http/https URL where noddatx is running
  --wallet-url TEXT=http://localhost:8888/
                              the http/https URL where kdatxd is running
...
Subcommands:
  ...
  transfer                    Transfer DATX from account to account
...
Usage: ./cldatx create SUBCOMMAND

Subcommands:
  key                         Create a new keypair and print the public and private keys
  account                     Create a new account on the blockchain

$ ./cldatx create account
Create a new account on the blockchain
Usage: ./cldatx create account [OPTIONS] creator name OwnerKey ActiveKey
...
#include <datxos/utilities/key_conversion.hpp>

#include <datxos/chain/name.hpp>
#include <datxos/chain/config.hpp>
#include <datxos/chain/wast_to_wasm.hpp>
#include <datxos/chain/trace.hpp>
...
using namespace datxos::chain;
using namespace datxos::utilities;
using namespace datxos::client::help;
using namespace datxos::client::http;
using namespace datxos::client::localize;
using namespace datxos::client::config;
using namespace boost::filesystem;

FC_DECLARE_EXCEPTION( explained_exception, 9000000, "explained exception, see error log" );
FC_DECLARE_EXCEPTION( localized_exception, 10000000, "an error occured" );
#define DATXC_ASSERT( TEST, ... ) \
  FC_EXPAND_MACRO( \
...
datxos::client::http::http_context context;
...
fc::variant call( const std::string& url,
                  ...
                  const T& v ) {
   try {
      auto sp = std::make_unique<datxos::client::http::connection_param>(context, parse_url(url) + path, no_verify ? false : true, headers);
      return datxos::client::http::do_http_call(*sp, fc::variant(v), print_request, print_response );
   }
   catch(boost::system::system_error& e) {
      if(url == ::url)
         std::cerr << localized("Failed to connect to noddatx at ${u}; is noddatx running?", ("u", url)) << std::endl;
      else if(url == ::wallet_url)
         std::cerr << localized("Failed to connect to kdatxd at ${u}; is kdatxd running?", ("u", url)) << std::endl;
...
datxos::chain_apis::read_only::get_info_results get_info() {
   return call(url, get_info_func).as<datxos::chain_apis::read_only::get_info_results>();
}
...
fc::variant push_transaction( signed_transaction& trx, int32_t extra_kcpu = 1000, packed_transaction::compression_type compression = packed_transaction::none ) {
   ...
   } DATX_RETHROW_EXCEPTIONS(invalid_ref_block_exception, "Invalid reference block num or id: ${block_num_or_id}", ("block_num_or_id", tx_ref_block_num_or_id));
...
auto abi_serializer_resolver = [](const name& account) -> optional<abi_serializer> {
   ...
   if ( it == abi_cache.end() ) {
      ...
      auto abi_results = result.as<datxos::chain_apis::read_only::get_abi_results>();
...
bytes json_or_file_to_bin( const account_name& account, const action_name& action, const string& data_or_filename ) {
   ...
   if( !data_or_filename.empty() ) {
      ...
      } DATX_RETHROW_EXCEPTIONS(action_type_exception, "Fail to parse action JSON data='${data}'", ("data", data_or_filename));
   }
...
chain::action create_newaccount(const name& creator, const name& newaccount, public_key_type owner, public_key_type active) {
   return action {
      ...
      datxos::chain::newaccount{
         .creator      = creator,
         .name         = newaccount,
         .owner        = datxos::chain::authority{1, {{owner, 1}}, {}},
         .active       = datxos::chain::authority{1, {{active, 1}}, {}}
      }
   };
}
...
authority parse_json_authority(const std::string& authorityJsonOrFile) {
   ...
   } DATX_RETHROW_EXCEPTIONS(authority_type_exception, "Fail to parse Authority JSON '${data}'", ("data",authorityJsonOrFile))
}

authority parse_json_authority_or_key(const std::string& authorityJsonOrFile) {
   if (boost::istarts_with(authorityJsonOrFile, "DATX") || boost::istarts_with(authorityJsonOrFile, "PUB_R1")) {
      try {
         return authority(public_key_type(authorityJsonOrFile));
      } DATX_RETHROW_EXCEPTIONS(public_key_type_exception, "Invalid public key: ${public_key}", ("public_key", authorityJsonOrFile))
   } else {
      auto result = parse_json_authority(authorityJsonOrFile);
      DATX_ASSERT( datxos::chain::validate(result), authority_type_exception, "Authority failed validation! ensure that keys, accounts, and waits are sorted and that the threshold is valid and satisfiable!");
      return result;
   }
}

asset to_asset( const string& code, const string& s ) {
   static map<datxos::chain::symbol_code, datxos::chain::symbol> cache;
   auto a = asset::from_string( s );
   datxos::chain::symbol_code sym = a.get_symbol().to_symbol_code();
   ...
   if ( it == cache.end() ) {
      ...
      if (obj_it != obj.end()) {
         auto result = obj_it->value().as<datxos::chain_apis::read_only::get_currency_stats_result>();
         auto p = cache.insert(make_pair( sym, result.max_supply.get_symbol() ));
         it = p.first;
      } else {
         DATX_THROW(symbol_type_exception, "Symbol ${s} is not supported by token contract ${c}", ("s", sym_str)("c", code));
      }
   }
   ...
   if ( a.decimals() < expected_symbol.decimals() ) {
      ...
      DATX_THROW(symbol_type_exception, "Too many decimal digits in ${a}, only ${d} supported", ("a", a)("d", expected_symbol.decimals()));
   } // else precision matches
   return a;
}

inline asset to_asset( const string& s ) {
   return to_asset( "datxos.token", s );
}
...
void try_local_port( const string& lo_address, uint16_t port, uint32_t duration ) {
   ...
   while ( !local_port_used(lo_address, port)) {
      if (duration_cast<std::chrono::milliseconds>( system_clock::now().time_since_epoch()).count() - start_time > duration ) {
         std::cerr << "Unable to connect to kdatxd, if kdatxd is running please kill the process and try again.\n";
         throw connection_exception(fc::log_messages{FC_LOG_MESSAGE(error, "Unable to connect to kdatxd")});
      }
   }
}

void ensure_kdatxd_running(CLI::App* app) {
    // get, version, net do not require kdatxd
    ...
    for (const auto& addr: resolved_url.resolved_addresses)
       if (local_port_used(addr, resolved_url.resolved_port))  // Hopefully taken by kdatxd
          return;

    boost::filesystem::path binPath = boost::dll::program_location();
    binPath.remove_filename();
    // This extra check is necessary when running cldatx like this: ./cldatx ...
    if (binPath.filename_is_dot())
        binPath.remove_filename();
    binPath.append(key_store_executable_name); // if cldatx and kdatxd are in the same installation directory
    if (!boost::filesystem::exists(binPath)) {
        binPath.remove_filename().remove_filename().append("kdatxd").append(key_store_executable_name);
    }
    ...
    if (boost::filesystem::exists(binPath)) {
        ...
        ::boost::process::child kdatx(binPath, pargs,
                                     bp::std_in.close(),
                                     bp::std_out > bp::null,
                                     bp::std_err > bp::null);
        if (kdatx.running()) {
        ...
        std::cerr << "No wallet service listening on " << lo_address << ":" << std::to_string(resolved_url.resolved_port)
                  << ". Cannot automatically start kdatxd because kdatxd was not found." << std::endl;
    }
}
   ...
   create_account_subcommand(CLI::App* actionRoot, bool s) : simple(s) {
      ...
      if (!simple) {
         createAccount->add_option("--stake-net", stake_net,
                                   (localized("The amount of DATX delegated for net bandwidth")))->required();
         createAccount->add_option("--stake-cpu", stake_cpu,
                                   (localized("The amount of DATX delegated for CPU bandwidth")))->required();
         ...
         createAccount->add_option("--buy-ram", buy_ram_datx,
                                   (localized("The amount of RAM bytes to purchase for the new account in DATX")));
         createAccount->add_flag("--transfer", transfer,
                                 (localized("Transfer voting power and right to unstake DATX to receiver")));
      }
      ...
      createAccount->set_callback([this] {
            ...
            } DATX_RETHROW_EXCEPTIONS(public_key_type_exception, "Invalid owner public key: ${public_key}", ("public_key", owner_key_str));
            try {
               active_key = public_key_type(active_key_str);
            } DATX_RETHROW_EXCEPTIONS(public_key_type_exception, "Invalid active public key: ${public_key}", ("public_key", active_key_str));
            auto create = create_newaccount(creator, account_name, owner_key, active_key);
            if (!simple) {
               if ( buy_ram_datx.empty() && buy_ram_bytes_in_kbytes == 0) {
                  std::cerr << "ERROR: Either --buy-ram or --buy-ram-kbytes with non-zero value is required" << std::endl;
                  return;
               }
               action buyram = !buy_ram_datx.empty() ? create_buyram(creator, account_name, to_asset(buy_ram_datx))
...
struct vote_producers_subcommand {
   string voter_str;
   vector<datxos::name> producer_names;
...
struct approve_producer_subcommand {
   ...
   approve_producer_subcommand(CLI::App* actionRoot) {
      ...
      approve_producer->set_callback([this] {
            ...
            auto res = result.as<datxos::chain_apis::read_only::get_table_rows_result>();
            ...
            DATX_ASSERT( 1 == res.rows.size(), multiple_voter_info, "More than one voter_info for account" );
            auto prod_vars = res.rows[0]["producers"].get_array();
            vector<datxos::name> prods;
...
struct list_producers_subcommand {
   ...
   list_producers_subcommand(CLI::App* actionRoot) {
      ...
      list_producers->set_callback([this] {
         ...
         auto result = rawResult.as<datxos::chain_apis::read_only::get_producers_result>();
...
struct get_transaction_id_subcommand {
   ...
   get_transaction_id_subcommand(CLI::App* actionRoot) {
      ...
      get_transaction_id->set_callback([&] {
         ...
         } DATX_RETHROW_EXCEPTIONS(transaction_type_exception, "Fail to parse transaction JSON '${data}'", ("data",trx_to_check))
      });
   }
};
...
struct delegate_bandwidth_subcommand {
   ...
   delegate_bandwidth_subcommand(CLI::App* actionRoot) {
      ...
      delegate_bandwidth->add_option("stake_net_quantity", stake_net_amount, localized("The amount of DATX to stake for network bandwidth"))->required();
      delegate_bandwidth->add_option("stake_cpu_quantity", stake_cpu_amount, localized("The amount of DATX to stake for CPU bandwidth"))->required();
      delegate_bandwidth->add_option("--buyram", buy_ram_amount, localized("The amount of DATX to buyram"));
      delegate_bandwidth->add_flag("--transfer", transfer, localized("Transfer voting power and right to unstake DATX to receiver"));
...
struct undelegate_bandwidth_subcommand {
   ...
   undelegate_bandwidth_subcommand(CLI::App* actionRoot) {
      ...
      undelegate_bandwidth->add_option("unstake_net_quantity", unstake_net_amount, localized("The amount of DATX to undelegate for network bandwidth"))->required();
      undelegate_bandwidth->add_option("unstake_cpu_quantity", unstake_cpu_amount, localized("The amount of DATX to undelegate for CPU bandwidth"))->required();
...
struct bidname_subcommand {
   ...
   bidname_subcommand(CLI::App *actionRoot) {
      ...
      bidname->add_option("bid", bid_amount, localized("The amount of DATX to bid"))->required();
   ...
   bidname_info_subcommand(CLI::App* actionRoot) {
      ...
      list_producers->set_callback([this] {
         auto rawResult = call(get_table_func, fc::mutable_variant_object("json", true)
                               ("code", "datxos")("scope", "datxos")("table", "namebids")
                               ("lower_bound", datxos::chain::string_to_name(newname_str.c_str()))("limit", 1));
         if ( print_json ) {
            std::cout << fc::json::to_pretty_string(rawResult) << std::endl;
            return;
         }
         auto result = rawResult.as<datxos::chain_apis::read_only::get_table_rows_result>();
         if ( result.rows.empty() ) {
...
struct buyram_subcommand {
   ...
   buyram_subcommand(CLI::App* actionRoot) {
      ...
      buyram->add_option("amount", amount, localized("The amount of DATX to pay for RAM, or number of kbytes of RAM if --kbytes is set"))->required();
...
struct sellram_subcommand {
   ...
   sellram_subcommand(CLI::App* actionRoot) {
      ...
      sellram->add_option("account", receiver_str, localized("The account to receive DATX for sold RAM"))->required();
...
void get_account( const string& accountName, bool json_format ) {
   ...
   auto res = json.as<datxos::chain_apis::read_only::get_account_results>();
   ...
   if (!json_format) {
      ...
      unordered_map<name, datxos::chain_apis::permission> cache;
      ...
      if ( res.total_resources.is_object() ) {
         ...
         if( net_total.get_symbol() != unstaking.get_symbol() ) {
            // Core symbol of noddatx responding to the request is different than core symbol built into cldatx
      ...
      if( res.refund_request.is_object() ) {
         ...
         if( unstaking > asset( 0, unstaking.get_symbol() ) ) {
            ...
            if( now >= refund_time ) {
               std::cout << " (available to claim now with 'datxos::refund' action)\n";
            } else {
...
int main( int argc, char** argv ) {
   ...
   context = datxos::client::http::create_http_context();

   CLI::App app{"Command Line Interface to DATXOS Client"};
   app.require_subcommand();
   app.add_option( "-H,--host", obsoleted_option_host_port, localized("the host where noddatx is running") )->group("hidden");
   app.add_option( "-p,--port", obsoleted_option_host_port, localized("the port where noddatx is running") )->group("hidden");
   app.add_option( "--wallet-host", obsoleted_option_host_port, localized("the host where kdatxd is running") )->group("hidden");
   app.add_option( "--wallet-port", obsoleted_option_host_port, localized("the port where kdatxd is running") )->group("hidden");

   app.add_option( "-u,--url", url, localized("the http/https URL where noddatx is running"), true );
   app.add_option( "--wallet-url", wallet_url, localized("the http/https URL where kdatxd is running"), true );

   app.add_option( "-r,--header", header_opt_callback, localized("pass specific HTTP header; repeat this option to pass multiple headers"));
   app.add_flag( "-n,--no-verify", no_verify, localized("don't verify peer certificate when using HTTPS"));
   app.set_callback([&app]{ ensure_kdatxd_running(&app);});
   ...
   version->add_subcommand("client", localized("Retrieve version information of the client"))->set_callback([] {
     std::cout << localized("Build version: ${ver}", ("ver", datxos::client::config::version_str)) << std::endl;
   });
   ...
   pack_transaction->add_flag("--pack-action-data", pack_action_data_flag, localized("Pack all action data within transaction, needs interaction with noddatx"));
   pack_transaction->set_callback([&] {
      fc::variant trx_var;
      try {
         trx_var = json_from_file_or_string( plain_signed_transaction_json );
      } DATX_RETHROW_EXCEPTIONS( transaction_type_exception, "Fail to parse plain transaction JSON '${data}'", ("data", plain_signed_transaction_json))
      if( pack_action_data_flag ) {
         ...
         } DATX_RETHROW_EXCEPTIONS( transaction_type_exception, "Fail to convert transaction, --pack-action-data likely needed" )
      }
   });
   ...
   unpack_transaction->add_flag("--unpack-action-data", unpack_action_data_flag, localized("Unpack all action data within transaction, needs interaction with noddatx"));
   unpack_transaction->set_callback([&] {
      ...
      } DATX_RETHROW_EXCEPTIONS( transaction_type_exception, "Fail to parse packed transaction JSON '${data}'", ("data", packed_transaction_json))
   ...
   pack_action_data->set_callback([&] {
      ...
      } DATX_RETHROW_EXCEPTIONS(transaction_type_exception, "Fail to parse unpacked action data JSON")
   ...
   unpack_action_data->set_callback([&] {
      DATX_ASSERT( packed_action_data_string.size() >= 2, transaction_type_exception, "No packed_action_data found" );
   ...
   getCode->set_callback([&] {
      ...
      try {
      ...
      catch(chain::missing_core_api_accessory_exception&) {
         ...
         if(code_as_wasm) {
            ...
            std::cout << localized("Warning: communicating to older noddatx which returns malformed binary wasm") << std::endl;
         }
         else
   ...
   getAccounts->set_callback([&] {
      ...
      } DATX_RETHROW_EXCEPTIONS(public_key_type_exception, "Invalid public key: ${public_key}", ("public_key", public_key_str))
   ...
   getTransaction->set_callback([&] {
      ...
      } DATX_RETHROW_EXCEPTIONS(transaction_id_type_exception, "Invalid transaction ID: ${transaction_id}", ("transaction_id", transaction_id_str))
   ...
   getTransactions->set_callback([&] {
      ...
      if (skip_seq_str.empty()) {
         ...
         } DATX_RETHROW_EXCEPTIONS(chain_type_exception, "Invalid Skip Seq: ${skip_seq}", ("skip_seq", skip_seq_str))
         if (num_seq_str.empty()) {
            ...
            } DATX_RETHROW_EXCEPTIONS(chain_type_exception, "Invalid Num Seq: ${num_seq}", ("num_seq", num_seq_str))
...
//                                ->check(CLI::ExistingFile);
   ...
   auto set_code_callback = [&]() {
      ...
      DATX_ASSERT( !wasm.empty(), wast_file_not_found, "no wasm file found ${f}", ("f", wasmPath) );
   ...
   auto set_abi_callback = [&]() {
      ...
      DATX_ASSERT( fc::exists( abiPath ), abi_file_not_found, "no abi file found ${f}", ("f", abiPath)  );

      try {
         actions.emplace_back( create_setabi(account, fc::json::from_file(abiPath).as<abi_def>()) );
      } DATX_RETHROW_EXCEPTIONS(abi_type_exception,  "Fail to parse ABI JSON")
      if ( shouldSend ) {
   ...
   // Transfer subcommand
   string con = "datxos.token";
   string sender;
   string recipient;
   string amount;
   string memo;
   auto transfer = app.add_subcommand("transfer", localized("Transfer DATX from account to account"), false);
   transfer->add_option("sender", sender, localized("The account sending DATX"))->required();
   transfer->add_option("recipient", recipient, localized("The account receiving DATX"))->required();
   transfer->add_option("amount", amount, localized("The amount of DATX to send"))->required();
   ...
   importWallet->set_callback([&wallet_name, &wallet_key_str] {
      ...
      try {
         ...
         DATX_THROW(private_key_type_exception, "Invalid private key: ${private_key}", ("private_key", wallet_key_str))
      }
   ...
   removeKeyWallet->set_callback([&wallet_name, &wallet_pw, &wallet_rm_key_str] {
      ...
      try {
         ...
         DATX_THROW(public_key_type_exception, "Invalid public key: ${public_key}", ("public_key", wallet_rm_key_str))
      }
   ...
   auto stopKdatxd = wallet->add_subcommand("stop", localized("Stop kdatxd (doesn't work with noddatx)."), false);
   stopKdatxd->set_callback([] {
      const auto& v = call(wallet_url, kdatxd_stop);
      if ( !v.is_object() || v.get_object().size() != 0 ) { //on success kdatxd responds with empty object
   ...
   sign->set_callback([&] {
      ...
      if( str_chain_id.size() == 0 ) {
         ilog( "grabbing chain_id from noddatx" );
         auto info = get_info();
   ...
   actionsSubcommand->set_callback([&] {
      ...
      if( !data.empty() ) {
         ...
         } DATX_RETHROW_EXCEPTIONS(action_type_exception, "Fail to parse action JSON data='${data}'", ("data", data))
      }
   ...
   trxSubcommand->set_callback([&] {
      ...
      } DATX_RETHROW_EXCEPTIONS(transaction_type_exception, "Fail to parse transaction JSON '${data}'", ("data",trx_to_push))
      try {
         signed_transaction trx = trx_var.as<signed_transaction>();
   ...
   trxsSubcommand->set_callback([&] {
      ...
      } DATX_RETHROW_EXCEPTIONS(transaction_type_exception, "Fail to parse transaction JSON '${data}'", ("data",trxsJson))
   ...
   propose_action->set_callback([&] {
      ...
      } DATX_RETHROW_EXCEPTIONS(transaction_type_exception, "Fail to parse permissions JSON '${data}'", ("data",requested_perm))
      fc::variant transaction_perm_var;
      try {
         transaction_perm_var = json_from_file_or_string(transaction_perm);
      } DATX_RETHROW_EXCEPTIONS(transaction_type_exception, "Fail to parse permissions JSON '${data}'", ("data",transaction_perm))
      fc::variant trx_var;
      try {
         trx_var = json_from_file_or_string(proposed_transaction);
      } DATX_RETHROW_EXCEPTIONS(transaction_type_exception, "Fail to parse transaction JSON '${data}'", ("data",proposed_transaction))
      ...
      } DATX_RETHROW_EXCEPTIONS(transaction_type_exception, "Wrong requested permissions format: '${data}'", ("data",requested_perm_var));

      vector<permission_level> trxperm;
      try {
         trxperm = transaction_perm_var.as<vector<permission_level>>();
      } DATX_RETHROW_EXCEPTIONS(transaction_type_exception, "Wrong transaction permissions format: '${data}'", ("data",transaction_perm_var));
      ...
      if (accountPermissions.empty()) {
         if (!proposer.empty()) {
            ...
            DATX_THROW(missing_auth_exception, "Authority is not provided (either by multisig parameter <proposer> or -p)");
         }
      }
      if (proposer.empty()) {
      ...
      send_actions({chain::action{accountPermissions, "datxos.msig", "propose", variant_to_bin( N(datxos.msig), N(propose), args ) }});
   });
   ...
   review->set_callback([&] {
      auto result = call(get_table_func, fc::mutable_variant_object("json", true)
                         ...
                         ("table", "proposal")
                         ("table_key", "")
                         ("lower_bound", datxos::chain::string_to_name(proposal_name.c_str()))
                         ("upper_bound", "")
                         ("limit", 1)
                         );
      //std::cout << fc::json::to_pretty_string(result) << std::endl;
   ...
   auto approve_or_unapprove = [&](const string& action) {
      ...
      } DATX_RETHROW_EXCEPTIONS(transaction_type_exception, "Fail to parse permissions JSON '${data}'", ("data",perm))
      auto args = fc::mutable_variant_object()
         ("proposer", proposer)
         ("proposal_name", proposal_name)
         ("level", perm_var);
      ...
      send_actions({chain::action{accountPermissions, "datxos.msig", action, variant_to_bin( N(datxos.msig), action, args ) }});
   };
   ...
   cancel->set_callback([&]() {
      ...
      if (accountPermissions.empty()) {
         if (!canceler.empty()) {
            ...
            DATX_THROW(missing_auth_exception, "Authority is not provided (either by multisig parameter <canceler> or -p)");
         }
      }
      if (canceler.empty()) {
      ...
      send_actions({chain::action{accountPermissions, "datxos.msig", "cancel", variant_to_bin( N(datxos.msig), N(cancel), args ) }});
      }
   );
   ...
   exec->set_callback([&] {
      ...
      if (accountPermissions.empty()) {
         if (!executer.empty()) {
            ...
            DATX_THROW(missing_auth_exception, "Authority is not provided (either by multisig parameter <executer> or -p)");
         }
      }
      if (executer.empty()) {
      ...
      send_actions({chain::action{accountPermissions, "datxos.msig", "exec", variant_to_bin( N(datxos.msig), N(exec), args ) }});
      }
   );
   ...
   sudo_exec->set_callback([&] {
      ...
      } DATX_RETHROW_EXCEPTIONS(transaction_type_exception, "Fail to parse transaction JSON '${data}'", ("data",trx_to_exec))

      auto accountPermissions = get_account_permissions(tx_permission);
      if( accountPermissions.empty() ) {
         accountPermissions = vector<permission_level>{{executer, config::active_name}, {"datxos.sudo", config::active_name}};
      }
      ...
      send_actions({chain::action{accountPermissions, "datxos.sudo", "exec", variant_to_bin( N(datxos.sudo), N(exec), args ) }});
   });

   // system subcommand
   auto system = app.add_subcommand("system", localized("Send datxos.system contract action to the blockchain."), false);
