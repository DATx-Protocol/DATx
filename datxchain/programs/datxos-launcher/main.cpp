...
#include <net/if.h>
#include <datxos/chain/genesis_state.hpp>
...
void
datxd_def::set_host( host_def* h, bool is_bios ) {
  host = h->host_name;
...
struct server_name_def {
  ...
  string datxos_home;
  uint16_t instances;
  server_name_def () : ipaddr(), name(), has_bios(false), datxos_home(), instances(1) {}
};
...
struct launcher_def {
  ...
  producer_set_def producer_set;
   ...
   void assign_name (datxd_def &node, bool is_bios);
  ...
  void prep_remote_config_dir (datxd_def &node, host_def *host);
  void launch (datxd_def &node, string &gts);
  void kill (launch_modes mode, string sig_opt);
  pair<host_def, datxd_def> find_node(uint16_t node_num);
  vector<pair<host_def, datxd_def>> get_nodes(const string& node_number_list);
...
launcher_def::set_options (bpo::options_description &cfg) {
  cfg.add_options()
    ...
    ("p2p-accessory", bpo::value<string>()->default_value("net"),"select a p2p accessory to use (either net or bnet). Defaults to net.")
    ("genesis,g",bpo::value<bfs::path>(&genesis)->default_value("./genesis.json"),"set the path to genesis.json")
    ("skip-signature", bpo::bool_switch(&skip_transaction_signatures)->default_value(false), "noddatx does not require transaction signatures.")
    ("noddatx", bpo::value<string>(&datxd_extra_args), "forward noddatx command line argument(s) to each instance of noddatx, enclose arg in quotes")
    ...
    ("per-host",bpo::value<int>(&per_host)->default_value(0),"specifies how many noddatx instances will run on a single host. Use 0 to indicate all on one.")
...
launcher_def::initialize (const variables_map &vmap) {
  ...
  string nc = vmap["p2p-accessory"].as<string>();
  if ( !nc.empty() ) {
     if (boost::iequals(nc,"net"))
        p2p = bp2p_accessory::NET;
     else if (boost::iequals(nc,"bnet"))
        p2p = bp2p_accessory::BNET;
     else {
        p2p = bp2p_accessory::NET;
     }
  }
  ...
  if( !host_map_file.empty() ) {
    try {
      ...
      for (auto &binding : bindings) {
        for (auto &datxd : binding.instances) {
          datxd.host = binding.host_name;
          datxd.p2p_endpoint = binding.public_name + ":" + boost::lexical_cast<string,uint16_t>(datxd.p2p_port);

          aliases.push_back (datxd.name);
        }
      }
  ...
  char* erd_env_var = getenv ("DATXOS_HOME");
...
void
launcher_def::assign_name (datxd_def &node, bool is_bios) {
...
launcher_def::define_network () {
  ...
  if (per_host == 0) {
    host_def local_host;
    local_host.datxos_home = erd;
    local_host.genesis = genesis.string();
    for (size_t i = 0; i < (total_nodes); i++) {
      datxd_def datxd;
      assign_name(datxd, i == 0);
      aliases.push_back(datxd.name);
      datxd.set_host (&local_host, i == 0);
      local_host.instances.emplace_back(move(datxd));
    }
    ...
    for (size_t i = total_nodes; i > 0; i--) {
      ...
      if (ph_count == 0) {
        ...
        lhost->datxos_home =
          (local_id.contains (lhost->host_name) || servers.default_datxos_home.empty()) ?
          erd : servers.default_datxos_home;
        host_ndx++;
      } // ph_count == 0

      datxd_def datxd;
      assign_name(datxd, do_bios);
      datxd.has_db = false;
      ...
      aliases.push_back(datxd.name);
      datxd.set_host (lhost, do_bios);
      do_bios = false;
      lhost->instances.emplace_back(move(datxd));
      --ph_count;
    } // for i
    bindings.emplace_back( move(*lhost) );
    delete lhost;
...
launcher_def::deploy_config_files (tn_node_def &node) {
  boost::system::error_code ec;
  datxd_def &instance = *node.instance;
  ...
  if (host->is_local()) {
    bfs::path cfgdir = bfs::path(host->datxos_home) / instance.config_dir_name;
    bfs::path dd = bfs::path(host->datxos_home) / instance.data_dir_name;
    ...
    bfs::path rfile = bfs::path (host->datxos_home) / instance.config_dir_name / "config.ini";
    ...
    rfile = bfs::path (host->datxos_home) / instance.config_dir_name / "logging.json";
    ...
    rfile = bfs::path (host->datxos_home) / instance.config_dir_name / "genesis.json";
...
launcher_def::write_config_file (tn_node_def &node) {
   ...
   if (p2p == bp2p_accessory::NET) {
      ...
      // Include the p2p_net_accessory endpoint, because the accessory is always loaded (even if not used).
  ...
  if (instance.has_db || node.producers.size()) {
    ...
    cfg << "accessory = datxos::producer_accessory\n";
  }
  if( instance.has_db ) {
    cfg << "accessory = datxos::mongo_db_accessory\n";
  }
  if ( p2p == bp2p_accessory::NET ) {
    cfg << "accessory = datxos::p2p_net_accessory\n";
  } else {
    cfg << "accessory = datxos::bp2p_accessory\n";
  }
  cfg << "accessory = datxos::core_api_accessory\n"
...
launcher_def::write_logging_config_file(tn_node_def &node) {
  ...
  if(gelf_enabled) {
    ...
    fc::logger_config p2p ("p2p_net_accessory_impl");
...
launcher_def::init_genesis () {
  bfs::path genesis_path = bfs::current_path() / "genesis.json";
   ...
   if (!src.good()) {
      ...
      datxos::chain::genesis_state default_genesis;
...
void
launcher_def::prep_remote_config_dir (datxd_def &node, host_def *host) {
  bfs::path abs_config_dir = bfs::path(host->datxos_home) / node.config_dir_name;
  bfs::path abs_data_dir = bfs::path(host->datxos_home) / node.data_dir_name;

  string acd = abs_config_dir.string();
  string add = abs_data_dir.string();
  string cmd = "cd " + host->datxos_home;

  cmd = "cd " + host->datxos_home;
  if (!do_ssh(cmd, host->host_name)) {
    cerr << "Unable to switch to path " << host->datxos_home
...
void
launcher_def::launch (datxd_def &instance, string &gts) {
  ...
  string datxdcmd = "programs/noddatx/noddatx ";
  if (skip_transaction_signatures) {
    datxdcmd += "--skip-transaction-signatures ";
  }
  if (!datxd_extra_args.empty()) {
    if (instance.name == "bios") {
       // Strip the mongo-related options out of the bios node so
       // the accessorys don't conflict between 00 and bios.
       regex r("--accessory +datxos::mongo_db_accessory");
       string args = std::regex_replace (datxd_extra_args,r,"");
       regex r2("--mongodb-uri +[^ ]+");
       args = std::regex_replace (args,r2,"");
       datxdcmd += args + " ";
    }
    else {
       datxdcmd += datxd_extra_args + " ";
    }
  }

  if( add_enable_stale_production ) {
    datxdcmd += "--enable-stale-production true ";
    add_enable_stale_production = false;
  }

  datxdcmd += " --config-dir " + instance.config_dir_name + " --data-dir " + instance.data_dir_name;
  datxdcmd += " --genesis-json " + instance.config_dir_name + "/genesis.json";
  if (gts.length()) {
    datxdcmd += " --genesis-timestamp " + gts;
  }

  if (!host->is_local()) {
    string cmdl ("cd ");
    cmdl += host->datxos_home + "; nohup " + datxdcmd + " > "
    ...
    string cmd = "cd " + host->datxos_home + "; kill -15 $(cat " + pidf.string() + ")";
    format_ssh (cmd, host->host_name, info.kill_cmd);
  }
  else {
    cerr << "spawning child, " << datxdcmd << endl;

    bp::child c(datxdcmd, bp::std_out > reout, bp::std_err > reerr );
    bfs::remove(reerr_sl);
    ...
    if(!c.running()) {
      cerr << "child not running after spawn " << datxdcmd << endl;
      for (int i = 0; i > 0; i++) {
        if (c.running () ) break;
      }
    }
...
#if 0
void
launcher_def::kill_instance(datxd_def, string sig_opt) {
}
#endif
...
vector<pair<host_def, datxd_def>>
launcher_def::get_nodes(const string& node_number_list) {
   vector<pair<host_def, datxd_def>> node_list;
...
launcher_def::bounce (const string& node_numbers) {
   ...
   for (auto node_pair: node_list) {
      ...
      const datxd_def& node = node_pair.second;
      string node_num = node.name.substr( node.name.length() - 2 );
      string cmd = "cd " + host.datxos_home + "; "
                 + "export DATXOS_HOME=" + host.datxos_home + string("; ")
                 + "export DATXOS_NODE=" + node_num + "; "
                 + "./scripts/datxos-tn_bounce.sh " + datxd_extra_args;
...
launcher_def::down (const string& node_numbers) {
   ...
   for (auto node_pair: node_list) {
      ...
      string node_num = node.name.substr( node.name.length() - 2 );
      string cmd = "cd " + host.datxos_home + "; "
                 + "export DATXOS_HOME=" + host.datxos_home + "; "
                 + "export DATXOS_NODE=" + node_num + "; "
         + "export DATXOS_TN_RESTART_CONFIG_DIR=" + node.config_dir_name + "; "
                 + "./scripts/datxos-tn_down.sh";
...
int main (int argc, char *argv[]) {
  ...
  cli.add_options()
    ...
    ("down", bpo::value<string>(&down_nodes),"comma-separated list of node numbers that will be taken down using the datxos-tn_down.sh script")
    ("bounce", bpo::value<string>(&bounce_nodes),"comma-separated list of node numbers that will be restarted using the datxos-tn_bounce.sh script")
    ("roll", bpo::value<string>(&roll_nodes),"comma-separated list of host names where the nodes should be rolled to a new version using the datxos-tn_roll.sh script")
  ...
  try {
    ...
    if (vmap.count("version") > 0) {
      cout << datxos::launcher::config::version_str << endl;
      return 0;
    }
...
FC_REFLECT( server_name_def, (ipaddr) (name) (has_bios) (datxos_home) (instances) )

FC_REFLECT( server_identities, (producer) (nonprod) (db) (default_datxos_home) (ssh) )
