/**
 *  @file
 *  @copyright defined in datxos/LICENSE.txt
 */
...
{
   try {
      app().set_version(datxos::noddatx::config::version);
      app().register_accessory<history_accessory>();

      auto root = fc::app_path();
      app().set_default_data_dir(root / "datxos/noddatx/data" );
      app().set_default_config_dir(root / "datxos/noddatx/config" );
      if(!app().initialize<core_accessory, http_accessory, p2p_net_accessory, producer_accessory>(argc, argv))
         return INITIALIZE_FAIL;
      initialize_logging();
      //ilog("noddatx version ${ver}", ("ver", app().version_string()));
      ilog("datxos root is ${root}", ("root", root.string()));
      app().startup();
      app().exec();
