...
#include <datxos/chain/block_summary_object.hpp>
#include <datxos/chain/global_property_object.hpp>
#include <datxos/chain/contract_table_objects.hpp>
...
struct controller_impl {
   ...
   void pop_block() {
      ...
      DATX_ASSERT( prev, block_validate_exception, "attempt to pop beyond last irreversible block" );
...
#define SET_APP_HANDLER( receiver, contract, action) \
   ...
   SET_APP_HANDLER( datxos, datxos, newaccount );
   SET_APP_HANDLER( datxos, datxos, setcode );
   SET_APP_HANDLER( datxos, datxos, setabi );
   SET_APP_HANDLER( datxos, datxos, updateauth );
   SET_APP_HANDLER( datxos, datxos, deleteauth );
   SET_APP_HANDLER( datxos, datxos, linkauth );
   SET_APP_HANDLER( datxos, datxos, unlinkauth );
/*
   SET_APP_HANDLER( datxos, datxos, postrecovery );
   SET_APP_HANDLER( datxos, datxos, passrecovery );
   SET_APP_HANDLER( datxos, datxos, vetorecovery );
*/

   SET_APP_HANDLER( datxos, datxos, canceldelay );
   ...
   /**
    *  accessorys / observers listening to signals emited (such as accepted_transaction) might trigger
   ...
   void on_irreversible( const block_state_ptr& s ) {
      ...
      DATX_ASSERT( log_head, block_log_exception, "block log head can not be found" );
...
//         edump((s->block_num)(s->block->previous)(log_head->id()));
      ...
      DATX_ASSERT( s->block_num - 1  == lh_block_num, unlinkable_block_exception, "unlinkable block", ("s->block_num",s->block_num)("lh_block_num", lh_block_num) );
      DATX_ASSERT( s->block->previous == log_head->id(), unlinkable_block_exception, "irreversible doesn't link to block log head" );
      blog.append(s->block);
   ...
   void init() {
      ...
      */
      ...
      if( objitr != ubi.rend() ) {
         DATX_ASSERT( objitr->blocknum == head->block_num, fork_database_exception,
         ...
         auto end = blog.read_head();
         DATX_ASSERT( end && end->block_num() == head->block_num, fork_database_exception,
                    "fork database exists but reversible block database does not, replay blockchain",
                    ("blog_head",end->block_num())("head",head->block_num)  );
      }

      DATX_ASSERT( db.revision() >= head->block_num, fork_database_exception, "fork database is inconsistent with shared memory",
   ...
   void create_native_account( account_name name, const authority& owner, const authority& active, bool is_privileged = false ) {
      db.create<account_object>([&](auto& a) {
         ...
         if( name == config::system_account_name ) {
            a.set_abi(datxos_contract_abi(abi_def()));
         }
      });
   ...
   void commit_block( bool add_to_fork_db ) {
      ...
      try {
         if (add_to_fork_db) {
            ...
            head = fork_db.head();
            DATX_ASSERT(new_bsp == head, fork_database_exception, "committed block did not become the new head in fork database");
         }
   ...
   transaction_trace_ptr push_scheduled_transaction( const transaction_id_type& trxid, fc::time_point deadline, uint32_t billed_cpu_time_us, bool explicit_billed_cpu_time = false ) {
      ...
      DATX_ASSERT( itr != idx.end(), unknown_transaction_exception, "unknown transaction" );
   ...
   { try {
      ...
      DATX_ASSERT( gtrx.delay_until <= self.pending_block_time(), transaction_exception, "this transaction isn't ready",
   ...
   const transaction_receipt& push_receipt( const T& trx, transaction_receipt_header::status_enum status,
      ...
      DATX_ASSERT( net_usage_words*8 == net_usage, transaction_exception, "net_usage is not divisible by 8" );
   ...
   transaction_trace_ptr push_transaction( const transaction_metadata_ptr& trx,
                                           ...
                                           bool explicit_billed_cpu_time = false )
   {
      DATX_ASSERT(deadline != fc::time_point(), transaction_exception, "deadline cannot be uninitialized");
   ...
   void start_block( block_timestamp_type when, uint16_t confirm_block_count, controller::block_status s ) {
      DATX_ASSERT( !pending, block_validate_exception, "pending block already exists" );
      ...
      if (!self.skip_db_sessions(s)) {
         DATX_ASSERT( db.revision() == head->block_num, database_exception, "db revision is not on par with head block",
   ...
   void apply_block( const signed_block_ptr& b, controller::block_status s ) { try {
      try {
         DATX_ASSERT( b->block_extensions.size() == 0, block_validate_exception, "no supported extensions" );
         ...
         for( const auto& receipt : b->transactions ) {
            ...
            if( receipt.trx.contains<packed_transaction>() ) {
               ...
               DATX_ASSERT( false, block_validate_exception, "encountered unexpected receipt type" );
            }
            ...
            DATX_ASSERT( pending->_pending_block_state->block->transactions.size() > 0,
                        block_validate_exception, "expected a receipt",
                        ("block", *b)("expected_receipt", receipt)
                      );
            DATX_ASSERT( pending->_pending_block_state->block->transactions.size() == num_pending_receipts + 1,
            ...
            DATX_ASSERT( r == static_cast<const transaction_receipt_header&>(receipt),
         ...
         DATX_ASSERT(b->id() == pending->_pending_block_state->header.id(),
   ...
   void push_block( const signed_block_ptr& b, controller::block_status s ) {
    //  idump((fc::json::to_pretty_string(*b)));
      DATX_ASSERT(!pending, block_validate_exception, "it is not valid to push a block when there is a pending block");
      try {
         DATX_ASSERT( b, block_validate_exception, "trying to push empty block" );
         DATX_ASSERT( s != controller::block_status::incomplete, block_validate_exception, "invalid block status for a completed block" );
   ...
   void push_confirmation( const header_confirmation& c ) {
      DATX_ASSERT(!pending, block_validate_exception, "it is not valid to push a confirmation when there is a pending block");
      fork_db.add( c );
   ...
   void maybe_switch_forks( controller::block_status s = controller::block_status::complete ) {
      ...
      if( new_head->header.previous == head->id ) {
         ...
         DATX_ASSERT( self.head_block_id() == branches.second.back()->header.previous, fork_database_exception,
   ...
   void finalize_block()
   {
      DATX_ASSERT(pending, block_validate_exception, "it is not valid to finalize when there is no pending block");
      try {
      ...
      */
      ...
      uint64_t CPU_TARGET = DATX_PERCENT(chain_config.max_block_cpu_usage, chain_config.target_block_cpu_usage_pct);
      resource_limits.set_block_parameters(
         ...
         {DATX_PERCENT(chain_config.max_block_net_usage, chain_config.target_block_net_usage_pct), chain_config.max_block_net_usage, config::block_size_average_window_ms / config::block_interval_ms, max_virtual_mult, {99, 100}, {1000, 999}}
      );
   ...
   void check_actor_list( const flat_set<account_name>& actors )const {
      if( conf.actor_whitelist.size() > 0 ) {
         ...
         DATX_ASSERT( excluded.size() == 0, actor_whitelist_exception,
         ...
         DATX_ASSERT( blacklisted.size() == 0, actor_blacklist_exception,
   ...
   void check_contract_list( account_name code )const {
      if( conf.contract_whitelist.size() > 0 ) {
         DATX_ASSERT( conf.contract_whitelist.find( code ) != conf.contract_whitelist.end(),
                     contract_whitelist_exception,
                     "account '${code}' is not on the contract whitelist", ("code", code)
                   );
      } else if( conf.contract_blacklist.size() > 0 ) {
         DATX_ASSERT( conf.contract_blacklist.find( code ) == conf.contract_blacklist.end(),
   ...
   void check_action_list( account_name code, action_name action )const {
      if( conf.action_blacklist.size() > 0 ) {
         DATX_ASSERT( conf.action_blacklist.find( std::make_pair(code, action) ) == conf.action_blacklist.end(),
   ...
   void check_key_list( const public_key_type& key )const {
      if( conf.key_blacklist.size() > 0 ) {
         DATX_ASSERT( conf.key_blacklist.find( key ) == conf.key_blacklist.end(),
   ...
   void validate_tapos( const transaction& trx )const {
      ...
      DATX_ASSERT(trx.verify_reference_block(tapos_block_summary.block_id), invalid_ref_block_exception,
...
transaction_trace_ptr controller::push_transaction( const transaction_metadata_ptr& trx, fc::time_point deadline, uint32_t billed_cpu_time_us ) {
   validate_db_available_size();
   DATX_ASSERT( get_read_mode() != chain::db_read_mode::READ_ONLY, transaction_type_exception, "push transaction not allowed in read-only mode" );
   DATX_ASSERT( trx && !trx->implicit && !trx->scheduled, transaction_type_exception, "Implicit/Scheduled transaction not allowed" );
...
void controller::set_action_blacklist( const flat_set< pair<account_name, action_name> >& new_action_blacklist ) {
   for (auto& act: new_action_blacklist) {
      DATX_ASSERT(act.first != account_name(), name_type_exception, "Action blacklist - contract name should not be empty");
      DATX_ASSERT(act.second != action_name(), action_type_exception, "Action blacklist - action name should not be empty");
   }
...
time_point controller::pending_block_time()const {
   DATX_ASSERT( my->pending, block_validate_exception, "no pending block" );
...
block_id_type controller::get_block_id_for_num( uint32_t block_num )const { try {
   ...
   DATX_ASSERT( BOOST_LIKELY( signed_blk != nullptr ), unknown_block_exception,
...
vector<transaction_metadata_ptr> controller::get_unapplied_transactions() const {
   ...
   if ( my->read_mode == db_read_mode::SPECULATIVE ) {
      ...
      DATX_ASSERT( my->unapplied_transactions.empty(), transaction_exception, "not empty unapplied_transactions in non-speculative mode" ); //should never happen
   }
   return result;
}
...
void controller::validate_referenced_accounts( const transaction& trx )const {
   for( const auto& a : trx.context_free_actions ) {
      ...
      DATX_ASSERT( code != nullptr, transaction_exception,
                  "action's code account '${account}' does not exist", ("account", a.account) );
      DATX_ASSERT( a.authorization.size() == 0, transaction_exception,
   ...
   for( const auto& a : trx.actions ) {
      ...
      for( const auto& auth : a.authorization ) {
         ...
         DATX_ASSERT( actor  != nullptr, transaction_exception,
                     "action's authorizing actor '${account}' does not exist", ("account", auth.actor) );
         DATX_ASSERT( my->authorization.find_permission(auth) != nullptr, transaction_exception,
                     "action's authorizations include a non-existent permission: {permission}",
                     ("permission", auth) );
      }
   }
   DATX_ASSERT( one_auth, tx_no_auths, "transaction must have at least one authorization" );
}

void controller::validate_expiration( const transaction& trx )const { try {
   const auto& chain_configuration = get_global_properties().configuration;

   DATX_ASSERT( time_point(trx.expiration) >= pending_block_time(),
               expired_tx_exception,
               "transaction has expired, "
               "expiration is ${trx.expiration} and pending block time is ${pending_block_time}",
               ("trx.expiration",trx.expiration)("pending_block_time",pending_block_time()));
   DATX_ASSERT( time_point(trx.expiration) <= pending_block_time() + fc::seconds(chain_configuration.max_transaction_lifetime),
...
void controller::validate_db_available_size() const {
   ...
   DATX_ASSERT(free >= guard, database_guard_exception, "database free: ${f}, guard size: ${g}", ("f", free)("g",guard));
}
...
void controller::validate_reversible_available_size() const {
   ...
   DATX_ASSERT(free >= guard, reversible_guard_exception, "reversible free: ${f}, guard size: ${g}", ("f", free)("g",guard));
}
