...
#include <datxos/chain/authority_checker.hpp>
...
namespace datxos { namespace chain {
   ...
   { try {
      DATX_ASSERT( !level.actor.empty() && !level.permission.empty(), invalid_permission, "Invalid permission" );
      return _db.find<permission_object, by_owner>( boost::make_tuple(level.actor,level.permission) );
   } DATX_RETHROW_EXCEPTIONS( chain::permission_query_exception, "Failed to retrieve permission: ${level}", ("level", level) ) }
                                                                             ...
                                                                             )const
   {
      ...
      if( scope == config::system_account_name ) {
          DATX_ASSERT( act_name != updateauth::get_name() &&
      ...
      try {
         ...
         if( *linked_permission == config::datxos_any_name )
            return optional<permission_name>();
   ...
   {
      DATX_ASSERT( auths.size() == 1, irrelevant_auth_exception,
                  "updateauth action should only have one declared authorization" );
      const auto& auth = auths[0];
      DATX_ASSERT( auth.actor == update.account, irrelevant_auth_exception,
      ...
      DATX_ASSERT( get_permission(auth).satisfies( *min_permission,
                                                             ...
                                                             )const
   {
      ...
      const auto& auth = auths[0];
      DATX_ASSERT( auth.actor == del.account, irrelevant_auth_exception,
                                                           ...
                                                           )const
   {
      ...
      const auto& auth = auths[0];
      DATX_ASSERT( auth.actor == link.account, irrelevant_auth_exception,
                  "the owner of the linked permission needs to be the actor of the declared authorization" );

      DATX_ASSERT( link.type != updateauth::get_name(),  action_validate_exception,
                  "Cannot link datxos::updateauth to a minimum permission" );
      DATX_ASSERT( link.type != deleteauth::get_name(),  action_validate_exception,
                  "Cannot link datxos::deleteauth to a minimum permission" );
      DATX_ASSERT( link.type != linkauth::get_name(),    action_validate_exception,
                  "Cannot link datxos::linkauth to a minimum permission" );
      DATX_ASSERT( link.type != unlinkauth::get_name(),  action_validate_exception,
                  "Cannot link datxos::unlinkauth to a minimum permission" );
      DATX_ASSERT( link.type != canceldelay::get_name(), action_validate_exception,
                  "Cannot link datxos::canceldelay to a minimum permission" );

      const auto linked_permission_name = lookup_minimum_permission(link.account, link.code, link.type);

      if( !linked_permission_name ) // if action is linked to datxos.any permission
         return;

      DATX_ASSERT( get_permission(auth).satisfies( get_permission({link.account, *linked_permission_name}),
                                                  _db.get_index<permission_index>().indices()              ),
                  irrelevant_auth_exception,
                                                             ...
                                                             )const
   {
      ...
      const auto& auth = auths[0];
      DATX_ASSERT( auth.actor == unlink.account, irrelevant_auth_exception,
                  "the owner of the linked permission needs to be the actor of the declared authorization" );

      const auto unlinked_permission_name = lookup_linked_permission(unlink.account, unlink.code, unlink.type);
      DATX_ASSERT( unlinked_permission_name.valid(), transaction_exception,
      ...
      if( *unlinked_permission_name == config::datxos_any_name )
         return;

      DATX_ASSERT( get_permission(auth).satisfies( get_permission({unlink.account, *unlinked_permission_name}),
                                                  _db.get_index<permission_index>().indices()                  ),
                  irrelevant_auth_exception,
                                                                          ...
                                                                          )const
   {
      ...
      DATX_ASSERT( get_permission(auth).satisfies( get_permission(cancel.canceling_auth),
                                                  _db.get_index<permission_index>().indices() ),
                  irrelevant_auth_exception,
      ...
      DATX_ASSERT( itr != generated_index.end() && itr->sender == account_name() && itr->trx_id == trx_id,
                  tx_not_found,
      ...
      DATX_ASSERT( found, action_validate_exception,
                                             ...
                                             )const
   {
      ...
      for( const auto& act : actions ) {
         ...
         for( const auto& declared_auth : act.authorization ) {
            ...
            if( !special_case ) {
               ...
               if( min_permission_name ) { // since special cases were already handled, it should only be false if the permission is datxos.any
                  const auto& min_permission = get_permission({declared_auth.actor, *min_permission_name});
                  DATX_ASSERT( get_permission(declared_auth).satisfies( min_permission,
                                                                       _db.get_index<permission_index>().indices() ),
                              irrelevant_auth_exception,
      ...
      for( const auto& p : permissions_to_satisfy ) {
         ...
         DATX_ASSERT( checker.satisfied( p.first, p.second ), unsatisfied_authorization,
      ...
      if( !allow_unused_keys ) {
         DATX_ASSERT( checker.all_keys_used(), tx_irrelevant_sig,
                                             ...
                                             )const
   {
      ...
      DATX_ASSERT( checker.satisfied({account, permission}), unsatisfied_authorization,
                                                                     ...
                                                                     )const
   {
      ...
      for (const auto& act : trx.actions ) {
         for (const auto& declared_auth : act.authorization) {
            DATX_ASSERT( checker.satisfied(declared_auth), unsatisfied_authorization,
