...
namespace datxos { namespace chain { namespace config {
...
const static uint64_t system_account_name    = N(datxos);
const static uint64_t null_account_name      = N(datxos.null);
const static uint64_t producers_account_name = N(datxos.prods);
...
const static uint64_t datxos_auth_scope       = N(datxos.auth);
const static uint64_t datxos_all_scope        = N(datxos.all);

const static uint64_t active_name = N(active);
const static uint64_t owner_name  = N(owner);
const static uint64_t datxos_any_name = N(datxos.any);
const static uint64_t datxos_code_name = N(datxos.code);
...
const static datxos::chain::wasm_interface::vm_type default_wasm_runtime = datxos::chain::wasm_interface::vm_type::binaryen;
...
} } } // namespace datxos::chain::config

constexpr uint64_t DATX_PERCENT(uint64_t value, uint32_t percentage) {
   return (value * percentage) / datxos::chain::config::percent_100;
}

template<typename Number>
Number DATX_PERCENT_CEIL(Number value, uint32_t percentage) {
   return ((value * percentage) + datxos::chain::config::percent_100 - datxos::chain::config::percent_1)  / datxos::chain::config::percent_100;
}
