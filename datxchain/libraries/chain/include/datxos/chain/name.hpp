...
#define N(X) datxos::chain::string_to_name(#X)
...
namespace std {
   template<> struct hash<datxos::chain::name> : private hash<uint64_t> {
      typedef datxos::chain::name argument_type;
...
namespace fc {
  class variant;
  void to_variant(const datxos::chain::name& c, fc::variant& v);
  void from_variant(const fc::variant& v, datxos::chain::name& check);
} // fc


FC_REFLECT( datxos::chain::name, (value) )
