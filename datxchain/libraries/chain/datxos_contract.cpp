...
#include <datxos/chain/permission_object.hpp>
#include <datxos/chain/permission_link_object.hpp>
...
#include <datxos/chain/abi_serializer.hpp>
...
void validate_authority_precondition( const apply_context& context, const authority& auth ) {
   for(const auto& a : auth.accounts) {
      ...
      if( a.permission.permission == config::datxos_code_name ) // virtual datxos.code permission does not really exist but is allowed
         continue;
 ...
 */
void apply_datxos_newaccount(apply_context& context) {
   auto create = context.act.data_as<newaccount>();
   try {
   context.require_authorization(create.creator);
//   context.require_write_lock( config::datxos_auth_scope );
   ...
   if( !creator.privileged ) {
      DATX_ASSERT( name_str.find( "datxos." ) != 0, action_validate_exception,
                  "only privileged accounts can have names that start with 'datxos.'" );
   }
...
void apply_datxos_setcode(apply_context& context) {
...
void apply_datxos_setabi(apply_context& context) {
   auto& db  = context.db;
...
void apply_datxos_updateauth(apply_context& context) {
   ...
   DATX_ASSERT( update.permission.to_string().find( "datxos." ) != 0, action_validate_exception,
               "Permission names that start with 'datxos.' are reserved" );
...
void apply_datxos_deleteauth(apply_context& context) {
//   context.require_write_lock( config::datxos_auth_scope );
...
void apply_datxos_linkauth(apply_context& context) {
//   context.require_write_lock( config::datxos_auth_scope );
   ...
   try {
      ...
      if( requirement.requirement != config::datxos_any_name ) {
...
void apply_datxos_unlinkauth(apply_context& context) {
//   context.require_write_lock( config::datxos_auth_scope );
...
void apply_datxos_canceldelay(apply_context& context) {
