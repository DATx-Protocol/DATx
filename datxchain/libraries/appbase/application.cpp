...
void application::startup() {
   try {
      for (auto accessory : initialized_accessorys)
         accessory->startup();
   } catch(...) {
      shutdown();
      throw;
...
{
   for(auto& plug : accessorys) {
      boost::program_options::options_description accessory_cli_opts("Command Line Options for " + plug.second->name());
      boost::program_options::options_description accessory_cfg_opts("Config Options for " + plug.second->name());
      plug.second->set_program_options(accessory_cli_opts, accessory_cfg_opts);
      if(accessory_cfg_opts.options().size()) {
         my->_app_options.add(accessory_cfg_opts);
         my->_cfg_options.add(accessory_cfg_opts);
      }
      if(accessory_cli_opts.options().size())
         my->_app_options.add(accessory_cli_opts);
   }
   ...
   app_cfg_opts.add_options()
         ("accessory", bpo::value< vector<string> >()->composing(), "Accessory(s) to enable, may be specified multiple times");
...
bool application::initialize_impl(int argc, char** argv, vector<abstract_accessory*> autostart_accessorys) {
   set_program_options();
  ...
  //  bfs::path config_file_name = workaround;
   bfs::path config_file_name = "config.ini";
   ...
   if(options.count("accessory") > 0)
   {
      auto accessorys = options.at("accessory").as<std::vector<std::string>>();
      for(auto& arg : accessorys)
      {
         vector<string> names;
         boost::split(names, arg, boost::is_any_of(" \t,"));
         for(const std::string& name : names)
            get_accessory(name).initialize(options);
      }
   }
   try {
      for (auto accessory : autostart_accessorys)
         if (accessory != nullptr && accessory->get_state() == abstract_accessory::registered)
            accessory->initialize(options);
...
void application::shutdown() {
   for(auto ritr = running_accessorys.rbegin();
       ritr != running_accessorys.rend(); ++ritr) {
      (*ritr)->shutdown();
   }
   for(auto ritr = running_accessorys.rbegin();
       ritr != running_accessorys.rend(); ++ritr) {
      accessorys.erase((*ritr)->name());
   }
   running_accessorys.clear();
   initialized_accessorys.clear();
   accessorys.clear();
   io_serv.reset();
}
...
void application::print_default_config(std::ostream& os) {
   ...
   for(auto& plug : accessorys) {
      boost::program_options::options_description accessory_cli_opts;
      boost::program_options::options_description accessory_cfg_opts;
      plug.second->set_program_options(accessory_cli_opts, accessory_cfg_opts);

      for(const boost::shared_ptr<bpo::option_description>& opt : accessory_cfg_opts.options())
...
abstract_accessory* application::find_accessory(const string& name)const
{
   auto itr = accessorys.find(name);
   if(itr == accessorys.end()) {
      return nullptr;
   }
   return itr->second.get();
}

abstract_accessory& application::get_accessory(const string& name)const {
   auto ptr = find_accessory(name);
   if(!ptr)
      BOOST_THROW_EXCEPTION(std::runtime_error("unable to find accessory: " + name));
   return *ptr;
}
