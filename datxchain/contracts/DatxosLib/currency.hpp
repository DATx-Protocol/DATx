...
namespace datxos {
    ...
    *  @deprecated This class is deprecated in favor of datxos.token in Dawn 3.0
    */
   class currency {
      public:
         ...
         struct create {
           ...
           // array<char,32>         issuer_agreement_hash;
            ...
            DATXLIB_SERIALIZE( create, (issuer)(maximum_supply)(issuer_can_freeze)(issuer_can_recall)(issuer_can_whitelist) )
         };
         ...
         struct transfer
         {
            ...
            DATXLIB_SERIALIZE( transfer, (from)(to)(quantity)(memo) )
         };
         ...
         struct issue {
            ...
            DATXLIB_SERIALIZE( issue, (to)(quantity)(memo) )
         };
         ...
         struct fee_schedule {
            ...
            DATXLIB_SERIALIZE( fee_schedule, (fee_per_length) )
         };
         ...
         struct account {
            ...
            DATXLIB_SERIALIZE( account, (balance)(frozen)(whitelist) )
         };
         ...
         struct currency_stats {
            ...
            DATXLIB_SERIALIZE( currency_stats, (supply)(max_supply)(issuer)(can_freeze)(can_recall)(can_whitelist)(is_frozen)(enforce_whitelist) )
         };
         ...
         bool apply( account_name contract, action_name act ) {
          ...
          void create_currency( const create& c ) {
            ...
            datxos_assert( sym.is_valid(), "invalid symbol name" );

             stats statstable( _contract, sym.name() );
             auto existing = statstable.find( sym.name() );
             datxos_assert( existing == statstable.end(), "token with symbol already exists" );
          ...
          void issue_currency( const issue& i ) {
             ...
             statstable.modify( st, 0, [&]( auto& s ) {
                ...
                datxos_assert( s.supply.amount >= 0, "underflow" );
             });
          ...
          void on( const issue& i ) {
             ...
             datxos_assert( i.quantity.is_valid(), "invalid quantity" );
             datxos_assert( i.quantity.amount > 0, "must issue positive quantity" );
          ...
          void on( const transfer& t ) {
             ...
             datxos_assert( t.quantity.is_valid(), "invalid quantity" );
             datxos_assert( t.quantity.amount > 0, "must transfer positive quantity" );
      ...
      private:
          void sub_balance( account_name owner, asset value, const currency_stats& st ) {
             ...
             datxos_assert( from.balance.amount >= value.amount, "overdrawn balance" );

             if( has_auth( owner ) ) {
                datxos_assert( !st.can_freeze || !from.frozen, "account is frozen by issuer" );
                datxos_assert( !st.can_freeze || !st.is_frozen, "all transfers are frozen by issuer" );
                datxos_assert( !st.enforce_whitelist || from.whitelist, "account is not white listed" );
             } else if( has_auth( st.issuer ) ) {
                datxos_assert( st.can_recall, "issuer may not recall token" );
             } else {
                datxos_assert( false, "insufficient authority" );
             }
          ...
          void add_balance( account_name owner, asset value, const currency_stats& st, account_name ram_payer )
          {
             ...
             if( to == to_acnts.end() ) {
                datxos_assert( !st.enforce_whitelist, "can only transfer to white listed accounts" );
                to_acnts.emplace( ram_payer, [&]( auto& a ){
                  a.balance = value;
                });
             } else {
                datxos_assert( !st.enforce_whitelist || to->whitelist, "receiver requires whitelist by issuer" );
