...
namespace datxos {
   ...
   struct action {
      ...
      void send_context_free() const {
         datxos_assert( authorization.size() == 0, "context free actions cannot have authorizations");
         auto serialize = pack(*this);
      ...
      template<typename T>
      T data_as() {
         datxos_assert( name == T::get_name(), "Invalid name" );
         datxos_assert( account == T::get_account(), "Invalid account" );
...
::datxos::inline_dispatcher<decltype(&CONTRACT_CLASS::FUNCTION_NAME), ACTION_NAME>::call

#define INLINE_ACTION_SENDER2( CONTRACT_CLASS, NAME )\
INLINE_ACTION_SENDER3( CONTRACT_CLASS, NAME, ::datxos::string_to_name(#NAME) )
 ...
 * @param NAME - The name of the action
 */
#define ACTION( CODE, NAME ) struct NAME : ::datxos::action_meta<CODE, ::datxos::string_to_name(#NAME) >
