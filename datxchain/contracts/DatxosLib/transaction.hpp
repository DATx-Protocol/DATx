 ...
 */
#pragma once
#include <DatxosLib/transaction.h>
...
namespace datxos {
   ...
   inline action get_action( uint32_t type, uint32_t index ) {
      ...
      datxos_assert( s > 0, "get_action size failed" );
      size_t size = static_cast<size_t>(s);
      char* buffer = (char*)( max_stack_buffer_size < size ? malloc(size) : alloca(size) );
      auto size2 = ::get_action( type, index, buffer, size );
      datxos_assert( size == static_cast<size_t>(size2), "get_action failed" );
      return datxos::unpack<datxos::action>( buffer, size );
   }
