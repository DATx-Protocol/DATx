...
#include <DatxosLib/system.h>
#include <DatxosLib/symbol.hpp>
#include <tuple>
#include <limits>
...
namespace datxos {
   ...
   */
   ...
   struct asset {
      ...
      :amount(a),symbol{s}
      {
         datxos_assert( is_amount_within_range(), "magnitude of asset amount must be less than 2^62" );
         datxos_assert( symbol.is_valid(),        "invalid symbol name" );
      }
      ...
      asset& operator-=( const asset& a ) {
         datxos_assert( a.symbol == symbol, "attempt to subtract asset with different symbol" );
         amount -= a.amount;
         datxos_assert( -max_amount <= amount, "subtraction underflow" );
         datxos_assert( amount <= max_amount,  "subtraction overflow" );
         return *this;
      }
       ...
       */
      asset& operator+=( const asset& a ) {
         datxos_assert( a.symbol == symbol, "attempt to add asset with different symbol" );
         amount += a.amount;
         datxos_assert( -max_amount <= amount, "addition underflow" );
         datxos_assert( amount <= max_amount,  "addition overflow" );
         return *this;
      }
      ...
      asset& operator*=( int64_t a ) {
         ...
         datxos_assert( tmp <= max_amount, "multiplication overflow" );
         datxos_assert( tmp >= -max_amount, "multiplication underflow" );
         amount = (int64_t)tmp;
         return *this;
      }
       ...
       */
      asset& operator/=( int64_t a ) {
         datxos_assert( a != 0, "divide by zero" );
         datxos_assert( !(amount == std::numeric_limits<int64_t>::min() && a == -1), "signed division overflow" );
         amount /= a;
         return *this;
      }
      ...
      friend int64_t operator/( const asset& a, const asset& b ) {
         datxos_assert( a.symbol == b.symbol, "comparison of assets with different symbols is not allowed" );
   ...
   */
   struct extended_asset : public asset {
      ...
      friend extended_asset operator - ( const extended_asset& a, const extended_asset& b ) {
         datxos_assert( a.contract == b.contract, "type mismatch" );
