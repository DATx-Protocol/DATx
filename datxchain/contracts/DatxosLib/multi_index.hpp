...
#include <DatxosLib/action.h>
#include <DatxosLib/types.hpp>
#include <DatxosLib/serialize.hpp>
#include <DatxosLib/datastream.hpp>
#include <DatxosLib/db.h>
#include <DatxosLib/fixed_key.hpp>
...
/**
 *  The indexed_by struct is used to instantiate the indices for the Multi-Index table. In DATXOS, up to 16 secondary indices can be specified.
 *  @brief The indexed_by struct is used to instantiate the indices for the Multi-Index table. In DATXOS, up to 16 secondary indices can be specified.
 *
 *  @tparam IndexName - is the name of the index. The name must be provided as an DATXOS base32 encoded 64-bit integer and must conform to the DATXOS naming requirements of a maximum of 13 characters, the first twelve from the lowercase characters a-z, digits 0-5, and ".", and if there is a 13th character, it is restricted to lowercase characters a-p and ".".
 *  @tparam Extractor - is a function call operator that takes a const reference to the table object type and returns either a secondary key type or a reference to a secondary key type. It is recommended to use the `datxos::const_mem_fun` template, which is a type alias to the `boost::multi_index::const_mem_fun`. See the documentation for the Boost `const_mem_fun` key extractor for more details.
 *
 *  Example:
       *
*
 *  @code
 *  #include <DatxosLib/datxos.hpp>
 *  using namespace datxos;
 *  class mycontract: datxos::contract {
 *    struct record {
 *       uint64_t    primary;
 *       uint128_t   secondary;
 *       uint64_t primary_key() const { return primary; }
 *       uint64_t get_secondary() const { return secondary; }
 *       DATXLIB_SERIALIZE( record, (primary)(secondary) )
 *     };
 *    public:
 ...
 *      }
 *  }
 *  DATXOS_ABI( mycontract, (myaction) )
 *  @endcode
 */
 ...
 *  @brief Defines DATXOS Multi Index Table
 *  @ingroup databasecpp
 *
 *
 *
 *  DATXOS Multi-Index API provides a C++ interface to the DATXOS database. It is patterned after Boost Multi Index Container.
 *  DATXOS Multi-Index table requires exactly a uint64_t primary key. For the table to be able to retrieve the primary key,
 *  the object stored inside the table is required to have a const member function called primary_key() that returns uint64_t.
 *  DATXOS Multi-Index table also supports up to 16 secondary indices. The type of the secondary indices could be any of:
 *  - uint64_t
 *  - uint128_t
 *  - uint256_t
 *  - double
 ...
 *      DATXLIB_SERIALIZE( record, (primary)(secondary_1)(secondary_2)(secondary_3)(secondary_4)(secondary_5) )
 *    };
 *    public:
 *      mycontract( account_name self ):contract(self){}
 *      void myaction() {
...
class multi_index
{
   private:
      ...
      struct index {
         public:
            ...
            struct const_iterator : public std::iterator<std::bidirectional_iterator_tag, const T> {
               public:
                  ...
                  const_iterator& operator++() {
                     ...
                     datxos_assert( _item != nullptr, "cannot increment end iterator" );
                  ...
                  const_iterator& operator--() {
                     ...
                     if( !_item ) {
                        ...
                        datxos_assert( ei != -1, "cannot decrement end iterator when the index is empty" );
                        prev_itr = secondary_index_db_functions<secondary_key_type>::db_idx_previous( ei , &prev_pk );
                        datxos_assert( prev_itr >= 0, "cannot decrement end iterator when the index is empty" );
                     } else {
                        if( _item->__iters[Number] == -1 ) {
                           secondary_key_type temp_secondary_key;
                           auto idxitr = secondary_index_db_functions<secondary_key_type>::db_idx_find_primary(_idx->get_code(), _idx->get_scope(), _idx->name(), _item->primary_key(), temp_secondary_key);
                        ...
                        datxos_assert( prev_itr >= 0, "cannot decrement iterator at beginning of index" );
                     }
            ...
            const_iterator require_find( const secondary_key_type& secondary, const char* error_msg = "unable to find secondary key" )const {
               auto lb = lower_bound( secondary );
               datxos_assert( lb != cend(), error_msg );
               datxos_assert( secondary == secondary_extractor_type()(*lb), error_msg );
               return lb;
            }
            ...
            const T& get( const secondary_key_type& secondary, const char* error_msg = "unable to find secondary key" )const {
               ...
               datxos_assert( result != cend(), error_msg );
               return *result;
            }
            ...
            const_iterator iterator_to( const T& obj ) {
               ...
               datxos_assert( objitem.__idx == _multidx, "object passed to iterator_to is not in multi_index" );
            ...
            void modify( const_iterator itr, uint64_t payer, Lambda&& updater ) {
               datxos_assert( itr != cend(), "cannot pass end iterator to modify" );

               _multidx->modify( *itr, payer, std::forward<Lambda&&>(updater) );
            }

            const_iterator erase( const_iterator itr ) {
               datxos_assert( itr != cend(), "cannot pass end iterator to erase" );
      ...
      const item& load_object_by_primary_iterator( int32_t itr )const {
         ...
         datxos_assert( size >= 0, "error reading iterator" );
   ...
   public:
       ...
       *
       *  Notes
       *  The `datxos::multi_index` template has template parameters `<uint64_t TableName, typename T, typename... Indices>`, where:
       ...
       *  - It is recommended to use the datxos::const_mem_fun template, which is a type alias to the boost::multi_index::const_mem_fun.  See the documentation for the Boost const_mem_fun key extractor for more details.
       *
       *  Example:
       *
       *  @code
       ...
       *       DATXLIB_SERIALIZE( address, (account_name)(first_name)(last_name)(street)(city)(state) )
       *    };
       *    public:
       *      addressbook(account_name self):contract(self) {}
       *      typedef datxos::multi_index< N(address), address > address_index;
       *      void myaction() {
       *        address_index addresses(_self, _self); // code, scope
       *      }
       *  }
       *  DATXOS_ABI( addressbook, (myaction) )
       *  @endcode
       */
       ...
       *        datxos_assert(addresses.get_code() == N(dan), "Codes don't match.");
       *      }
       *  }
       *  DATXOS_ABI( addressbook, (myaction) )
       *  @endcode
       ...
       *      void myaction() {
       *        address_index addresses(N(dan), N(dan)); // code, scope
       *        datxos_assert(addresses.get_code() == N(dan), "Scopes don't match");
       *      }
       *  }
       *  DATXOS_ABI( addressbook, (myaction) )
       *  @endcode
      ...
      struct const_iterator : public std::iterator<std::bidirectional_iterator_tag, const T> {
         ...
         const_iterator& operator--() {
            ...
            if( !_item ) {
               ...
               datxos_assert( ei != -1, "cannot decrement end iterator when the table is empty" );
               prev_itr = db_previous_i64( ei , &prev_pk );
               datxos_assert( prev_itr >= 0, "cannot decrement end iterator when the table is empty" );
            } else {
               prev_itr = db_previous_i64( _item->__primary_itr, &prev_pk );
               datxos_assert( prev_itr >= 0, "cannot decrement iterator at beginning of table" );
            }
       ...
       *          address.street = "1 DATX Way";
       *          address.city = "Blacksburg";
       *          address.state = "VA";
       *        });
       *        auto itr = addresses.find(N(dan));
       *        datxos_assert(itr == addresses.cbegin(), "Only address is not at front.");
       *      }
       *  }
       *  DATXOS_ABI( addressbook, (myaction) )
       *  @endcode
       ...
       *        });
       *        auto itr = addresses.find(N(dan));
       *        datxos_assert(itr == addresses.begin(), "Only address is not at front.");
       *      }
       *  }
       *  DATXOS_ABI( addressbook, (myaction) )
       *  @endcode
       ...
       *        });
       *        auto itr = addresses.find(N(dan));
       *        datxos_assert(itr != addresses.cend(), "Address for account doesn't exist");
       *      }
       *  }
       *  DATXOS_ABI( addressbook, (myaction) )
       *  @endcode
       ...
       *        });
       *        auto itr = addresses.find(N(dan));
       *        datxos_assert(itr != addresses.end(), "Address for account doesn't exist");
       *      }
       *  }
       *  DATXOS_ABI( addressbook, (myaction) )
       *  @endcode
       ...
       *        datxos_assert(itr->account_name == N(dan), "Incorrect Last Record ");
       *        itr++;
       *        datxos_assert(itr->account_name == N(brendan), "Incorrect Second Last Record");
       *      }
       *  }
       *  DATXOS_ABI( addressbook, (myaction) )
       *  @endcode
       ...
       *        itr--;
       *        datxos_assert(itr->account_name == N(brendan), "Incorrect First Record ");
       *        itr--;
       *        datxos_assert(itr->account_name == N(dan), "Incorrect Second Record");
       *      }
       *  }
       *  DATXOS_ABI( addressbook, (myaction) )
       *  @endcode
       ...
       *       DATXLIB_SERIALIZE( address, (account_name)(first_name)(last_name)(street)(city)(state)(zip) )
       *    };
       *    public:
       *      addressbook(account_name self):contract(self) {}
       *      typedef datxos::multi_index< N(address), address, indexed_by< N(zip), const_mem_fun<address, uint64_t, &address::by_zip> > address_index;
       *      void myaction() {
       *        address_index addresses(_self, _self);  // code, scope
       *        // add to table, first argument is account to bill for storage
       *        addresses.emplace(payer, [&](auto& address) {
       ...
       *        datxos_assert(itr->account_name == N(brendan), "Incorrect First Lower Bound Record ");
       *        itr++;
       *        datxos_assert(itr->account_name == N(dan), "Incorrect Second Lower Bound Record");
       *        itr++;
       *        datxos_assert(itr == zip_index.end(), "Incorrect End of Iterator");
       *      }
       *  }
       *  DATXOS_ABI( addressbook, (myaction) )
       *  @endcode
       ...
       *        datxos_assert(itr->account_name == N(dan), "Incorrect First Upper Bound Record ");
       *        itr++;
       *        datxos_assert(itr == zip_index.end(), "Incorrect End of Iterator");
       *      }
       *  }
       ...
       *       DATXLIB_SERIALIZE( address, (key)(first_name)(last_name)(street)(city)(state) )
       *    };
       *    public:
       *      addressbook(account_name self):contract(self) {}
       *      typedef datxos::multi_index< N(address), address > address_index;
      ...
      uint64_t available_primary_key()const {
         ...
         datxos_assert( _next_primary_key < no_available_primary_key, "next primary key in table is at autoincrement limit");
       ...
       *        datxos_assert(itr->account_name == N(dan), "Incorrect Record ");
       *      }
       *  }
       *  DATXOS_ABI( addressbook, (myaction) )
       *  @endcode
       ...
       *        datxos_assert(iterator_to(user) == itr, "Invalid iterator");
       *      }
       *  }
       *  DATXOS_ABI( addressbook, (myaction) )
       *  @endcode
      ...
      const_iterator iterator_to( const T& obj )const {
         const auto& objitem = static_cast<const item&>(obj);
         datxos_assert( objitem.__idx == this, "object passed to iterator_to is not in multi_index" );
         return {this, &objitem};
      }
      /**
      ...
      const_iterator emplace( uint64_t payer, Lambda&& constructor ) {
         ...
         datxos_assert( _code == current_receiver(), "cannot create objects in table of another contract" ); // Quick fix for mutating db using multi_index that shouldn't allow mutation. Real fix can come in RC2.
       ...
       *        });
       *        auto itr = addresses.find(N(dan));
       *        datxos_assert(itr != addresses.end(), "Address for account not found");
      ...
      template<typename Lambda>
      void modify( const_iterator itr, uint64_t payer, Lambda&& updater ) {
         datxos_assert( itr != end(), "cannot pass end iterator to modify" );
       ...
       *          address.state = "CA";
       *        });
       *        datxos_assert(itr->city == "San Luis Obispo", "Address not modified");
       *      }
       *  }
       *  DATXOS_ABI( addressbook, (myaction) )
       *  @endcode
      ...
      void modify( const T& obj, uint64_t payer, Lambda&& updater ) {
         ...
         const auto& objitem = static_cast<const item&>(obj);
         datxos_assert( objitem.__idx == this, "object passed to modify is not in multi_index" );
         auto& mutableitem = const_cast<item&>(objitem);
         datxos_assert( _code == current_receiver(), "cannot modify objects in table of another contract" ); // Quick fix for mutating db using multi_index that shouldn't allow mutation. Real fix can come in RC2.
         ...
         datxos_assert( pk == obj.primary_key(), "updater cannot change primary key when modifying an object" );
       ...
       *        });
       *        auto user = addresses.get(N(dan));
       *        datxos_assert(user.first_name == "Daniel", "Couldn't get him.");
       *      }
       *  }
       *  DATXOS_ABI( addressbook, (myaction) )
       *  @endcode
       ...
       *        });
       *        auto itr = addresses.find(N(dan));
       *        datxos_assert(itr != addresses.end(), "Couldn't get him.");
       *      }
       *  }
       *  DATXOS_ABI( addressbook, (myaction) )
       *  @endcode
       ...
       *        datxos_assert(itr != addresses.end(), "Address for account not found");
       *        addresses.erase( itr );
       *        datxos_assert(itr != addresses.end(), "Address not erased properly");
       *      }
       *  }
       *  DATXOS_ABI( addressbook, (myaction) )
       *  @endcode
       */
      const_iterator erase( const_iterator itr ) {
         datxos_assert( itr != end(), "cannot pass end iterator to erase" );
       ...
       *        });
       *        auto itr = addresses.find(N(dan));
       *        datxos_assert(itr != addresses.end(), "Record is not found");
       *        addresses.erase(*itr);
       *        itr = addresses.find(N(dan));
       *        datxos_assert(itr == addresses.end(), "Record is not deleted");
       *      }
       *  }
       *  DATXOS_ABI( addressbook, (myaction) )
       *  @endcode
      ...
      void erase( const T& obj ) {
         ...
         const auto& objitem = static_cast<const item&>(obj);
         datxos_assert( objitem.__idx == this, "object passed to erase is not in multi_index" );
         datxos_assert( _code == current_receiver(), "cannot erase objects in table of another contract" ); // Quick fix for mutating db using multi_index that shouldn't allow mutation. Real fix can come in RC2.
         ...
         datxos_assert( itr2 != _items_vector.rend(), "attempt to remove object that was not in multi_index" );
