...
namespace datxos {
  class microseconds {
    public:
        ...
        int64_t _count;
        DATXLIB_SERIALIZE( microseconds, (_count) )
    private:
  ...
  class time_point {
    public:
        ...
        DATXLIB_SERIALIZE( time_point, (elapsed) )
  };
  ...
  class time_point_sec
  {
    public:
        ...
        operator time_point()const { return time_point( datxos::seconds( utc_seconds) ); }
        uint32_t sec_since_epoch()const { return utc_seconds; }

        time_point_sec operator = ( const datxos::time_point& t )
        {
          utc_seconds = uint32_t(t.time_since_epoch().count() / 1000000ll);
          return *this;
        }
        ...
        DATXLIB_SERIALIZE( time_point_sec, (utc_seconds) )
  };
   ...
   class block_timestamp {
      public:
         ...
         block_timestamp next() const {
            datxos_assert( std::numeric_limits<uint32_t>::max() - slot >= 1, "block timestamp overflow" );
         ...
         DATXLIB_SERIALIZE( block_timestamp, (slot) )
      private:
