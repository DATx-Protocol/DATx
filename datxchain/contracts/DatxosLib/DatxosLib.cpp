...
namespace datxos {
   ...
   void set_blockchain_parameters(const datxos::blockchain_parameters& params) {
      char buf[sizeof(datxos::blockchain_parameters)];
      datxos::datastream<char *> ds( buf, sizeof(buf) );
      ds << params;
      set_blockchain_parameters_packed( buf, ds.tellp() );
   }

   void get_blockchain_parameters(datxos::blockchain_parameters& params) {
      char buf[sizeof(datxos::blockchain_parameters)];
      size_t size = get_blockchain_parameters_packed( buf, sizeof(buf) );
      datxos_assert( size <= sizeof(buf), "buffer is too small" );
      datxos::datastream<const char*> ds( buf, size_t(size) );
      ds >> params;
   }
   ...
   private:
      ...
      public:
         ...
         {
            datxos_assert(_offset == _heap_size, "malloc_from_freed was designed to only be called after _heap was completely allocated");
...
{
   return datxos::memory_heap.malloc(size);
}

void* calloc(size_t count, size_t size)
{
   void* ptr = datxos::memory_heap.malloc(count*size);
...
{
   return datxos::memory_heap.realloc(ptr, size);
}

void free(void* ptr)
{
   return datxos::memory_heap.free(ptr);
}
