  ...
  # popd &> /dev/null
   ...
   if [ "$ARCH" == "Linux" ]; then
      ...
      if [ ! -e /etc/os-release ]; then
         printf "\\n\\tDATXOS currently supports Amazon, Centos, Fedora, Mint & Ubuntu Linux only.\\n"
      ...
      case "$OS_NAME" in
         "Amazon Linux AMI")
            FILE="${SOURCE_DIR}/scripts/datxos_build_amazon.sh"
            CXX_COMPILER=g++
            C_COMPILER=gcc
         ...
         ;;
         "CentOS Linux")
            FILE="${SOURCE_DIR}/scripts/datxos_build_centos.sh"
            CXX_COMPILER=g++
            C_COMPILER=gcc
            MONGOD_CONF=${HOME}/opt/mongodb/mongod.conf
            export LLVM_DIR=${HOME}/opt/wasm/lib/cmake/llvm
         ...
         ;;
         "elementary OS")
            FILE="${SOURCE_DIR}/scripts/datxos_build_ubuntu.sh"
         ...
         ;;
         "Fedora")
            FILE="${SOURCE_DIR}/scripts/datxos_build_fedora.sh"
            CXX_COMPILER=g++
            C_COMPILER=gcc
   ...
   if [ "$ARCH" == "Darwin" ]; then
      FILE="${SOURCE_DIR}/scripts/datxos_build_darwin.sh"
      CXX_COMPILER=clang++
      C_COMPILER=clang
   ...
   printf "\\n\\n>>>>>>>> ALL dependencies sucessfully found or installed . Installing DATXOS\\n\\n"
   ...
   if ! "${CMAKE}" -DCMAKE_BUILD_TYPE="${CMAKE_BUILD_TYPE}" -DCMAKE_CXX_COMPILER="${CXX_COMPILER}" \
      ...
      -DOPENSSL_ROOT_DIR="${OPENSSL_ROOT_DIR}" -DBUILD_MONGO_DB_accessory=true \
      -DENABLE_COVERAGE_TESTING="${ENABLE_COVERAGE_TESTING}" -DBUILD_DOXYGEN="${DOXYGEN}" \
      -DCMAKE_INSTALL_PREFIX="/usr/local/datxos" "${SOURCE_DIR}"
   then
      printf "\\n\\t>>>>>>>>>>>>>>>>>>>> CMAKE building DATXOS has exited with the above error.\\n\\n"
      exit -1
   fi

   if [ "${START_MAKE}" == "false" ]; then
      printf "\\n\\t>>>>>>>>>>>>>>>>>>>> DATXOS has been successfully configured but not yet built.\\n\\n"
      exit 0
   fi

   if ! make -j"${CPU_CORE}"
   then
      printf "\\n\\t>>>>>>>>>>>>>>>>>>>> MAKE building DATXOS has exited with the above error.\\n\\n"
      exit -1
   fi
   ...
   printf "\\n\\tDatx has been successfully built. %02d:%02d:%02d\\n\\n" $(($TIME_END/3600)) $(($TIME_END%3600/60)) $(($TIME_END%60))
