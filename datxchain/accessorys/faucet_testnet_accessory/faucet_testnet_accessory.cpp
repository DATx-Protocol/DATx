 ...
 */
#include <datxos/faucet_testp2p_net_accessory/faucet_testp2p_net_accessory.hpp>
...
FC_REFLECT(datxos::detail::faucet_testnet_empty, );
FC_REFLECT(datxos::detail::faucet_testnet_keys, (owner)(active));
FC_REFLECT(datxos::detail::faucet_testnet_create_account_params, (account)(keys));
FC_REFLECT(datxos::detail::faucet_testnet_create_account_alternates_response, (alternates)(message));
FC_REFLECT(datxos::detail::faucet_testnet_create_account_rate_limited_response, (message));

namespace datxos {

static appbase::abstract_accessory& _faucet_testp2p_net_accessory = app().register_accessory<faucet_testp2p_net_accessory>();
...
{std::string("/v1/" #api_name "/" #call_name), \
   [this](string, string body, url_response_callback response_cb) mutable { \
          try { \
             ...
             http_accessory::handle_exception(#api_name, #call_name, body, response_cb); \
          } \
       }}

struct faucet_testp2p_net_accessory_impl {
   struct create_faucet_account_alternate_results {
      std::vector<std::string> alternates;
   };

   faucet_testp2p_net_accessory_impl(appbase::application& app)
   : _app(app)
   ...
   results_pair find_alternates(const std::string& new_account_name) {
      ...
      const datxos::detail::faucet_testnet_create_account_alternates_response response{
   ...
   results_pair create_account(const std::string& new_account_name, const fc::crypto::public_key& owner_pub_key, const fc::crypto::public_key& active_pub_key) {
      ...
      DATX_ASSERT(creating_account != nullptr, transaction_exception,
      ...
      {
         datxos::detail::faucet_testnet_create_account_rate_limited_response response{
      ...
      auto& accessory = _app.get_accessory<core_accessory>();
      accessory.get_chain_id(chainid);
      controller& cc = accessory.chain();
      ...
      _timer.async_wait(boost::bind(&faucet_testp2p_net_accessory_impl::timer_fired, this));

      return std::make_pair(account_created, fc::variant(datxos::detail::faucet_testnet_empty()));
   }

   results_pair create_faucet_account(const std::string& body) {
      const datxos::detail::faucet_testnet_create_account_params params = fc::json::from_string(body).as<datxos::detail::faucet_testnet_create_account_params>();
   ...
   const chainbase::database& database() {
      ...
      if (db == nullptr)
         db = &_app.get_accessory<core_accessory>().chain().db();
...
const uint32_t faucet_testp2p_net_accessory_impl::_default_create_interval_msec = 1000;
const uint32_t faucet_testp2p_net_accessory_impl::_default_create_alternates_to_return = 3;
const std::string faucet_testp2p_net_accessory_impl::_default_create_account_name = "faucet";
// defaults to the public/private key of init accounts in private testnet genesis.json
const key_pair faucet_testp2p_net_accessory_impl::_default_key_pair = {"DATX6MRyAjQq8ud7hVNYcfnVPJqcVpscN5So8BhtHuGYqET5GDW5CV", "5KQwrPbwdL6PhXujxW37FSSQZ1JiwsST4cqQzDeyXtP79zkvFD3"};


faucet_testp2p_net_accessory::faucet_testp2p_net_accessory()
: my(new faucet_testp2p_net_accessory_impl(app()))
{
}

faucet_testp2p_net_accessory::~faucet_testp2p_net_accessory() {}

void faucet_testp2p_net_accessory::set_program_options(options_description&, options_description& cfg) {
   cfg.add_options()
         ("faucet-create-interval-ms", bpo::value<uint32_t>()->default_value(faucet_testp2p_net_accessory_impl::_default_create_interval_msec),
          "Time to wait, in milliseconds, between creating next faucet created account.")
         ("faucet-name", bpo::value<std::string>()->default_value(faucet_testp2p_net_accessory_impl::_default_create_account_name),
          "Name to use as creator for faucet created accounts.")
         ("faucet-private-key", boost::program_options::value<std::string>()->default_value(fc::json::to_string(faucet_testp2p_net_accessory_impl::_default_key_pair)),
          "[public key, WIF private key] for signing for faucet creator account")
         ;
}

void faucet_testp2p_net_accessory::accessory_initialize(const variables_map& options) {
   try {
...
void faucet_testp2p_net_accessory::accessory_startup() {
   app().get_accessory<http_accessory>().add_api({
      CALL(faucet, my, create_account, faucet_testp2p_net_accessory_impl::create_faucet_account )
   });
}

void faucet_testp2p_net_accessory::accessory_shutdown() {
   try {
      my->_timer.cancel();
