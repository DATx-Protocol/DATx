...
#include <datxos/login_accessory/login_accessory.hpp>

#include <fc/io/json.hpp>

namespace datxos {

static appbase::abstract_accessory& _login_accessory = app().register_accessory<login_accessory>();
...
login_accessory::login_accessory() : my{std::make_unique<login_accessory_impl>()} {}
login_accessory::~login_accessory() {}

void login_accessory::set_program_options(options_description&, options_description& cfg) {
   cfg.add_options() //
...
void login_accessory::accessory_initialize(const variables_map& options) {
   ...
   {                                                                                                                   \
      std::string("/v1/login/" #call_name), [this](string, string body, url_response_callback cb) mutable {            \
         try {                                                                                                         \
            if (body.empty())                                                                                          \
               body = "{}";                                                                                            \
            auto result = call_name(fc::json::from_string(body).as<login_accessory::call_name##_params>());               \
            cb(http_response_code, fc::json::to_string(result));                                                       \
         } catch (...) {                                                                                               \
            http_accessory::handle_exception("login", #call_name, body, cb);                                              \
         }                                                                                                             \
      }                                                                                                                \
   }

void login_accessory::accessory_startup() {
   ilog("starting login_accessory");
...
void login_accessory::accessory_shutdown() {}

login_accessory::start_login_request_results
login_accessory::start_login_request(const login_accessory::start_login_request_params& params) {
   my->expire_requests();
   DATX_ASSERT(params.expiration_time > fc::time_point::now(), fc::timeout_exception,
              "Requested expiration time ${expiration_time} is in the past",
              ("expiration_time", params.expiration_time));
   DATX_ASSERT(my->requests.size() < my->max_login_requests, fc::timeout_exception, "Too many pending login requests");
...
login_accessory::finalize_login_request_results
login_accessory::finalize_login_request(const login_accessory::finalize_login_request_params& params) {
   ...
   try {
      auto noop_checktime = [] {};
      auto& chain = app().get_accessory<core_accessory>().chain();
...
login_accessory::do_not_use_gen_r1_key_results
login_accessory::do_not_use_gen_r1_key(const login_accessory::do_not_use_gen_r1_key_params& params) {
   auto priv = chain::private_key_type::generate_r1();
   return {priv.get_public_key(), priv};
}

login_accessory::do_not_use_sign_results
login_accessory::do_not_use_sign(const login_accessory::do_not_use_sign_params& params) {
   return {params.priv_key.sign(chain::sha256::hash(params.data))};
}

login_accessory::do_not_use_get_secret_results
login_accessory::do_not_use_get_secret(const login_accessory::do_not_use_get_secret_params& params) {
