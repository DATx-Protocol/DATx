...
#include <datxos/chain/block_state.hpp>
#include <datxos/chain/transaction_metadata.hpp>
#include <datxos/chain/trace.hpp>

namespace datxos { namespace chain { namespace accessory_interface {
   ...
   struct core_accessory_interface;
   ...
   namespace methods {
      using get_block_by_number    = method_decl<core_accessory_interface, signed_block_ptr(uint32_t block_num)>;
      using get_block_by_id        = method_decl<core_accessory_interface, signed_block_ptr(const block_id_type& block_id)>;
      using get_head_block_id      = method_decl<core_accessory_interface, block_id_type ()>;
      using get_lib_block_id       = method_decl<core_accessory_interface, block_id_type ()>;

      using get_last_irreversible_block_number = method_decl<core_accessory_interface, uint32_t ()>;
   }
   ...
   namespace incoming {
      ...
      namespace methods {
         ...
         using block_sync            = method_decl<core_accessory_interface, void(const signed_block_ptr&), first_provider_policy>;
         using transaction_async     = method_decl<core_accessory_interface, void(const packed_transaction_ptr&, bool, next_function<transaction_trace_ptr>), first_provider_policy>;
      }
   }
