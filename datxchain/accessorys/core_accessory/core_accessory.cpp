...
#include <datxos/chain/fork_database.hpp>
#include <datxos/chain/block_log.hpp>
#include <datxos/chain/exceptions.hpp>
#include <datxos/chain/authorization_manager.hpp>
#include <datxos/chain/producer_object.hpp>
#include <datxos/chain/config.hpp>
#include <datxos/chain/wasm_interface.hpp>
#include <datxos/chain/resource_limits.hpp>
#include <datxos/chain/reversible_block_object.hpp>
#include <datxos/chain/controller.hpp>
#include <datxos/chain/generated_transaction_object.hpp>

#include <datxos/chain/datxos_contract.hpp>

#include <datxos/utilities/key_conversion.hpp>
#include <datxos/utilities/common.hpp>
...
std::ostream& operator<<(std::ostream& osm, datxos::chain::db_read_mode m) {
   if ( m == datxos::chain::db_read_mode::SPECULATIVE ) {
      osm << "speculative";
   } else if ( m == datxos::chain::db_read_mode::HEAD ) {
      osm << "head";
   } else if ( m == datxos::chain::db_read_mode::READ_ONLY ) {
      osm << "read-only";
   } else if ( m == datxos::chain::db_read_mode::IRREVERSIBLE ) {
      osm << "irreversible";
   }
...
void validate(boost::any& v,
              ...
              datxos::chain::db_read_mode* /* target_type */,
              int)
{
  ...
  if ( s == "speculative" ) {
     v = boost::any(datxos::chain::db_read_mode::SPECULATIVE);
  } else if ( s == "head" ) {
     v = boost::any(datxos::chain::db_read_mode::HEAD);
  } else if ( s == "read-only" ) {
     v = boost::any(datxos::chain::db_read_mode::READ_ONLY);
  } else if ( s == "irreversible" ) {
     v = boost::any(datxos::chain::db_read_mode::IRREVERSIBLE);
  } else {
     throw validation_error(validation_error::invalid_option_value);
  }
}

std::ostream& operator<<(std::ostream& osm, datxos::chain::validation_mode m) {
   if ( m == datxos::chain::validation_mode::FULL ) {
      osm << "full";
   } else if ( m == datxos::chain::validation_mode::LIGHT ) {
      osm << "light";
   }
...
void validate(boost::any& v,
              const std::vector<std::string>& values,
              datxos::chain::validation_mode* /* target_type */,
              int)
{
  using namespace boost::program_options;
  ...
  if ( s == "full" ) {
     v = boost::any(datxos::chain::validation_mode::FULL);
  } else if ( s == "light" ) {
     v = boost::any(datxos::chain::validation_mode::LIGHT);
  } else {
     throw validation_error(validation_error::invalid_option_value);
  }
}
...
using namespace datxos::chain::config;
using namespace datxos::chain::accessory_interface;
...
core_accessory::core_accessory()
:my(new core_accessory_impl()) {
}

core_accessory::~core_accessory(){}

void core_accessory::set_program_options(options_description& cli, options_description& cfg)
{
   cfg.add_options()
         ("blocks-dir", bpo::value<bfs::path>()->default_value("blocks"),
          "the location of the blocks directory (absolute path or relative to application data dir)")
         ("checkpoint", bpo::value<vector<string>>()->composing(), "Pairs of [BLOCK_NUM,BLOCK_ID] that should be enforced as checkpoints.")
         ("wasm-runtime", bpo::value<datxos::chain::wasm_interface::vm_type>()->value_name("wavm/binaryen"), "Override default WASM runtime")
         ...
         ("read-mode", boost::program_options::value<datxos::chain::db_read_mode>()->default_value(datxos::chain::db_read_mode::SPECULATIVE),
         ...
         ("validation-mode", boost::program_options::value<datxos::chain::validation_mode>()->default_value(datxos::chain::validation_mode::FULL),
...
void core_accessory::accessory_initialize(const variables_map& options) {
   ilog("initializing chain accessory");

   try {
      try {
         genesis_state gs; // Check if DATXOS_ROOT_KEY is bad
      } catch ( const fc::exception& ) {
         elog( "DATXOS_ROOT_KEY ('${root_key}') is invalid. Recompile with a valid public key.",
               ("root_key", genesis_state::datxos_root_key));
         throw;
      }
      ...
      if( options.count( "action-blacklist" )) {
         ...
         for( const auto& a : acts ) {
            auto pos = a.find( "::" );
            DATX_ASSERT( pos != std::string::npos, accessory_config_exception, "Invalid entry in action-blacklist: '${a}'", ("a", a));
      ...
      if( options.count("checkpoint") ) {
         ...
         for( const auto& cp : cps ) {
            ...
            if( itr != my->loaded_checkpoints.end() ) {
               DATX_ASSERT( itr->second == item.second,
                           accessory_config_exception,
      ...
      if( options.count( "extract-genesis-json" ) || options.at( "print-genesis-json" ).as<bool>()) {
         ...
         DATX_THROW( extract_genesis_state_exception, "extracted genesis state from blocks.log" );
      }
      ...
      if( options.count("export-reversible-blocks") ) {
         ...
         DATX_THROW( node_management_success, "exported reversible blocks" );
      }
      ...
      if( options.at( "delete-all-blocks" ).as<bool>()) {
         ...
         DATX_THROW( fixed_reversible_db_exception, "fixed corrupted reversible blocks database" );
         ...
         DATX_THROW( node_management_success, "imported reversible blocks" );
      }
      ...
      if( options.count( "genesis-json" )) {
         DATX_ASSERT( !fc::exists( my->blocks_dir / "blocks.log" ),
                     accessory_config_exception,
         ...
         DATX_ASSERT( fc::is_regular_file( genesis_file ),
                     accessory_config_exception,
      ...
      if ( options.count("read-mode") ) {
         ...
         DATX_ASSERT( my->chain_config->read_mode != db_read_mode::IRREVERSIBLE, accessory_config_exception, "irreversible mode not currently supported." );
      }
      ...
      my->pre_accepted_block_connection = my->chain->pre_accepted_block.connect([this](const signed_block_ptr& blk) {
         ...
         if( itr != my->loaded_checkpoints.end() ) {
            auto id = blk->id();
            DATX_ASSERT( itr->second == id, checkpoint_exception,
...
void core_accessory::accessory_startup()
{ try {
   try {
      my->chain->startup();
...
void core_accessory::accessory_shutdown() {
...
void chain_apis::read_write::validate() const {
   DATX_ASSERT( db.get_read_mode() != chain::db_read_mode::READ_ONLY, missing_core_api_accessory_exception, "Not allowed, node in read-only mode" );
}

chain_apis::read_write core_accessory::get_read_write_api() {
   return chain_apis::read_write(chain(), get_abi_serializer_max_time());
}

void core_accessory::accept_block(const signed_block_ptr& block ) {
   my->incoming_block_sync_method(block);
}

void core_accessory::accept_transaction(const chain::packed_transaction& trx, next_function<chain::transaction_trace_ptr> next) {
   my->incoming_transaction_async_method(std::make_shared<packed_transaction>(trx), false, std::forward<decltype(next)>(next));
}

bool core_accessory::block_is_on_preferred_chain(const block_id_type& block_id) {
   auto b = chain().fetch_block_by_number( block_header::num_from_id(block_id) );
   return b && b->id() == block_id;
}

bool core_accessory::recover_reversible_blocks( const fc::path& db_dir, uint32_t cache_size,
   ...
   if( new_db_dir ) {
      ...
      DATX_ASSERT( reversible_dir_name != ".", invalid_reversible_blocks_dir, "Invalid path to reversible directory" );
      backup_dir = reversible_dir.parent_path() / reversible_dir_name.append("-").append( now );

      DATX_ASSERT( !fc::exists(backup_dir),
   ...
   try {
      for( ; itr != ubi.end(); ++itr ) {
         DATX_ASSERT( itr->blocknum == end + 1, gap_in_reversible_blocks_db,
...
bool core_accessory::import_reversible_blocks( const fc::path& reversible_dir,
                                             uint32_t cache_size,
   ...
   try {
      while( reversible_blocks.tellg() < end_pos ) {
         ...
         if( start == 0 ) {
            ...
            DATX_ASSERT( num == end + 1, gap_in_reversible_blocks_db,
...
bool core_accessory::export_reversible_blocks( const fc::path& reversible_dir,
                                             const fc::path& reversible_blocks_file ) {
...
controller::config& core_accessory::chain_config() {
   // will trigger optional assert if called before/after accessory_initialize()
   return *my->chain_config;
}

controller& core_accessory::chain() { return *my->chain; }
const controller& core_accessory::chain() const { return *my->chain; }

chain::chain_id_type core_accessory::get_chain_id()const {
   DATX_ASSERT( my->chain_id.valid(), chain_id_type_exception, "chain ID has not been initialized yet" );
   return *my->chain_id;
}

fc::microseconds core_accessory::get_abi_serializer_max_time() const {
   return my->abi_serializer_max_time_ms;
}

void core_accessory::log_guard_exception(const chain::guard_exception&e ) const {
...
void core_accessory::handle_guard_exception(const chain::guard_exception& e) const {
   log_guard_exception(e);
...
read_only::get_info_results read_only::get_info(const read_only::get_info_params&) const {
   ...
   return {
      datxos::utilities::common::itoh(static_cast<uint32_t>(app().version())),
      db.get_chain_id(),
...
uint64_t read_only::get_table_index_name(const read_only::get_table_rows_params& p, bool& primary) {
   ...
   DATX_ASSERT( index == table, chain::contract_table_query_exception, "Unsupported table name: ${n}", ("n", p.table) );
   ...
   if (p.index_position.empty() || p.index_position == "first" || p.index_position == "primary" || p.index_position == "one") {
      ...
      try {
         ...
         DATX_ASSERT( false, chain::contract_table_query_exception, "Invalid index_position: ${p}", ("p", p.index_position));
      }
      if (pos < 2) {
         primary = true;
         pos = 0;
...
uint64_t convert_to_type(const string& str, const string& desc) {
   ...
   try {
      ...
      try {
         ...
         try {
            auto symb = datxos::chain::symbol::from_string(str);
            value = symb.value();
         } catch( ... ) {
            try {
               value = ( datxos::chain::string_to_symbol( 0, str.c_str() ) >> 8 );
            } catch( ... ) {
               DATX_ASSERT( false, chain_type_exception, "Could not convert ${desc} string '${str}' to any of the following: "
...
abi_def get_abi( const controller& db, const name& account ) {
   ...
   DATX_ASSERT(code_accnt != nullptr, chain::account_query_exception, "Fail to retrieve account for ${account}", ("account", account) );
   abi_def abi;
...
string get_table_type( const abi_def& abi, const name& table_name ) {
   ...
   DATX_ASSERT( false, chain::contract_table_query_exception, "Table ${table} is not specified in the ABI", ("table",table_name) );
}

read_only::get_table_rows_result read_only::get_table_rows( const read_only::get_table_rows_params& p )const {
   const abi_def abi = datxos::chain_apis::get_abi( db, p.code );

   bool primary = false;
   auto table_with_index = get_table_index_name( p, primary );
   if( primary ) {
      DATX_ASSERT( p.table == table_with_index, chain::contract_table_query_exception, "Invalid table name ${t}", ( "t", p.table ));
      auto table_type = get_table_type( abi, p.table );
      if( table_type == KEYi64 || p.key_type == "i64" || p.key_type == "name" ) {
         return get_table_rows_ex<key_value_index>(p,abi);
      }
      DATX_ASSERT( false, chain::contract_table_query_exception,  "Invalid table type ${type}", ("type",table_type)("abi",abi));
   } else {
      DATX_ASSERT( !p.key_type.empty(), chain::contract_table_query_exception, "key type required for non-primary index" );
      ...
      DATX_ASSERT(false, chain::contract_table_query_exception,  "Unsupported secondary index type: ${t}", ("t", p.key_type));
   }
}
...
vector<asset> read_only::get_currency_balance( const read_only::get_currency_balance_params& p )const {
   ...
   walk_key_value_table(p.code, p.account, N(accounts), [&](const key_value_object& obj){
      DATX_ASSERT( obj.value.size() >= sizeof(asset), chain::asset_type_exception, "Invalid data on table");

      asset cursor;
      fc::datastream<const char *> ds(obj.value.data(), obj.value.size());
      fc::raw::unpack(ds, cursor);

      DATX_ASSERT( cursor.get_symbol().valid(), chain::asset_type_exception, "Invalid asset");
...
fc::variant read_only::get_currency_stats( const read_only::get_currency_stats_params& p )const {
   ...
   uint64_t scope = ( datxos::chain::string_to_symbol( 0, boost::algorithm::to_upper_copy(p.symbol).c_str() ) >> 8 );

   walk_key_value_table(p.code, scope, N(stat), [&](const key_value_object& obj){
      DATX_ASSERT( obj.value.size() >= sizeof(read_only::get_currency_stats_result), chain::asset_type_exception, "Invalid data on table");
...
static fc::variant get_global_row( const database& db, const abi_def& abi, const abi_serializer& abis, const fc::microseconds& abi_serializer_max_time_ms ) {
   ...
   DATX_ASSERT(table_type == read_only::KEYi64, chain::contract_table_query_exception, "Invalid table type ${type} for table global", ("type",table_type));
   ...
   DATX_ASSERT(table_id, chain::contract_table_query_exception, "Missing table global");
   ...
   DATX_ASSERT(it != kv_index.end(), chain::contract_table_query_exception, "Missing row in table global");
...
read_only::get_producers_result read_only::get_producers( const read_only::get_producers_params& p ) const {
   const abi_def abi = datxos::chain_apis::get_abi(db, config::system_account_name);
   const auto table_type = get_table_type(abi, N(producers));
   const abi_serializer abis{ abi, abi_serializer_max_time };
   DATX_ASSERT(table_type == KEYi64, chain::contract_table_query_exception, "Invalid table type ${type} for table producers", ("type",table_type));
   ...
   DATX_ASSERT(table_id && secondary_table_id, chain::contract_table_query_exception, "Missing producers table");
      ...
      if (!p.lower_bound.empty()) {
         try {
            ...
            try {
               ...
               if (itr == by_txid.end()) {
                  DATX_THROW(transaction_exception, "Unknown Transaction ID: ${txid}", ("txid", txid));
               }
...
fc::variant read_only::get_block(const read_only::get_block_params& params) const {
   ...
   DATX_ASSERT(!params.block_num_or_id.empty() && params.block_num_or_id.size() <= 64, chain::block_id_type_exception, "Invalid Block number or ID, must be greater than 0 and less than 64 characters" );
   try {
   ...
   } DATX_RETHROW_EXCEPTIONS(chain::block_id_type_exception, "Invalid block ID: ${block_num_or_id}", ("block_num_or_id", params.block_num_or_id))

   DATX_ASSERT( block, unknown_block_exception, "Could not find block: ${block}", ("block", params.block_num_or_id));
...
fc::variant read_only::get_block_header_state(const get_block_header_state_params& params) const {
   ...
   DATX_ASSERT( b, unknown_block_exception, "Could not find reversible block: ${block}", ("block", params.block_num_or_id));
...
void read_write::push_transaction(const read_write::push_transaction_params& params, next_function<read_write::push_transaction_results> next) {
   ...
   try {
      ...
      } DATX_RETHROW_EXCEPTIONS(chain::packed_transaction_type_exception, "Invalid packed transaction")
...
void read_write::push_transactions(const read_write::push_transactions_params& params, next_function<read_write::push_transactions_results> next) {
   try {
      DATX_ASSERT( params.size() <= 1000, too_many_tx_at_once, "Attempt to push too many transactions at once" );
...
read_only::get_code_results read_only::get_code( const get_code_params& params )const {
   ...
   DATX_ASSERT( params.code_as_wasm, unsupported_feature, "Returning WAST from get_code is no longer supported" );
...
read_only::get_account_results read_only::get_account( const get_account_params& params )const {
   ...
   while( perm != permissions.end() && perm->owner == params.account_name ) {
      ...
      if( perm->parent._id ) {
         ...
         if( p ) {
            DATX_ASSERT(perm->owner == p->owner, invalid_parent_permission, "Invalid parent permission");
            parent = p->name;
         }
      }
   ...
   if( abi_serializer::to_abi(code_account.abi, abi) ) {
      ...
      const auto token_code = N(datxos.token);
...
read_only::abi_json_to_bin_result read_only::abi_json_to_bin( const read_only::abi_json_to_bin_params& params )const try {
   ...
   DATX_ASSERT(code_account != nullptr, contract_query_exception, "Contract can't be found ${contract}", ("contract", params.code));

   abi_def abi;
   if( abi_serializer::to_abi(code_account->abi, abi) ) {
      abi_serializer abis( abi, abi_serializer_max_time );
      auto action_type = abis.get_action_type(params.action);
      DATX_ASSERT(!action_type.empty(), action_validate_exception, "Unknown action ${action} in contract ${contract}", ("action", params.action)("contract", params.code));
      try {
         result.binargs = abis.variant_to_binary(action_type, params.args, abi_serializer_max_time);
      } DATX_RETHROW_EXCEPTIONS(chain::invalid_action_args_exception,
                                "'${args}' is invalid args for action '${action}' code '${code}'. expected '${proto}'",
                                ("args", params.args)("action", params.action)("code", params.code)("proto", action_abi_to_variant(abi, action_type)))
   } else {
      DATX_ASSERT(false, abi_not_found_exception, "No ABI found for ${contract}", ("contract", params.code));
   }
   return result;
...
read_only::get_required_keys_result read_only::get_required_keys( const get_required_keys_params& params )const {
   ...
   } DATX_RETHROW_EXCEPTIONS(chain::transaction_type_exception, "Invalid transaction")
