...
#include <datxos/history_accessory/account_control_history_object.hpp>
#include <datxos/history_accessory/public_key_history_object.hpp>
...
namespace datxos { 
   ...
   static appbase::abstract_accessory& _history_accessory = app().register_accessory<history_accessory>();
...
CHAINBASE_SET_INDEX_TYPE(datxos::account_history_object, datxos::account_history_index)
CHAINBASE_SET_INDEX_TYPE(datxos::action_history_object, datxos::action_history_index)
...
namespace datxos {
   ...
   history_accessory::history_accessory()
   :my(std::make_shared<history_accessory_impl>()) {
   }
   ...
   void history_accessory::set_program_options(options_description& cli, options_description& cfg) {
      cfg.add_options()
   ...
   void history_accessory::accessory_initialize(const variables_map& options) {
      try {
         if( options.count( "filter-on" )) {
            ...
            for( auto& s : fo ) {
               if( s == "*" ) {
                  ...
                  wlog( "--filter-on * enabled. This can fill shared_mem, causing noddatx to stop." );
                  break;
               }
               std::vector<std::string> v;
               boost::split( v, s, boost::is_any_of( ":" ));
               DATX_ASSERT( v.size() == 3, fc::invalid_arg_exception, "Invalid value ${s} for --filter-on", ("s", s));
         ...
         if( options.count( "filter-out" )) {
            ...
            for( auto& s : fo ) {
               std::vector<std::string> v;
               boost::split( v, s, boost::is_any_of( ":" ));
               DATX_ASSERT( v.size() == 3, fc::invalid_arg_exception, "Invalid value ${s} for --filter-out", ("s", s));
               filter_entry fe{v[0], v[1], v[2]};
               DATX_ASSERT( fe.receiver.value, fc::invalid_arg_exception,
   ...
   void history_accessory::accessory_startup() {
   }

   void history_accessory::accessory_shutdown() {
   ...
   namespace history_apis { 
      read_only::get_actions_result read_only::get_actions( const read_only::get_actions_params& params )const {
        ...
        DATX_ASSERT( end >= start, chain::accessory_exception, "end position is earlier than start position" );
      ...
      read_only::get_transaction_result read_only::get_transaction( const read_only::get_transaction_params& p )const {
         ...
         if( !in_history && !p.block_num_hint ) {
            DATX_THROW(tx_not_found, "Transaction ${id} not found in history and no block hint was given", ("id",p.id));
         }
         ...
         if (in_history) {
            ...
            if (!found) {
               DATX_THROW(tx_not_found, "Transaction ${id} not found in history or in block number ${n}", ("id",p.id)("n", *p.block_num_hint));
            }
         }
