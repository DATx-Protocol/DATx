...
http_client_accessory::http_client_accessory():my(new http_client()){}
http_client_accessory::~http_client_accessory(){}

void http_client_accessory::set_program_options(options_description&, options_description& cfg) {
   cfg.add_options()
...
void http_client_accessory::accessory_initialize(const variables_map& options) {
   try {
         ...
         for( const auto& root_pem : root_pems ) {
            ...
            if( !boost::algorithm::starts_with( pem_str, "-----BEGIN CERTIFICATE-----\n" )) {
               try {
                  ...
                  sstr << infile.rdbuf();
                  pem_str = sstr.str();
                  DATX_ASSERT( boost::algorithm::starts_with( pem_str, "-----BEGIN CERTIFICATE-----\n" ),
...
void http_client_accessory::accessory_startup() {

}

void http_client_accessory::accessory_shutdown() {
