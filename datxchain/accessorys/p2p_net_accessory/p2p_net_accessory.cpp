...
#include <datxos/chain/accessory_interface.hpp>
...
using namespace datxos::chain::accessory_interface::compat;
...
namespace datxos {
   static appbase::abstract_accessory& _p2p_net_accessory = app().register_accessory<p2p_net_accessory>();
   ...
   using datxos::chain::transaction_id_type;
   ...
   public:
      ...
      core_accessory*                 chain_plug = nullptr;
   ...
   const fc::string logger_name("p2p_net_accessory_impl");
   fc::logger logger;
...
#define peer_elog( PEER, FORMAT, ... ) \
  ...
  FC_MULTILINE_MACRO_END
   ...
   static p2p_net_accessory_impl *my_impl;
   ...
   struct update_request_time {
      ...
      void operator () (struct datxos::peer_block_state &bs) {
         bs.requested_time = time_point::now();
      }
   } set_request_time;

   typedef multi_index_container<
      datxos::peer_block_state,
      indexed_by<
         ordered_unique< tag<by_id>, member<datxos::peer_block_state, block_id_type, &datxos::peer_block_state::id > >,
         ordered_unique< tag<by_block_num>, member<datxos::peer_block_state, uint32_t, &datxos::peer_block_state::block_num > >
         >
      > peer_block_state_index;


   struct update_known_by_peer {
      void operator() (datxos::peer_block_state& bs) {
         bs.is_known = true;
      }
      void operator() (transaction_state& ts) {
   ...
   public:
       ...
       * part of the message and impl in the net accessory implementation
       * that will handle the message.
       * Returns true is successful. Returns false if an error was
       * encountered unpacking or processing the message.
       */
      bool process_next_message(p2p_net_accessory_impl& impl, uint32_t message_length);
   ...
   struct msgHandler : public fc::visitor<void> {
      p2p_net_accessory_impl &impl;
      connection_ptr c;
      msgHandler( p2p_net_accessory_impl &imp, connection_ptr conn) : impl(imp), c(conn) {}
   ...
   bool connection::enqueue_sync_block() {
      controller& cc = app().find_accessory<core_accessory>()->chain();
      if (!peer_requested)
         return false;
   ...
   bool connection::process_next_message(p2p_net_accessory_impl& impl, uint32_t message_length) {
      try {
   ...
   //-----------------------------------------------------------
    ...
    sync_manager::sync_manager( uint32_t req_span )
      ...
      ,state(in_sync)
   {
      chain_plug = app( ).find_accessory<core_accessory>( );
   ...
   void p2p_net_accessory_impl::connect( connection_ptr c ) {
   ...
   void p2p_net_accessory_impl::connect( connection_ptr c, tcp::resolver::iterator endpoint_itr ) {
      if( c->no_retry != go_away_reason::no_reason) {
   ...
   bool p2p_net_accessory_impl::start_session( connection_ptr con ) {
   ...
   void p2p_net_accessory_impl::start_listen_loop( ) {
   ...
   void p2p_net_accessory_impl::start_read_message( connection_ptr conn ) {
            ...
            [this,weak_conn]( boost::system::error_code ec, std::size_t bytes_transferred ) {
               ...
               try {
                  if( !ec ) {
                     ...
                     DATX_ASSERT(bytes_transferred <= conn->pending_message_buffer.bytes_to_write(), accessory_exception, "");
   ...
   size_t p2p_net_accessory_impl::count_open_sockets() const
   {
      size_t count = 0;
   ...
   template<typename VerifierFunc>
   void p2p_net_accessory_impl::send_all( const net_message &msg, VerifierFunc verify) {
      for( auto &c : connections) {
   ...
   bool p2p_net_accessory_impl::is_valid( const handshake_message &msg) {
   ...
   void p2p_net_accessory_impl::handle_message( connection_ptr c, const chain_size_message &msg) {
      peer_ilog(c, "received chain_size_message");

   }

   void p2p_net_accessory_impl::handle_message( connection_ptr c, const handshake_message &msg) {
   ...
   void p2p_net_accessory_impl::handle_message( connection_ptr c, const go_away_message &msg ) {
   ...
   void p2p_net_accessory_impl::handle_message(connection_ptr c, const time_message &msg) {
   ...
   void p2p_net_accessory_impl::handle_message( connection_ptr c, const notice_message &msg) {
   ...
   void p2p_net_accessory_impl::handle_message( connection_ptr c, const request_message &msg) {
   ...
   void p2p_net_accessory_impl::handle_message( connection_ptr c, const sync_request_message &msg) {
      if( msg.end_block == 0) {
   ...
   void p2p_net_accessory_impl::handle_message( connection_ptr c, const packed_transaction &msg) {
      fc_dlog(logger, "got a packed transaction, cancel wait");
      peer_ilog(c, "received packed_transaction");
      controller& cc = my_impl->chain_plug->chain();
      if( cc.get_read_mode() == datxos::db_read_mode::READ_ONLY ) {
   ...
   void p2p_net_accessory_impl::handle_message( connection_ptr c, const signed_block &msg) {
      controller &cc = chain_plug->chain();
   ...
   void p2p_net_accessory_impl::start_conn_timer(boost::asio::steady_timer::duration du, std::weak_ptr<connection> from_connection) {
   ...
   void p2p_net_accessory_impl::start_txn_timer() {
   ...
   void p2p_net_accessory_impl::ticker() {
   ...
   void p2p_net_accessory_impl::start_monitors() {
   ...
   void p2p_net_accessory_impl::expire_txns() {
      start_txn_timer( );
   ...
   void p2p_net_accessory_impl::connection_monitor(std::weak_ptr<connection> from_connection) {
   ...
   void p2p_net_accessory_impl::close( connection_ptr c ) {
   ...
   void p2p_net_accessory_impl::accepted_block_header(const block_state_ptr& block) {
      fc_dlog(logger,"signaled, id = ${id}",("id", block->id));
   }

   void p2p_net_accessory_impl::accepted_block(const block_state_ptr& block) {
      fc_dlog(logger,"signaled, id = ${id}",("id", block->id));
      dispatcher->bcast_block(*block->block);
   }

   void p2p_net_accessory_impl::irreversible_block(const block_state_ptr&block) {
      fc_dlog(logger,"signaled, id = ${id}",("id", block->id));
   }

   void p2p_net_accessory_impl::accepted_transaction(const transaction_metadata_ptr& md) {
      fc_dlog(logger,"signaled, id = ${id}",("id", md->id));
//      dispatcher->bcast_transaction(md->packed_trx);
   }

   void p2p_net_accessory_impl::applied_transaction(const transaction_trace_ptr& txn) {
      fc_dlog(logger,"signaled, id = ${id}",("id", txn->id));
   }

   void p2p_net_accessory_impl::accepted_confirmation(const header_confirmation& head) {
      fc_dlog(logger,"signaled, id = ${id}",("id", head.block_id));
   }

   void p2p_net_accessory_impl::transaction_ack(const std::pair<fc::exception_ptr, packed_transaction_ptr>& results) {
   ...
   bool p2p_net_accessory_impl::authenticate_peer(const handshake_message& msg) const {
      ...
      if(allowed_connections & (Producers | Specified)) {
         ...
         producer_accessory* pp = app().find_accessory<producer_accessory>();
         if(pp != nullptr)
   ...
   chain::public_key_type p2p_net_accessory_impl::get_authentication_key() const {
      if(!private_keys.empty())
         return private_keys.begin()->first;
      /*producer_accessory* pp = app().find_accessory<producer_accessory>();
      if(pp != nullptr && pp->get_state() == abstract_accessory::started)
         return pp->first_producer_public_key();*/
      return chain::public_key_type();
   }

   chain::signature_type p2p_net_accessory_impl::sign_compact(const chain::public_key_type& signer, const fc::sha256& digest) const
   {
   ...
   p2p_net_accessory::p2p_net_accessory()
      :my( new p2p_net_accessory_impl ) {
      my_impl = my.get();
   }
   ...
   void p2p_net_accessory::set_program_options( options_description& /*cli*/, options_description& cfg )
   {
      cfg.add_options()
         ...
         ( "agent-name", bpo::value<string>()->default_value("\"DATX Test Agent\""), "The name supplied to identify this node amongst the peers.")
   ...
   void p2p_net_accessory::accessory_initialize( const variables_map& options ) {
      ilog("Initialize net accessory");
      try {
         ...
         if( options.count( "allowed-connection" )) {
            ...
            for( const std::string& allowed_remote : allowed_remotes ) {
               if( allowed_remote == "any" )
                  my->allowed_connections |= p2p_net_accessory_impl::Any;
               else if( allowed_remote == "producers" )
                  my->allowed_connections |= p2p_net_accessory_impl::Producers;
               else if( allowed_remote == "specified" )
                  my->allowed_connections |= p2p_net_accessory_impl::Specified;
               else if( allowed_remote == "none" )
                  my->allowed_connections = p2p_net_accessory_impl::None;
            }
         }

         if( my->allowed_connections & p2p_net_accessory_impl::Specified )
            DATX_ASSERT( options.count( "peer-key" ),
         ...
         my->chain_plug = app().find_accessory<core_accessory>();
         DATX_ASSERT( my->chain_plug, chain::missing_core_accessory_exception, ""  );
         my->chain_id = app().get_accessory<core_accessory>().get_chain_id();
   ...
   void p2p_net_accessory::accessory_startup() {
      if( my->acceptor ) {
         ...
         try {
           ...
           ilog("p2p_net_accessory::accessory_startup failed to bind to port ${port}",
      ...
      {
         cc.accepted_block_header.connect( boost::bind(&p2p_net_accessory_impl::accepted_block_header, my.get(), _1));
         cc.accepted_block.connect(  boost::bind(&p2p_net_accessory_impl::accepted_block, my.get(), _1));
         cc.irreversible_block.connect( boost::bind(&p2p_net_accessory_impl::irreversible_block, my.get(), _1));
         cc.accepted_transaction.connect( boost::bind(&p2p_net_accessory_impl::accepted_transaction, my.get(), _1));
         cc.applied_transaction.connect( boost::bind(&p2p_net_accessory_impl::applied_transaction, my.get(), _1));
         cc.accepted_confirmation.connect( boost::bind(&p2p_net_accessory_impl::accepted_confirmation, my.get(), _1));
      }

      my->incoming_transaction_ack_subscription = app().get_channel<channels::transaction_ack>().subscribe(boost::bind(&p2p_net_accessory_impl::transaction_ack, my.get(), _1));
   ...
   void p2p_net_accessory::accessory_shutdown() {
      try {
         ilog( "shutdown.." );
         my->done = true;
         if( my->acceptor ) {
   ...
   size_t p2p_net_accessory::num_peers() const {
    ...
    */
   string p2p_net_accessory::connect( const string& host ) {
   ...
   string p2p_net_accessory::disconnect( const string& host ) {
   ...
   optional<connection_status> p2p_net_accessory::status( const string& host )const {
   ...
   vector<connection_status> p2p_net_accessory::connections()const {
   ...
   connection_ptr p2p_net_accessory_impl::find_connection( string host )const {
      for( const auto& c : connections )
         if( c->peer_addr == host ) return c;
      return connection_ptr();
   }

   uint16_t p2p_net_accessory_impl::to_protocol_version (uint16_t v) {
