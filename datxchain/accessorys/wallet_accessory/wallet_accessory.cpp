...
#include <datxos/wallet_accessory/yubihsm_wallet.hpp>
...
static appbase::abstract_accessory& _wallet_accessory = app().register_accessory<wallet_accessory>();

wallet_accessory::wallet_accessory() {}

wallet_manager& wallet_accessory::get_wallet_manager() {
   return *wallet_manager_ptr;
}

void wallet_accessory::set_program_options(options_description& cli, options_description& cfg) {
   cfg.add_options()
...
void wallet_accessory::accessory_initialize(const variables_map& options) {
   ilog("initializing wallet accessory");
   try {
      ...
      if (options.count("unlock-timeout")) {
         ...
         DATX_ASSERT(timeout > 0, chain::invalid_lock_timeout_exception, "Please specify a positive timeout ${t}", ("t", timeout));
