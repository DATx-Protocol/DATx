 ...
 */
#include <datxos/mongo_db_accessory/mongo_db_accessory.hpp>
...
static appbase::abstract_accessory& _mongo_db_accessory = app().register_accessory<mongo_db_accessory>();
...
class mongo_db_accessory_impl {
public:
   mongo_db_accessory_impl();
   ~mongo_db_accessory_impl();
...
const action_name mongo_db_accessory_impl::newaccount = chain::newaccount::get_name();
const action_name mongo_db_accessory_impl::setabi = chain::setabi::get_name();
const action_name mongo_db_accessory_impl::updateauth = chain::updateauth::get_name();
const action_name mongo_db_accessory_impl::deleteauth = chain::deleteauth::get_name();
const permission_name mongo_db_accessory_impl::owner = chain::config::owner_name;
const permission_name mongo_db_accessory_impl::active = chain::config::active_name;

const std::string mongo_db_accessory_impl::block_states_col = "block_states";
const std::string mongo_db_accessory_impl::blocks_col = "blocks";
const std::string mongo_db_accessory_impl::trans_col = "transactions";
const std::string mongo_db_accessory_impl::trans_traces_col = "transaction_traces";
const std::string mongo_db_accessory_impl::action_traces_col = "action_traces";
const std::string mongo_db_accessory_impl::accounts_col = "accounts";
const std::string mongo_db_accessory_impl::pub_keys_col = "pub_keys";
const std::string mongo_db_accessory_impl::account_controls_col = "account_controls";

bool mongo_db_accessory_impl::filter_include( const chain::action_trace& action_trace ) const {
   bool include = false;
...
void mongo_db_accessory_impl::queue( Queue& queue, const Entry& e ) {
...
void mongo_db_accessory_impl::accepted_transaction( const chain::transaction_metadata_ptr& t ) {
   try {
...
void mongo_db_accessory_impl::applied_transaction( const chain::transaction_trace_ptr& t ) {
   try {
...
void mongo_db_accessory_impl::applied_irreversible_block( const chain::block_state_ptr& bs ) {
   try {
...
void mongo_db_accessory_impl::accepted_block( const chain::block_state_ptr& bs ) {
   try {
...
void mongo_db_accessory_impl::consume_blocks() {
   try {
      while (true) {
         boost::mutex::scoped_lock lock(mtx);
      ...
      ilog("mongo_db_accessory consume thread shutdown gracefully");
   } catch (fc::exception& e) {
...
void mongo_db_accessory_impl::purge_abi_cache() {
...
optional<abi_serializer> mongo_db_accessory_impl::get_abi_serializer( account_name n ) {
   using bsoncxx::builder::basic::kvp;
   using bsoncxx::builder::basic::make_document;
   if( n.good()) {
      try {
         ...
         if(account) {
            ...
            if( view.find( "abi" ) != view.end()) {
               ...
               if( n == chain::config::system_account_name ) {
                  // redefine datxos setabi.abi from bytes to abi_def
                           ...
                           abis.add_specialized_unpack_pack( "abi_def",
                                 std::make_pair<abi_serializer::unpack_function, abi_serializer::pack_function>(
                                       []( fc::datastream<const char*>& stream, bool is_array, bool is_optional ) -> fc::variant {
                                          DATX_ASSERT( !is_array && !is_optional, chain::mongo_db_exception, "unexpected abi_def");
                                          chain::bytes temp;
                                       ...
                                       []( const fc::variant& var, fc::datastream<char*>& ds, bool is_array, bool is_optional ) {
                                          DATX_ASSERT( false, chain::mongo_db_exception, "never called" );
                                       }
                                 ) );
                        }
                     }
...
template<typename T>
fc::variant mongo_db_accessory_impl::to_variant_with_abi( const T& obj ) {
...
void mongo_db_accessory_impl::process_accepted_transaction( const chain::transaction_metadata_ptr& t ) {
   try {
      if( start_block_reached ) {
...
void mongo_db_accessory_impl::process_applied_transaction( const chain::transaction_trace_ptr& t ) {
   try {
...
void mongo_db_accessory_impl::process_irreversible_block(const chain::block_state_ptr& bs) {
  try {
     if( start_block_reached ) {
...
void mongo_db_accessory_impl::process_accepted_block( const chain::block_state_ptr& bs ) {
   try {
      if( start_block_reached ) {
...
void mongo_db_accessory_impl::_process_accepted_transaction( const chain::transaction_metadata_ptr& t ) {
   ...
   try {
      ...
      if( !trans.update_one( make_document( kvp( "trx_id", trx_id_str ) ),
         ...
         DATX_ASSERT( false, chain::mongo_db_insert_fail, "Failed to insert trans ${id}", ("id", trx_id) );
      }
   } catch( ... ) {
...
bool
mongo_db_accessory_impl::add_action_trace( mongocxx::bulk_write& bulk_action_traces, const chain::action_trace& atrace,
                                        bool executed, const std::chrono::milliseconds& now )
{
   using namespace bsoncxx::types;
   using bsoncxx::builder::basic::kvp;
...
void mongo_db_accessory_impl::_process_applied_transaction( const chain::transaction_trace_ptr& t ) {
   using namespace bsoncxx::types;
   using bsoncxx::builder::basic::kvp;
   ...
   if( write_atraces ) {
      try {
         if( !bulk_action_traces.execute() ) {
            DATX_ASSERT( false, chain::mongo_db_insert_fail, "Bulk action traces insert failed for transaction trace: ${id}", ("id", t->id));
         }
      } catch(...) {
   ...
   try {
      if( !trans_traces.insert_one( trans_traces_doc.view())) {
         DATX_ASSERT( false, chain::mongo_db_insert_fail, "Failed to insert trans ${id}", ("id", t->id));
      }
   } catch(...) {
...
void mongo_db_accessory_impl::_process_accepted_block( const chain::block_state_ptr& bs ) {
   using namespace bsoncxx::types;
   ...
   if( store_block_states ) {
      ...
      try {
         if( !block_states.update_one( make_document( kvp( "block_id", block_id_str ) ),
            ...
            DATX_ASSERT( false, chain::mongo_db_insert_fail, "Failed to insert block_state ${bid}", ("bid", block_id) );
         }
      } catch( ... ) {
   ...
   if( store_blocks ) {
      ...
      try {
         if( !blocks.update_one( make_document( kvp( "block_id", block_id_str ) ),
            ...
            DATX_ASSERT( false, chain::mongo_db_insert_fail, "Failed to insert block ${bid}", ("bid", block_id) );
         }
      } catch( ... ) {
...
void mongo_db_accessory_impl::_process_irreversible_block(const chain::block_state_ptr& bs)
{
   using namespace bsoncxx::types;
   using namespace bsoncxx::builder;
   using bsoncxx::builder::basic::make_document;
   ...
   if( store_transactions ) {
      ...
      if( transactions_in_block ) {
         try {
            if( !bulk.execute() ) {
               DATX_ASSERT( false, chain::mongo_db_insert_fail, "Bulk transaction insert failed for block: ${bid}", ("bid", block_id) );
            }
         } catch( ... ) {
...
void mongo_db_accessory_impl::add_pub_keys( const vector<chain::key_weight>& keys, const account_name& name,
                                         const permission_name& permission, const std::chrono::milliseconds& now )
{
   using bsoncxx::builder::basic::kvp;
   using bsoncxx::builder::basic::make_document;
   ...
   try {
      if( !bulk.execute()) {
         DATX_ASSERT( false, chain::mongo_db_insert_fail,
...
void mongo_db_accessory_impl::remove_pub_keys( const account_name& name, const permission_name& permission )
{
   using bsoncxx::builder::basic::kvp;
   using bsoncxx::builder::basic::make_document;
   ...
   try {
      auto result = pub_keys.delete_many( make_document( kvp( "account", name.to_string()),
                                                         kvp( "permission", permission.to_string())));
      if( !result ) {
         DATX_ASSERT( false, chain::mongo_db_update_fail,
...
void mongo_db_accessory_impl::add_account_control( const vector<chain::permission_level_weight>& controlling_accounts,
                                                const account_name& name, const permission_name& permission,
                                                const std::chrono::milliseconds& now )
{
   using bsoncxx::builder::basic::kvp;
...
void mongo_db_accessory_impl::remove_account_control( const account_name& name, const permission_name& permission )
{
   using bsoncxx::builder::basic::kvp;
   using bsoncxx::builder::basic::make_document;
...
void create_account( mongocxx::collection& accounts, const name& name, std::chrono::milliseconds& now ) {
   ...
   try {
      if( !accounts.update_one( make_document( kvp( "name", name_str )), update.view(), update_opts )) {
         DATX_ASSERT( false, chain::mongo_db_update_fail, "Failed to insert account ${n}", ("n", name));
      }
   } catch (...) {
...
void mongo_db_accessory_impl::update_account(const chain::action& act)
{
   using bsoncxx::builder::basic::kvp;
   using bsoncxx::builder::basic::make_document;
   using namespace bsoncxx::types;
            ...
            try{
               ...
               try {
                  if( !accounts.update_one( make_document( kvp( "_id", account->view()["_id"].get_oid())),
                                            update_from.view())) {
                     DATX_ASSERT( false, chain::mongo_db_update_fail, "Failed to udpdate account ${n}", ("n", setabi.account));
                  }
               } catch( ... ) {
...
mongo_db_accessory_impl::mongo_db_accessory_impl()
: mongo_inst{}
, mongo_conn{}
{
}
...
mongo_db_accessory_impl::~mongo_db_accessory_impl() {
   if (!startup) {
      try {
         ilog( "mongo_db_accessory shutdown in process please be patient this can take a few minutes" );
         done = true;
         condition.notify_one();

         consume_thread.join();
      } catch( std::exception& e ) {
         elog( "Exception on mongo_db_accessory shutdown of consume thread: ${e}", ("e", e.what()));
      }
   }
}

void mongo_db_accessory_impl::wipe_database() {
...
void mongo_db_accessory_impl::init() {
   using namespace bsoncxx::types;
   using bsoncxx::builder::basic::make_document;
   using bsoncxx::builder::basic::kvp;
   ...
   if (accounts.count(make_document()) == 0) {
      ...
      try {
         if( !accounts.insert_one( doc.view())) {
            DATX_ASSERT( false, chain::mongo_db_insert_fail, "Failed to insert account ${n}",
   ...
   ilog("starting db accessory thread");
...
mongo_db_accessory::mongo_db_accessory()
:my(new mongo_db_accessory_impl)
{
}
...
void mongo_db_accessory::set_program_options(options_description& cli, options_description& cfg)
{
   cfg.add_options()
         ("mongodb-queue-size,q", bpo::value<uint32_t>()->default_value(1024),
         "The target queue size between noddatx and MongoDB accessory thread.")
         ...
         "MongoDB URI connection string, see: https://docs.mongodb.com/master/reference/connection-string/."
               " If not specified then accessory is disabled. Default database 'DATX' is used if not specified in URI."
               " Example: mongodb://127.0.0.1:27017/DATX")
...
void mongo_db_accessory::accessory_initialize(const variables_map& options)
{
   try {
      if( options.count( "mongodb-uri" )) {
         ilog( "initializing mongo_db_accessory" );
         my->configured = true;
         ...
         if( options.at( "replay-blockchain" ).as<bool>() || options.at( "hard-replay-blockchain" ).as<bool>() || options.at( "delete-all-blocks" ).as<bool>() ) {
            if( options.at( "mongodb-wipe" ).as<bool>()) {
               ...
               DATX_ASSERT( false, chain::accessory_config_exception, "--mongodb-wipe required with --replay-blockchain, --hard-replay-blockchain, or --delete-all-blocks"
                                 " --mongodb-wipe will remove all DATX collections from mongodb." );
            }
         }

         if( options.count( "abi-serializer-max-time-ms") == 0 ) {
            DATX_ASSERT(false, chain::accessory_config_exception, "--abi-serializer-max-time-ms required as default value not appropriate for parsing full blocks");
         }
         my->abi_serializer_max_time = app().get_accessory<core_accessory>().get_abi_serializer_max_time();
         ...
         if( options.count( "mongodb-abi-cache-size" )) {
            ...
            DATX_ASSERT( my->abi_cache_size > 0, chain::accessory_config_exception, "mongodb-abi-cache-size > 0 required" );
         }
         ...
         if( options.count( "mongodb-filter-on" )) {
            ...
            for( auto& s : fo ) {
               ...
               DATX_ASSERT( v.size() == 3, fc::invalid_arg_exception, "Invalid value ${s} for --mongodb-filter-on", ("s", s));
               filter_entry fe{v[0], v[1], v[2]};
               DATX_ASSERT( fe.receiver.value && fe.action.value, fc::invalid_arg_exception,
         ...
         if( options.count( "mongodb-filter-out" )) {
            ...
            for( auto& s : fo ) {
               std::vector<std::string> v;
               boost::split( v, s, boost::is_any_of( ":" ));
               DATX_ASSERT( v.size() == 3, fc::invalid_arg_exception, "Invalid value ${s} for --mongodb-filter-out", ("s", s));
               filter_entry fe{v[0], v[1], v[2]};
               DATX_ASSERT( fe.receiver.value, fc::invalid_arg_exception,
         ...
         core_accessory* chain_plug = app().find_accessory<core_accessory>();
         DATX_ASSERT( chain_plug, chain::missing_core_accessory_exception, ""  );
         ...
         wlog( "datxos::mongo_db_accessory configured, but no --mongodb-uri specified." );
         wlog( "mongo_db_accessory disabled." );
      }
   } FC_LOG_AND_RETHROW()
}

void mongo_db_accessory::accessory_startup()
{
}

void mongo_db_accessory::accessory_shutdown()
{
