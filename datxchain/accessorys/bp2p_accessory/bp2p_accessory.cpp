...
#include <datxos/bp2p_accessory/bp2p_accessory.hpp>
...
namespace datxos {
   ...
   static appbase::abstract_accessory& _bp2p_accessory = app().register_accessory<bp2p_accessory>();
...
const fc::string logger_name("bp2p_accessory");
fc::logger accessory_logger;
std::string peer_log_format;

#define peer_dlog( PEER, FORMAT, ... ) \
  FC_MULTILINE_MACRO_BEGIN \
   if( accessory_logger.is_enabled( fc::log_level::debug ) ) \
      accessory_logger.log( FC_LOG_MESSAGE( debug, peer_log_format + FORMAT, __VA_ARGS__ (PEER->get_logger_variant()) ) ); \
  FC_MULTILINE_MACRO_END

#define peer_ilog( PEER, FORMAT, ... ) \
  FC_MULTILINE_MACRO_BEGIN \
   if( accessory_logger.is_enabled( fc::log_level::info ) ) \
      accessory_logger.log( FC_LOG_MESSAGE( info, peer_log_format + FORMAT, __VA_ARGS__ (PEER->get_logger_variant()) ) ); \
  FC_MULTILINE_MACRO_END

#define peer_wlog( PEER, FORMAT, ... ) \
  FC_MULTILINE_MACRO_BEGIN \
   if( accessory_logger.is_enabled( fc::log_level::warn ) ) \
      accessory_logger.log( FC_LOG_MESSAGE( warn, peer_log_format + FORMAT, __VA_ARGS__ (PEER->get_logger_variant()) ) ); \
  FC_MULTILINE_MACRO_END

#define peer_elog( PEER, FORMAT, ... ) \
  FC_MULTILINE_MACRO_BEGIN \
   if( accessory_logger.is_enabled( fc::log_level::error ) ) \
      accessory_logger.log( FC_LOG_MESSAGE( error, peer_log_format + FORMAT, __VA_ARGS__ (PEER->get_logger_variant())) ); \
  FC_MULTILINE_MACRO_END


using datxos::public_key_type;
using datxos::chain_id_type;
using datxos::block_id_type;
using datxos::block_timestamp_type;
using std::string;
using datxos::sha256;
using datxos::signed_block_ptr;
using datxos::packed_transaction_ptr;
using std::vector;
...
namespace datxos {
  using namespace chain::accessory_interface;

  class bp2p_accessory_impl;
     ...
     public:
       ...
       //    shared_ptr< vector<char> >  block_msg; ///< packed bnet_message for this block
        ...
        :_resolver(socket.get_io_service()),
         _p2p_net_accessory( std::move(net_plug) ),
        ...
        void async_get_pending_block_ids( L&& callback ) {
           /// send peer my head block status which is read from chain accessory
           _app_ios.post( [self = shared_from_this(),callback]{
              auto& control = app().get_accessory<core_accessory>().chain();
        ...
        void on( const signed_block_ptr& b ) {
           ...
           if (!b) {
              ...
              DATX_THROW(block_validate_exception, "bad block" );
           }
        ...
        void on( const packed_transaction_ptr& p ) {
           ...
           if (!p) {
              ...
              DATX_THROW(transaction_exception, "bad transaction");
           }
           if( app().get_accessory<core_accessory>().chain().get_read_mode() == chain::db_read_mode::READ_ONLY )
              return;
  ...
  class listener : public std::enable_shared_from_this<listener> {
     ...
     public:
        ...
        :_acceptor(ioc), _socket(ioc), _p2p_net_accessory(std::move(np))
        {
           boost::system::error_code ec;
        ...
        void run() {
           DATX_ASSERT( _acceptor.is_open(), accessory_exception, "unable top open listen socket" );
           do_accept();
        }
   ...
   class bp2p_accessory_impl : public std::enable_shared_from_this<bp2p_accessory_impl> {
      public:
         bp2p_accessory_impl() = default;
         ...
         std::unique_ptr<boost::asio::io_context>               _ioc; // lifetime guarded by shared_ptr of bp2p_accessory_impl
   ...
   void listener::on_accept( boost::system::error_code ec ) {
     ...
     try {
        newsession = std::make_shared<session>( move( _socket ), _p2p_net_accessory );
     }
     ...
     if( newsession ) {
        _p2p_net_accessory->async_add_session( newsession );
        newsession->_local_peer_id = _p2p_net_accessory->_peer_id;
        newsession->run();
     }
     do_accept();
   }
   ...
   bp2p_accessory::bp2p_accessory()
   :my(std::make_shared<bp2p_accessory_impl>()) {
   }

   bp2p_accessory::~bp2p_accessory() {
   }

   void bp2p_accessory::set_program_options(options_description& cli, options_description& cfg) {
      cfg.add_options()
   ...
   void bp2p_accessory::accessory_initialize(const variables_map& options) {
      ilog( "Initialize bnet accessory" );
   ...
   void bp2p_accessory::accessory_startup() {
      if(fc::get_logger_map().find(logger_name) != fc::get_logger_map().end())
         accessory_logger = fc::get_logger_map()[logger_name];
   ...
   void bp2p_accessory::accessory_shutdown() {
      try {
         my->_timer->cancel();
         my->_timer.reset();
      } catch ( ... ) {
      ...
      my->for_each_session([](auto ses){
         DATX_ASSERT( false, accessory_exception, "session ${ses} still active", ("ses", ses->_session_num) );
      });

      // lifetime of _ioc is guarded by shared_ptr of bp2p_accessory_impl
   }


   session::~session() {
     wlog( "close session ${n}",("n",_session_num) );
     std::weak_ptr<bp2p_accessory_impl> netp = _p2p_net_accessory;
   ...
   void session::do_hello() {
      ...
      async_get_pending_block_ids( [self = shared_from_this() ]( const vector<block_id_type>& ids, uint32_t lib ){
          ...
          hello_msg.request_transactions = self->_p2p_net_accessory->_request_trx;
          hello_msg.chain_id = app().get_accessory<core_accessory>().get_chain_id(); // TODO: Quick fix in a rush. Maybe a better solution is needed.

          self->_local_lib = lib;
          if ( self->_p2p_net_accessory->_follow_irreversible ) {
   ...
   void session::check_for_redundant_connection() {
     app().get_io_service().post( [self=shared_from_this()]{
       self->_p2p_net_accessory->for_each_session( [self]( auto ses ){
   ...
   void session::on( const hello& hi, fc::datastream<const char*>& ds ) {
      ...
      if( hi.chain_id != app().get_accessory<core_accessory>().get_chain_id() ) { // TODO: Quick fix in a rush. Maybe a better solution is needed.
      ...
      if ( _p2p_net_accessory->_follow_irreversible && hi.protocol_version <= "1.0.0") {
