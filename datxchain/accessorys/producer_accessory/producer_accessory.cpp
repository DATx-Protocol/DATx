...
#include <datxos/chain/transaction_object.hpp>
...
const fc::string logger_name("producer_accessory");
fc::logger _log;

namespace datxos {

static appbase::abstract_accessory& _producer_accessory = app().register_accessory<producer_accessory>();
...
class producer_accessory_impl : public std::enable_shared_from_this<producer_accessory_impl> {
   public:
      producer_accessory_impl(boost::asio::io_service& io)
      :_timer(io)
      ...
      uint32_t _production_skip_flags              = 0; //datxos::chain::skip_nothing;
   ...
   //                  ilog( "${n} confirmed", ("n",name(producer)) );
         ...
         chain::controller& chain = app().get_accessory<core_accessory>().chain();
      ...
      void on_incoming_block(const signed_block_ptr& block) {
         ...
         DATX_ASSERT( block->timestamp < (fc::time_point::now() + fc::seconds(7)), block_from_the_future, "received a block from the future, ignoring it" );
         ...
         try {
            ...
            app().get_accessory<core_accessory>().handle_guard_exception(e);
            return;
         } catch( const fc::exception& e ) {
      ...
      void schedule_delayed_production_loop(const std::weak_ptr<producer_accessory_impl>& weak_this, const block_timestamp_type& current_block_time);
};

void new_chain_banner(const datxos::chain::controller& db)
{
   std::cerr << "\n"
      "*******************************\n"
      "*                             *\n"
...
producer_accessory::producer_accessory()
   : my(new producer_accessory_impl(app().get_io_service())){
      my->_self = this;
   }

producer_accessory::~producer_accessory() {}

void producer_accessory::set_program_options(
   ...
   boost::program_options::options_description& config_file_options)
{
   ...
   producer_options.add_options()
         ...
         ("signature-provider", boost::program_options::value<vector<string>>()->composing()->multitoken()->default_value({std::string(default_priv_key.get_public_key()) + "=KEY:" + std::string(default_priv_key)}, std::string(default_priv_key.get_public_key()) + "=KEY:" + std::string(default_priv_key)),
          ...
          "Where:\n"
          "   <public-key>    \tis a string form of a vaild DATXOS public key\n\n"
          "   <provider-spec> \tis a string in the form <provider-type>:<data>\n\n"
          "   <provider-type> \tis KEY, or KDATXD\n\n"
          "   KEY:<data>      \tis a string form of a valid DATXOS private key which maps to the provided public key\n\n"
          "   KDATXD:<data>    \tis the URL where kdatxd is available and the approptiate wallet(s) are unlocked")
         ("kdatxd-provider-timeout", boost::program_options::value<int32_t>()->default_value(5),
          "Limits the maximum time (in milliseconds) that is allowd for sending blocks to a kdatxd provider for signing")
...
bool producer_accessory::is_producer_key(const chain::public_key_type& key) const
{
...
chain::signature_type producer_accessory::sign_compact(const chain::public_key_type& key, const fc::sha256& digest) const
{
  if(key != chain::public_key_type()) {
    auto private_key_itr = my->_signature_providers.find(key);
    DATX_ASSERT(private_key_itr != my->_signature_providers.end(), producer_priv_key_not_found, "Local producer has no private key in config.ini corresponding to public key ${key}", ("key", key));
...
static producer_accessory_impl::signature_provider_type
...
static producer_accessory_impl::signature_provider_type
make_kdatxd_signature_provider(const std::shared_ptr<producer_accessory_impl>& impl, const string& url_str, const public_key_type pubkey) {
   auto kdatxd_url = fc::url(url_str);
   std::weak_ptr<producer_accessory_impl> weak_impl = impl;

   return [weak_impl, kdatxd_url, pubkey]( const chain::digest_type& digest ) {
      auto impl = weak_impl.lock();
      if (impl) {
         fc::variant params;
         fc::to_variant(std::make_pair(digest, pubkey), params);
         auto deadline = impl->_kdatxd_provider_timeout_us.count() >= 0 ? fc::time_point::now() + impl->_kdatxd_provider_timeout_us : fc::time_point::maximum();
         return app().get_accessory<http_client_accessory>().get_client().post_sync(kdatxd_url, params, deadline).as<chain::signature_type>();
      } else {
...
void producer_accessory::accessory_initialize(const boost::program_options::variables_map& options)
{ try {
   my->_options = &options;
   ...
   if( options.count("signature-provider") ) {
      ...
      for (const auto& key_spec_pair : key_spec_pairs) {
         try {
            ...
            DATX_ASSERT(delim != std::string::npos, accessory_config_exception, "Missing \"=\" in the key spec pair");
            auto pub_key_str = key_spec_pair.substr(0, delim);
            auto spec_str = key_spec_pair.substr(delim + 1);

            auto spec_delim = spec_str.find(":");
            DATX_ASSERT(spec_delim != std::string::npos, accessory_config_exception, "Missing \":\" in the key spec pair");
            ...
            } else if (spec_type_str == "KDATXD") {
               my->_signature_providers[pubkey] = make_kdatxd_signature_provider(my, spec_data, pubkey);
            }
   ...
   my->_kdatxd_provider_timeout_us = fc::milliseconds(options.at("kdatxd-provider-timeout").as<int32_t>());
...
void producer_accessory::accessory_startup()
{ try {
   if(fc::get_logger_map().find(logger_name) != fc::get_logger_map().end()) {
      _log = fc::get_logger_map()[logger_name];
   }

   ilog("producer accessory:  accessory_startup() begin");

   chain::controller& chain = app().get_accessory<core_accessory>().chain();
   DATX_ASSERT( my->_producers.empty() || chain.get_read_mode() == chain::db_read_mode::SPECULATIVE, accessory_config_exception,
              "node cannot have any producer-name configured because block production is impossible when read_mode is not \"speculative\"" );

   DATX_ASSERT( my->_producers.empty() || chain.get_validation_mode() == chain::validation_mode::FULL, accessory_config_exception,
   ...
   if (!my->_producers.empty()) {
      ...
      if (my->_production_enabled) {
         ...
         //_production_skip_flags |= datxos::chain::skip_undo_history_check;
      }
   }

   my->schedule_production_loop();

   ilog("producer accessory:  accessory_startup() end");
} FC_CAPTURE_AND_RETHROW() }

void producer_accessory::accessory_shutdown() {
   try {
      my->_timer.cancel();
   } catch(fc::exception& e) {
...
void producer_accessory::pause() {
   my->_pause_production = true;
}

void producer_accessory::resume() {
...
bool producer_accessory::paused() const {
   return my->_pause_production;
}

void producer_accessory::update_runtime_options(const runtime_options& options) {
...
producer_accessory::runtime_options producer_accessory::get_runtime_options() const {
   return {
...
void producer_accessory::add_greylist_accounts(const greylist_params& params) {
   chain::controller& chain = app().get_accessory<core_accessory>().chain();
...
void producer_accessory::remove_greylist_accounts(const greylist_params& params) {
   chain::controller& chain = app().get_accessory<core_accessory>().chain();
   for (auto &acc : params.accounts) {
...
producer_accessory::greylist_params producer_accessory::get_greylist() const {
   chain::controller& chain = app().get_accessory<core_accessory>().chain();
...
producer_accessory::whitelist_blacklist producer_accessory::get_whitelist_blacklist() const {
   chain::controller& chain = app().get_accessory<core_accessory>().chain();
   return {
...
void producer_accessory::set_whitelist_blacklist(const producer_accessory::whitelist_blacklist& params) {
   chain::controller& chain = app().get_accessory<core_accessory>().chain();
...
optional<fc::time_point> producer_accessory_impl::calculate_next_block_time(const account_name& producer_name, const block_timestamp_type& current_block_time) const {
   chain::controller& chain = app().get_accessory<core_accessory>().chain();
...
fc::time_point producer_accessory_impl::calculate_pending_block_time() const {
   const chain::controller& chain = app().get_accessory<core_accessory>().chain();
...
producer_accessory_impl::start_block_result producer_accessory_impl::start_block(bool &last_block) {
   chain::controller& chain = app().get_accessory<core_accessory>().chain();
...
void producer_accessory_impl::schedule_production_loop() {
   chain::controller& chain = app().get_accessory<core_accessory>().chain();
   _timer.cancel();
   std::weak_ptr<producer_accessory_impl> weak_this = shared_from_this();
   ...
   if (result == start_block_result::failed) {
      ...
      if (result == start_block_result::succeeded) {
         ...
         DATX_ASSERT( chain.pending_block_state(), missing_pending_block_state, "producing without pending_block_state, start_block succeeded" );
         ...
         DATX_ASSERT( chain.pending_block_state(), missing_pending_block_state, "producing without pending_block_state" );
      ...
      DATX_ASSERT( chain.pending_block_state(), missing_pending_block_state, "speculating without pending_block_state" );
      const auto& pbs = chain.pending_block_state();
...
void producer_accessory_impl::schedule_delayed_production_loop(const std::weak_ptr<producer_accessory_impl>& weak_this, const block_timestamp_type& current_block_time) {
...
bool producer_accessory_impl::maybe_produce_block() {
...
void producer_accessory_impl::produce_block() {
   //ilog("produce_block ${t}", ("t", fc::time_point::now())); // for testing _produce_time_offset_us
   DATX_ASSERT(_pending_block_mode == pending_block_mode::producing, producer_exception, "called produce_block while not actually producing");
   chain::controller& chain = app().get_accessory<core_accessory>().chain();
   const auto& pbs = chain.pending_block_state();
   const auto& hbs = chain.head_block_state();
   DATX_ASSERT(pbs, missing_pending_block_state, "pending_block_state does not exist but it should, another accessory may have corrupted it");
   auto signature_provider_itr = _signature_providers.find( pbs->block_signing_key );

   DATX_ASSERT(signature_provider_itr != _signature_providers.end(), producer_priv_key_not_found, "Attempting to produce a block for which we don't have the private key");
