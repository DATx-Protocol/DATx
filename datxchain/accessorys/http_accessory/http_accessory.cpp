...
namespace datxos {
   ...
   static appbase::abstract_accessory& _http_accessory = app().register_accessory<http_accessory>();
      ...
      public:
         ...
         ssl_context_ptr on_tls_init(websocketpp::connection_hdl hdl) {
            ...
            try {
               ...
               if (!ecdh)
                  DATX_THROW(chain::http_exception, "Failed to set NID_secp384r1");
               if(SSL_CTX_set_tmp_ecdh(ctx->native_handle(), (EC_KEY*)ecdh) != 1)
                  DATX_THROW(chain::http_exception, "Failed to set ECDH PFS");
               ...
               if(SSL_CTX_set_cipher_list(ctx->native_handle(), \
                  ...
                  DATX_THROW(chain::http_exception, "Failed to set HTTPS cipher list");
   ...
   http_accessory::http_accessory():my(new http_accessory_impl()){}
   http_accessory::~http_accessory(){}

   void http_accessory::set_program_options(options_description&, options_description& cfg) {
      cfg.add_options()
   ...
   void http_accessory::accessory_initialize(const variables_map& options) {
      try {
   ...
   void http_accessory::accessory_startup() {
      if(my->listen_endpoint) {
         try {
   ...
   void http_accessory::accessory_shutdown() {
   ...
   void http_accessory::add_handler(const string& url, const url_handler& handler) {
   ...
   void http_accessory::handle_exception( const char *api_name, const char *call_name, const string& body, url_response_callback cb ) {
      try {
         try {
            throw;
   ...
   bool http_accessory::is_on_loopback() const {
   ...
   bool http_accessory::is_secure() const {
