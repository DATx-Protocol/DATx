...
func GetDATXResource(account string) (*ResourceInfo, error) {
	...
	stats, err := GetDATXStats("DATX", "datxos.token")
	if err != nil {
		return nil, fmt.Errorf("datx get_currency_stats error %v", err)
	}
	supply := QuantityToAmount(stats.Data.Supply)

	info := &ResourceInfo{}

	cpuWeight, _ := strconv.ParseFloat(resource.CPUWeight, 64)
	netWeight, _ := strconv.ParseFloat(resource.NetWeight, 64)
	...
	cpuResource := float64(cpuWeight) / 10000
	netResource := float64(netWeight) / 10000
	ramResource := ramTotal / (64*1024*1024*1024 + 1) * supply
...
// GetDATXStats ...
func GetDATXStats(token string, code string) (*DATXStats, error) {
	formData := make(map[string]interface{})
	formData["code"] = code
	formData["symbol"] = token
	bytesData, err := json.Marshal(formData)
	if err != nil {
		return nil, fmt.Errorf("datx get_currency_stats parameter error %v", formData)
	}

	URL := WalletConfig.DatxIP + "/v1/chain/get_currency_stats"
	request, err := http.NewRequest("POST", URL, bytes.NewReader(bytesData))
	if err != nil {
		return nil, fmt.Errorf("datx get_currency_stats request error %v", err)
	}
	request.Header.Set("Content-Type", "application/json")
	...
	resp, err := client.Do(request)
	if err != nil {
		return nil, fmt.Errorf("datx get_currency_stats resp error %v", err)
	}
	defer resp.Body.Close()

	if resp.Status != "200 OK" {
		return nil, fmt.Errorf("datx get_currency_stats not found %v", resp.Status)
	}
	body, err := ioutil.ReadAll(resp.Body)
	if err != nil {
		return nil, fmt.Errorf("datx get_account body error %v", err)
	...
	if err := json.Unmarshal([]byte(body), &stats); err != nil {
		return nil, fmt.Errorf("datx get_currency_stats unmarshal error %v", string(body))
	}
