...
void apply_context::exec()
{
   ...
   if( _cfa_inline_actions.size() > 0 || _inline_actions.size() > 0 ) {
      DATX_ASSERT( recurse_depth < control.get_global_properties().configuration.max_inline_action_depth,
...
void apply_context::require_authorization( const account_name& account ) {
   ...
   DATX_ASSERT( false, missing_auth_exception, "missing authority of ${account}", ("account",account));
}
...
void apply_context::require_authorization(const account_name& account,
  ...
  DATX_ASSERT( false, missing_auth_exception, "missing authority of ${account}/${permission}",
...
void apply_context::execute_inline( action&& a ) {
   ...
   DATX_ASSERT( code != nullptr, action_validate_exception,
               "inline action's code account ${account} does not exist", ("account", a.account) );

   for( const auto& auth : a.authorization ) {
      auto* actor = control.db().find<account_object, by_name>(auth.actor);
      DATX_ASSERT( actor != nullptr, action_validate_exception,
                  "inline action's authorizing actor ${account} does not exist", ("account", auth.actor) );
      DATX_ASSERT( control.get_authorization_manager().find_permission(auth) != nullptr, action_validate_exception,
   ...
   if( !control.skip_auth_check() && !privileged && a.account != receiver ) {
      control.get_authorization_manager()
             .check_authorization( {a},
                                   {},
                                   {{receiver, config::datxio_code_name}},
...
void apply_context::execute_context_free_inline( action&& a ) {
   ...
   DATX_ASSERT( a.authorization.size() == 0, action_validate_exception,
...
void apply_context::schedule_deferred_transaction( const uint128_t& sender_id, account_name payer, transaction&& trx, bool replace_existing ) {
   DATX_ASSERT( trx.context_free_actions.size() == 0, cfa_inside_generated_tx, "context free actions are not currently allowed in generated transactions" );
   ...
   if ( auto ptr = d.find<generated_transaction_object,by_sender_id>(boost::make_tuple(receiver, sender_id)) ) {
      DATX_ASSERT( replace_existing, deferred_tx_duplicate, "deferred transaction with the same sender_id and payer already exists" );

      // TODO: Remove the following subjective check when the deferred trx replacement RAM bug has been fixed with a hard fork.
      DATX_ASSERT( !control.is_producing_block(), subjective_block_production_exception,
...
int apply_context::get_action( uint32_t type, uint32_t index, char* buffer, size_t buffer_size )const
{
   ...
   DATX_ASSERT(act_ptr, action_not_found_exception, "action is not found" );
...
//   require_write_lock( scope );
   ...
   DATX_ASSERT( payer != account_name(), invalid_table_payer, "must specify a valid account to pay for new record" );
...
void apply_context::db_update_i64( int iterator, account_name payer, const char* buffer, size_t buffer_size ) {
   ...
   DATX_ASSERT( table_obj.code == receiver, table_access_violation, "db access violation" );
...
int apply_context::db_previous_i64( int iterator, uint64_t& primary ) {
   ...
   if( iterator < -1 ) // is end iterator
   {
      ...
      DATX_ASSERT( tab, invalid_table_iterator, "not a valid end iterator" );
