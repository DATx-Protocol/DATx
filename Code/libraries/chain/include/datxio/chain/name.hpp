...
#define N(X) datxio::chain::string_to_name(#X)
...
namespace std {
   template<> struct hash<datxio::chain::name> : private hash<uint64_t> {
      typedef datxio::chain::name argument_type;
...
namespace fc {
  class variant;
  void to_variant(const datxio::chain::name& c, fc::variant& v);
  void from_variant(const fc::variant& v, datxio::chain::name& check);
} // fc


FC_REFLECT( datxio::chain::name, (value) )
