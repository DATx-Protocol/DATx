...
namespace datxio { namespace chain { namespace config {
...
const static uint64_t system_account_name    = N(datxio);
const static uint64_t null_account_name      = N(datxio.null);
const static uint64_t producers_account_name = N(datxio.prods);
...
const static uint64_t datxio_auth_scope       = N(datxio.auth);
const static uint64_t datxio_all_scope        = N(datxio.all);

const static uint64_t active_name = N(active);
const static uint64_t owner_name  = N(owner);
const static uint64_t datxio_any_name = N(datxio.any);
const static uint64_t datxio_code_name = N(datxio.code);
...
const static datxio::chain::wasm_interface::vm_type default_wasm_runtime = datxio::chain::wasm_interface::vm_type::binaryen;
...
} } } // namespace datxio::chain::config

constexpr uint64_t DATX_PERCENT(uint64_t value, uint32_t percentage) {
   return (value * percentage) / datxio::chain::config::percent_100;
}

template<typename Number>
Number DATX_PERCENT_CEIL(Number value, uint32_t percentage) {
   return ((value * percentage) + datxio::chain::config::percent_100 - datxio::chain::config::percent_1)  / datxio::chain::config::percent_100;
}
