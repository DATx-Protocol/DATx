...
namespace datxio { namespace chain { namespace webassembly { namespace wavm {
...
class wavm_runtime : public datxio::chain::wasm_runtime_interface {
   public:
      wavm_runtime();
      ~wavm_runtime();
...
#define _REGISTER_WAVM_INTRINSIC(CLS, MOD, METHOD, WASM_SIG, NAME, SIG)\
   static Intrinsics::Function _INTRINSIC_NAME(__intrinsic_fn, __COUNTER__) (\
      MOD "." NAME,\
      datxio::chain::webassembly::wavm::wasm_function_type_provider<WASM_SIG>::type(),\
      (void *)datxio::chain::webassembly::wavm::intrinsic_function_invoker_wrapper<WASM_SIG, SIG>::type::fn<&CLS::METHOD>()\
   );\
