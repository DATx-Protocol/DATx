...
#include <datxio/chain/transaction.hpp>
#include <datxio/chain/exceptions.hpp>

#include <datxio/chain/account_object.hpp>
#include <datxio/chain/permission_object.hpp>
#include <datxio/chain/permission_link_object.hpp>
...
#include <datxio/chain/abi_serializer.hpp>
...
void validate_authority_precondition( const apply_context& context, const authority& auth ) {
   for(const auto& a : auth.accounts) {
      ...
      if( a.permission.permission == config::datxio_code_name ) // virtual datxio.code permission does not really exist but is allowed
         continue;

      try {
         context.control.get_authorization_manager().get_permission({a.permission.actor, a.permission.permission});
      } catch( const permission_query_exception& ) {
         DATX_THROW( action_validate_exception,
 ...
 */
void apply_datxio_newaccount(apply_context& context) {
   auto create = context.act.data_as<newaccount>();
   try {
   context.require_authorization(create.creator);
//   context.require_write_lock( config::datxio_auth_scope );
   auto& authorization = context.control.get_mutable_authorization_manager();

   DATX_ASSERT( validate(create.owner), action_validate_exception, "Invalid owner authority");
   DATX_ASSERT( validate(create.active), action_validate_exception, "Invalid active authority");

   auto& db = context.db;

   auto name_str = name(create.name).to_string();

   DATX_ASSERT( !create.name.empty(), action_validate_exception, "account name cannot be empty" );
   DATX_ASSERT( name_str.size() <= 12, action_validate_exception, "account names can only be 12 chars long" );

   // Check if the creator is privileged
   const auto &creator = db.get<account_object, by_name>(create.creator);
   if( !creator.privileged ) {
      DATX_ASSERT( name_str.find( "datxio." ) != 0, action_validate_exception,
                  "only privileged accounts can have names that start with 'datxio.'" );
   }

   auto existing_account = db.find<account_object, by_name>(create.name);
   DATX_ASSERT(existing_account == nullptr, account_name_exists_exception,
...
void apply_datxio_setcode(apply_context& context) {
   ...
   DATX_ASSERT( act.vmtype == 0, invalid_contract_vm_type, "code should be 0" );
   DATX_ASSERT( act.vmversion == 0, invalid_contract_vm_version, "version should be 0" );
   ...
   DATX_ASSERT( account.code_version != code_id, set_exact_code, "contract is already running this version of code" );
...
void apply_datxio_setabi(apply_context& context) {
   auto& db  = context.db;
...
void apply_datxio_updateauth(apply_context& context) {
   ...
   DATX_ASSERT(!update.permission.empty(), action_validate_exception, "Cannot create authority with empty name");
   DATX_ASSERT( update.permission.to_string().find( "datxio." ) != 0, action_validate_exception,
               "Permission names that start with 'datxio.' are reserved" );
   DATX_ASSERT(update.permission != update.parent, action_validate_exception, "Cannot set an authority as its own parent");
   db.get<account_object, by_name>(update.account);
   DATX_ASSERT(validate(update.auth), action_validate_exception,
              "Invalid authority: ${auth}", ("auth", update.auth));
   if( update.permission == config::active_name )
      DATX_ASSERT(update.parent == config::owner_name, action_validate_exception, "Cannot change active authority's parent from owner", ("update.parent", update.parent) );
   if (update.permission == config::owner_name)
      DATX_ASSERT(update.parent.empty(), action_validate_exception, "Cannot change owner authority's parent");
   else
      DATX_ASSERT(!update.parent.empty(), action_validate_exception, "Only owner permission can have empty parent" );

   if( update.auth.waits.size() > 0 ) {
      auto max_delay = context.control.get_global_properties().configuration.max_transaction_delay;
      DATX_ASSERT( update.auth.waits.back().wait_sec <= max_delay, action_validate_exception,
   ...
   if( permission ) {
      DATX_ASSERT(parent_id == permission->parent, action_validate_exception,
...
void apply_datxio_deleteauth(apply_context& context) {
//   context.require_write_lock( config::datxio_auth_scope );
   ...
   DATX_ASSERT(remove.permission != config::active_name, action_validate_exception, "Cannot delete active authority");
   DATX_ASSERT(remove.permission != config::owner_name, action_validate_exception, "Cannot delete owner authority");
   ...
   { // Check for links to this permission
      ...
      DATX_ASSERT(range.first == range.second, action_validate_exception,
...
void apply_datxio_linkauth(apply_context& context) {
//   context.require_write_lock( config::datxio_auth_scope );

   auto requirement = context.act.data_as<linkauth>();
   try {
      DATX_ASSERT(!requirement.requirement.empty(), action_validate_exception, "Required permission cannot be empty");
      ...
      DATX_ASSERT(account != nullptr, account_query_exception,
      ...
      DATX_ASSERT(code != nullptr, account_query_exception,
                 "Failed to retrieve code for account: ${account}", ("account", requirement.code));
      if( requirement.requirement != config::datxio_any_name ) {
         const auto *permission = db.find<permission_object, by_name>(requirement.requirement);
         DATX_ASSERT(permission != nullptr, permission_query_exception,
      ...
      if( link ) {
         DATX_ASSERT(link->required_permission != requirement.requirement, action_validate_exception,
...
void apply_datxio_unlinkauth(apply_context& context) {
//   context.require_write_lock( config::datxio_auth_scope );
   ...
   auto link = db.find<permission_link_object, by_action_name>(link_key);
   DATX_ASSERT(link != nullptr, action_validate_exception, "Attempting to unlink authority, but no link found");
   context.trx_context.add_ram_usage(
      link->account,
      -(int64_t)(config::billable_size_v<permission_link_object>)
   );
...
void apply_datxio_canceldelay(apply_context& context) {
