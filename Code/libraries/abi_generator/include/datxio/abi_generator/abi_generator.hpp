...
namespace datxio {
      ...
      FC_EXPAND_MACRO( \
       FC_MULTILINE_MACRO_BEGIN \
         ...
         {                                                                      \
           ...
           FC_THROW_EXCEPTION( datxio::abi_generation_exception, #TEST ": "  __VA_ARGS__ ); \
         }                                                                      \
   ...
   /**
     * @brief Generates datxio::abi_def struct handling events from ASTConsumer
     */
   class abi_generator {
      private:
   ...
   struct find_DATXIO_ABI_macro_action : public PreprocessOnlyAction {

         string& contract;
         vector<string>& actions;
         const string& abi_context;

         find_DATXIO_ABI_macro_action(string& contract, vector<string>& actions, const string& abi_context
            ): contract(contract),
         ...
         struct callback_handler : public PPCallbacks {
            ...
            CompilerInstance& compiler_instance;
            find_DATXIO_ABI_macro_action& act;

            callback_handler(CompilerInstance& compiler_instance, find_DATXIO_ABI_macro_action& act)
            ...
            void MacroExpands (const Token &token, const MacroDefinition &md, SourceRange range, const MacroArgs *args) override {
               ...
               if( id->getName() != "DATXIO_ABI" ) return;
