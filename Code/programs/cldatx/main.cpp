 ...
 *  @defgroup datxclienttool DATXIO Command Line Client Reference
...
```
$ ./cldatx
Command Line Interface to DATXIO Client
...
using namespace datxio::utilities;
using namespace datxio::client::help;
using namespace datxio::client::http;
using namespace datxio::client::localize;
using namespace datxio::client::config;
...
datxio::client::http::http_context context;
...
fc::variant call( const std::string& url,
                  ...
                  const T& v ) {
   try {
      auto sp = std::make_unique<datxio::client::http::connection_param>(context, parse_url(url) + path, no_verify ? false : true, headers);
      return datxio::client::http::do_http_call(*sp, fc::variant(v), print_request, print_response );
   }
...
datxio::chain_apis::read_only::get_info_results get_info() {
   return call(url, get_info_func).as<datxio::chain_apis::read_only::get_info_results>();
}
...
auto abi_serializer_resolver = [](const name& account) -> optional<abi_serializer> {
   ...
   if ( it == abi_cache.end() ) {
      ...
      auto abi_results = result.as<datxio::chain_apis::read_only::get_abi_results>();
...
chain::action create_newaccount(const name& creator, const name& newaccount, public_key_type owner, public_key_type active) {
   return action {
      ...
      datxio::chain::newaccount{
         .creator      = creator,
         .name         = newaccount,
         .owner        = datxio::chain::authority{1, {{owner, 1}}, {}},
         .active       = datxio::chain::authority{1, {{active, 1}}, {}}
      }
   };
}
...
authority parse_json_authority_or_key(const std::string& authorityJsonOrFile) {
   if (boost::istarts_with(authorityJsonOrFile, "DATX") || boost::istarts_with(authorityJsonOrFile, "PUB_R1")) {
      ...
      DATX_ASSERT( datxio::chain::validate(result), authority_type_exception, "Authority failed validation! ensure that keys, accounts, and waits are sorted and that the threshold is valid and satisfiable!");
      return result;
   }
}

asset to_asset( const string& code, const string& s ) {
   static map<datxio::chain::symbol_code, datxio::chain::symbol> cache;
   auto a = asset::from_string( s );
   datxio::chain::symbol_code sym = a.get_symbol().to_symbol_code();
   ...
   if ( it == cache.end() ) {
      ...
      if (obj_it != obj.end()) {
         auto result = obj_it->value().as<datxio::chain_apis::read_only::get_currency_stats_result>();
...
inline asset to_asset( const string& s ) {
   return to_asset( "datxio.token", s );
}
...
struct vote_producers_subcommand {
   string voter_str;
   vector<datxio::name> producer_names;
...
struct approve_producer_subcommand {
   ...
   approve_producer_subcommand(CLI::App* actionRoot) {
      ...
      approve_producer->set_callback([this] {
            ...
            auto res = result.as<datxio::chain_apis::read_only::get_table_rows_result>();
...
struct list_producers_subcommand {
   ...
   list_producers_subcommand(CLI::App* actionRoot) {
      ...
      list_producers->set_callback([this] {
         ...
         auto result = rawResult.as<datxio::chain_apis::read_only::get_producers_result>();
   ...
   bidname_info_subcommand(CLI::App* actionRoot) {
      ...
      list_producers->set_callback([this] {
         auto rawResult = call(get_table_func, fc::mutable_variant_object("json", true)
                               ("code", "datxio")("scope", "datxio")("table", "namebids")
                               ("lower_bound", datxio::chain::string_to_name(newname_str.c_str()))("limit", 1));
         if ( print_json ) {
            std::cout << fc::json::to_pretty_string(rawResult) << std::endl;
            return;
         }
         auto result = rawResult.as<datxio::chain_apis::read_only::get_table_rows_result>();
         if ( result.rows.empty() ) {
...
void get_account( const string& accountName, bool json_format ) {
   ...
   auto res = json.as<datxio::chain_apis::read_only::get_account_results>();
   ...
   if (!json_format) {
      ...
      unordered_map<name, datxio::chain_apis::permission> cache;
      ...
      if( res.refund_request.is_object() ) {
         ...
         if( unstaking > asset( 0, unstaking.get_symbol() ) ) {
            ...
            if( now >= refund_time ) {
               std::cout << " (available to claim now with 'datxio::refund' action)\n";
            } else {
...
int main( int argc, char** argv ) {
   ...
   context = datxio::client::http::create_http_context();

   CLI::App app{"Command Line Interface to DATXIO Client"};
   ...
   version->add_subcommand("client", localized("Retrieve version information of the client"))->set_callback([] {
     std::cout << localized("Build version: ${ver}", ("ver", datxio::client::config::version_str)) << std::endl;
   });
...
//                                ->check(CLI::ExistingFile);
   ...
   propose_action->set_callback([&] {
      ...
      send_actions({chain::action{accountPermissions, "datxio.msig", "propose", variant_to_bin( N(datxio.msig), N(propose), args ) }});
   });
   ...
   review->set_callback([&] {
      auto result = call(get_table_func, fc::mutable_variant_object("json", true)
                         ...
                         ("table", "proposal")
                         ("table_key", "")
                         ("lower_bound", datxio::chain::string_to_name(proposal_name.c_str()))
                         ("upper_bound", "")
                         ("limit", 1)
                         );
      //std::cout << fc::json::to_pretty_string(result) << std::endl;
   ...
   auto approve_or_unapprove = [&](const string& action) {
      ...
      send_actions({chain::action{accountPermissions, "datxio.msig", action, variant_to_bin( N(datxio.msig), action, args ) }});
   };
   ...
   cancel->set_callback([&]() {
      ...
      send_actions({chain::action{accountPermissions, "datxio.msig", "cancel", variant_to_bin( N(datxio.msig), N(cancel), args ) }});
      }
   );
   ...
   exec->set_callback([&] {
      ...
      send_actions({chain::action{accountPermissions, "datxio.msig", "exec", variant_to_bin( N(datxio.msig), N(exec), args ) }});
      }
   );
   ...
   sudo_exec->set_callback([&] {
      ...
      auto accountPermissions = get_account_permissions(tx_permission);
      if( accountPermissions.empty() ) {
         accountPermissions = vector<permission_level>{{executer, config::active_name}, {"datxio.sudo", config::active_name}};
      }
      ...
      send_actions({chain::action{accountPermissions, "datxio.sudo", "exec", variant_to_bin( N(datxio.sudo), N(exec), args ) }});
   });

   // system subcommand
   auto system = app.add_subcommand("system", localized("Send datxio.system contract action to the blockchain."), false);
