...
#include <net/if.h>
#include <datxio/chain/genesis_state.hpp>
...
struct server_name_def {
  ...
  string datxio_home;
  uint16_t instances;
  server_name_def () : ipaddr(), name(), has_bios(false), datxio_home(), instances(1) {}
};
...
launcher_def::initialize (const variables_map &vmap) {
  ...
  char* erd_env_var = getenv ("DATXIO_HOME");
  ...
  if (per_host == 0) {
    ...
    for (size_t i = total_nodes; i > 0; i--) {
      ...
      if (ph_count == 0) {
        ...
        lhost->datxio_home =
          (local_id.contains (lhost->host_name) || servers.default_datxio_home.empty()) ?
          erd : servers.default_datxio_home;
        host_ndx++;
      } // ph_count == 0
...
launcher_def::deploy_config_files (tn_node_def &node) {
  ...
  if (host->is_local()) {
    bfs::path cfgdir = bfs::path(host->datxio_home) / instance.config_dir_name;
    bfs::path dd = bfs::path(host->datxio_home) / instance.data_dir_name;
    ...
    bfs::path rfile = bfs::path (host->datxio_home) / instance.config_dir_name / "config.ini";
    ...
    rfile = bfs::path (host->datxio_home) / instance.config_dir_name / "logging.json";
    ...
    rfile = bfs::path (host->datxio_home) / instance.config_dir_name / "genesis.json";
...
launcher_def::write_config_file (tn_node_def &node) {
  ...
  if (instance.has_db || node.producers.size()) {
    ...
    cfg << "plugin = datxio::producer_plugin\n";
  }
  if( instance.has_db ) {
...
launcher_def::init_genesis () {
  bfs::path genesis_path = bfs::current_path() / "genesis.json";
   ...
   if (!src.good()) {
      ...
      datxio::chain::genesis_state default_genesis;
...
launcher_def::prep_remote_config_dir (datxd_def &node, host_def *host) {
  bfs::path abs_config_dir = bfs::path(host->datxio_home) / node.config_dir_name;
  bfs::path abs_data_dir = bfs::path(host->datxio_home) / node.data_dir_name;

  string acd = abs_config_dir.string();
  string add = abs_data_dir.string();
  string cmd = "cd " + host->datxio_home;

  cmd = "cd " + host->datxio_home;
  if (!do_ssh(cmd, host->host_name)) {
    cerr << "Unable to switch to path " << host->datxio_home
...
launcher_def::launch (datxd_def &instance, string &gts) {
  ...
  if (!datxd_extra_args.empty()) {
    if (instance.name == "bios") {
       ...
       regex r("--plugin +datxio::mongo_db_plugin");
  ...
  if (!host->is_local()) {
    string cmdl ("cd ");
    cmdl += host->datxio_home + "; nohup " + datxdcmd + " > "
    ...
    string cmd = "cd " + host->datxio_home + "; kill -15 $(cat " + pidf.string() + ")";
...
launcher_def::bounce (const string& node_numbers) {
   ...
   for (auto node_pair: node_list) {
      ...
      string cmd = "cd " + host.datxio_home + "; "
                 + "export DATXIO_HOME=" + host.datxio_home + string("; ")
                 + "export DATXIO_NODE=" + node_num + "; "
                 + "./scripts/datxio-tn_bounce.sh " + datxd_extra_args;
...
launcher_def::down (const string& node_numbers) {
   ...
   for (auto node_pair: node_list) {
      ...
      string node_num = node.name.substr( node.name.length() - 2 );
      string cmd = "cd " + host.datxio_home + "; "
                 + "export DATXIO_HOME=" + host.datxio_home + "; "
                 + "export DATXIO_NODE=" + node_num + "; "
         + "export DATXIO_TN_RESTART_CONFIG_DIR=" + node.config_dir_name + "; "
                 + "./scripts/datxio-tn_down.sh";
...
int main (int argc, char *argv[]) {
  ...
  cli.add_options()
    ...
    ("down", bpo::value<string>(&down_nodes),"comma-separated list of node numbers that will be taken down using the datxio-tn_down.sh script")
    ("bounce", bpo::value<string>(&bounce_nodes),"comma-separated list of node numbers that will be restarted using the datxio-tn_bounce.sh script")
    ("roll", bpo::value<string>(&roll_nodes),"comma-separated list of host names where the nodes should be rolled to a new version using the datxio-tn_roll.sh script")
  ...
  try {
    ...
    if (vmap.count("version") > 0) {
      cout << datxio::launcher::config::version_str << endl;
      return 0;
    }
...
FC_REFLECT( host_def,
            (genesis)(ssh_identity)(ssh_args)(datxio_home)
...
FC_REFLECT( server_name_def, (ipaddr) (name) (has_bios) (datxio_home) (instances) )

FC_REFLECT( server_identities, (producer) (nonprod) (db) (default_datxio_home) (ssh) )
