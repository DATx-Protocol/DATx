	...
	if [ "${OS_MIN}" -lt 12 ]; then
		echo "You must be running Mac OS 10.12.x or higher to install DATXIO."
		echo "Exiting now."
		exit 1
	fi

	if [ "${DISK_AVAIL}" -lt "$DISK_MIN" ]; then
		echo "You must have at least ${DISK_MIN}GB of available storage to install DATXIO."
		echo "Exiting now."
		exit 1
	fi
	...
	then
		printf "\\tHomebrew must be installed to compile DATX.IO\\n\\n"
	...
	do
		...
		(( COUNT++ ))
	done < "${SOURCE_DIR}/scripts/datxio_build_dep"
	IFS="${var_ifs}"
	...
	if [ "${BVERSION}" != "106700" ]; then
		if [ ! -z "${BVERSION}" ]; then
			...
			printf "\\tDATX.IO requires Boost version 1.67.\\n"
