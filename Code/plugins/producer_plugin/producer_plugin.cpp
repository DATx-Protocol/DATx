...
class producer_plugin_impl : public std::enable_shared_from_this<producer_plugin_impl> {
   public:
      ...
      uint32_t _production_skip_flags              = 0; //datxio::chain::skip_nothing;
   ...
   //                  ilog( "${n} confirmed", ("n",name(producer)) );
         ...
         chain::controller& chain = app().get_plugin<core_plugin>().chain();
      ...
      void on_incoming_block(const signed_block_ptr& block) {
         ...
         try {
            ...
            app().get_plugin<core_plugin>().handle_guard_exception(e);
            return;
         } catch( const fc::exception& e ) {
...
void new_chain_banner(const datxio::chain::controller& db)
{
   std::cerr << "\n"
      "*******************************\n"
      "*                             *\n"
   ...
   boost::program_options::options_description& config_file_options)
{
   ...
   producer_options.add_options()
         ...
         ("signature-provider", boost::program_options::value<vector<string>>()->composing()->multitoken()->default_value({std::string(default_priv_key.get_public_key()) + "=KEY:" + std::string(default_priv_key)}, std::string(default_priv_key.get_public_key()) + "=KEY:" + std::string(default_priv_key)),
          ...
          "Where:\n"
          "   <public-key>    \tis a string form of a vaild DATXIO public key\n\n"
          "   <provider-spec> \tis a string in the form <provider-type>:<data>\n\n"
          "   <provider-type> \tis KEY, or KDATXD\n\n"
          "   KEY:<data>      \tis a string form of a valid DATXIO private key which maps to the provided public key\n\n"
...
static producer_plugin_impl::signature_provider_type
make_kdatxd_signature_provider(const std::shared_ptr<producer_plugin_impl>& impl, const string& url_str, const public_key_type pubkey) {
   ...
   return [weak_impl, kdatxd_url, pubkey]( const chain::digest_type& digest ) {
      ...
      if (impl) {
         ...
         return app().get_plugin<http_client_plugin>().get_client().post_sync(kdatxd_url, params, deadline).as<chain::signature_type>();
      } else {
   ...
   if( options.count("signature-provider") ) {
      ...
      for (const auto& key_spec_pair : key_spec_pairs) {
         try {
            ...
            DATX_ASSERT(delim != std::string::npos, plugin_config_exception, "Missing \"=\" in the key spec pair");
            auto pub_key_str = key_spec_pair.substr(0, delim);
            auto spec_str = key_spec_pair.substr(delim + 1);

            auto spec_delim = spec_str.find(":");
            DATX_ASSERT(spec_delim != std::string::npos, plugin_config_exception, "Missing \":\" in the key spec pair");
...
{ try {
   ...
   chain::controller& chain = app().get_plugin<core_plugin>().chain();
   DATX_ASSERT( my->_producers.empty() || chain.get_read_mode() == chain::db_read_mode::SPECULATIVE, plugin_config_exception,
              "node cannot have any producer-name configured because block production is impossible when read_mode is not \"speculative\"" );

   DATX_ASSERT( my->_producers.empty() || chain.get_validation_mode() == chain::validation_mode::FULL, plugin_config_exception,
   ...
   if (!my->_producers.empty()) {
      ...
      if (my->_production_enabled) {
         ...
         //_production_skip_flags |= datxio::chain::skip_undo_history_check;
      }
   }
...
fc::time_point producer_plugin_impl::calculate_pending_block_time() const {
   const chain::controller& chain = app().get_plugin<core_plugin>().chain();
...
void producer_plugin_impl::produce_block() {
   ...
   const auto& pbs = chain.pending_block_state();
   const auto& hbs = chain.head_block_state();
   DATX_ASSERT(pbs, missing_pending_block_state, "pending_block_state does not exist but it should, another plugin may have corrupted it");
