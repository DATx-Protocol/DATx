...
#include <datxio/bp2p_plugin/bp2p_plugin.hpp>
...
namespace datxio {
   ...
   static appbase::abstract_plugin& _bp2p_plugin = app().register_plugin<bp2p_plugin>();
...
const fc::string logger_name("bp2p_plugin");
...
using datxio::public_key_type;
using datxio::chain_id_type;
using datxio::block_id_type;
using datxio::block_timestamp_type;
using std::string;
using datxio::sha256;
using datxio::signed_block_ptr;
using datxio::packed_transaction_ptr;
using std::vector;
...
namespace datxio {
     ...
     public:
       ...
       //    shared_ptr< vector<char> >  block_msg; ///< packed bnet_message for this block
        ...
        :_resolver(socket.get_io_service()),
         _p2p_net_plugin( std::move(net_plug) ),
        ...
        void async_get_pending_block_ids( L&& callback ) {
           ...
           _app_ios.post( [self = shared_from_this(),callback]{
              auto& control = app().get_plugin<core_plugin>().chain();
        ...
        void on( const packed_transaction_ptr& p ) {
           ...
           if( app().get_plugin<core_plugin>().chain().get_read_mode() == chain::db_read_mode::READ_ONLY )
              return;
  ...
  class listener : public std::enable_shared_from_this<listener> {
     ...
     public:
        ...
        :_acceptor(ioc), _socket(ioc), _p2p_net_plugin(std::move(np))
        {
           boost::system::error_code ec;
        ...
        void run() {
           DATX_ASSERT( _acceptor.is_open(), plugin_exception, "unable top open listen socket" );
           do_accept();
        }
   ...
   class bp2p_plugin_impl : public std::enable_shared_from_this<bp2p_plugin_impl> {
      public:
         bp2p_plugin_impl() = default;
         ...
         std::unique_ptr<boost::asio::io_context>               _ioc; // lifetime guarded by shared_ptr of bp2p_plugin_impl
   ...
   void listener::on_accept( boost::system::error_code ec ) {
     ...
     try {
        newsession = std::make_shared<session>( move( _socket ), _p2p_net_plugin );
     }
     ...
     if( newsession ) {
        _p2p_net_plugin->async_add_session( newsession );
        newsession->_local_peer_id = _p2p_net_plugin->_peer_id;
        newsession->run();
     }
     do_accept();
   }
   ...
   bp2p_plugin::bp2p_plugin()
   :my(std::make_shared<bp2p_plugin_impl>()) {
   }

   bp2p_plugin::~bp2p_plugin() {
   }

   void bp2p_plugin::set_program_options(options_description& cli, options_description& cfg) {
      cfg.add_options()
   ...
   void bp2p_plugin::plugin_initialize(const variables_map& options) {
   ...
   void bp2p_plugin::plugin_startup() {
   ...
   void bp2p_plugin::plugin_shutdown() {
      try {
         my->_timer->cancel();
         my->_timer.reset();
      } catch ( ... ) {
      ...
      my->for_each_session([](auto ses){
         DATX_ASSERT( false, plugin_exception, "session ${ses} still active", ("ses", ses->_session_num) );
      });

      // lifetime of _ioc is guarded by shared_ptr of bp2p_plugin_impl
   }


   session::~session() {
     wlog( "close session ${n}",("n",_session_num) );
     std::weak_ptr<bp2p_plugin_impl> netp = _p2p_net_plugin;
   ...
   void session::do_hello() {
      ...
      async_get_pending_block_ids( [self = shared_from_this() ]( const vector<block_id_type>& ids, uint32_t lib ){
          ...
          hello_msg.request_transactions = self->_p2p_net_plugin->_request_trx;
          hello_msg.chain_id = app().get_plugin<core_plugin>().get_chain_id(); // TODO: Quick fix in a rush. Maybe a better solution is needed.

          self->_local_lib = lib;
          if ( self->_p2p_net_plugin->_follow_irreversible ) {
   ...
   void session::check_for_redundant_connection() {
     app().get_io_service().post( [self=shared_from_this()]{
       self->_p2p_net_plugin->for_each_session( [self]( auto ses ){
   ...
   void session::on( const hello& hi, fc::datastream<const char*>& ds ) {
      ...
      if( hi.chain_id != app().get_plugin<core_plugin>().get_chain_id() ) { // TODO: Quick fix in a rush. Maybe a better solution is needed.
      ...
      if ( _p2p_net_plugin->_follow_irreversible && hi.protocol_version <= "1.0.0") {
