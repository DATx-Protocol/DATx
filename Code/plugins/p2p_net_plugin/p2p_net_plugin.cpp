...
#include <datxio/chain/plugin_interface.hpp>
...
using namespace datxio::chain::plugin_interface::compat;
...
namespace datxio {
   static appbase::abstract_plugin& _p2p_net_plugin = app().register_plugin<p2p_net_plugin>();
   ...
   using datxio::chain::transaction_id_type;
   ...
   const fc::string logger_name("p2p_net_plugin_impl");
   fc::logger logger;
...
#define peer_elog( PEER, FORMAT, ... ) \
  ...
  FC_MULTILINE_MACRO_END
   ...
   static p2p_net_plugin_impl *my_impl;
   ...
   struct update_request_time {
      ...
      void operator () (struct datxio::peer_block_state &bs) {
         bs.requested_time = time_point::now();
      }
   } set_request_time;

   typedef multi_index_container<
      datxio::peer_block_state,
      indexed_by<
         ordered_unique< tag<by_id>, member<datxio::peer_block_state, block_id_type, &datxio::peer_block_state::id > >,
         ordered_unique< tag<by_block_num>, member<datxio::peer_block_state, uint32_t, &datxio::peer_block_state::block_num > >
         >
      > peer_block_state_index;


   struct update_known_by_peer {
      void operator() (datxio::peer_block_state& bs) {
         bs.is_known = true;
      }
      void operator() (transaction_state& ts) {
   ...
   public:
       ...
       */
      bool process_next_message(p2p_net_plugin_impl& impl, uint32_t message_length);
   ...
   struct msgHandler : public fc::visitor<void> {
      p2p_net_plugin_impl &impl;
      connection_ptr c;
      msgHandler( p2p_net_plugin_impl &imp, connection_ptr conn) : impl(imp), c(conn) {}
   ...
   bool connection::enqueue_sync_block() {
      controller& cc = app().find_plugin<core_plugin>()->chain();
      if (!peer_requested)
         return false;
   ...
   bool connection::process_next_message(p2p_net_plugin_impl& impl, uint32_t message_length) {
      try {
   ...
   //-----------------------------------------------------------
    ...
    sync_manager::sync_manager( uint32_t req_span )
      ...
      ,state(in_sync)
   {
      chain_plug = app( ).find_plugin<core_plugin>( );
   ...
   void p2p_net_plugin_impl::connect( connection_ptr c ) {
   ...
   void p2p_net_plugin_impl::connect( connection_ptr c, tcp::resolver::iterator endpoint_itr ) {
      if( c->no_retry != go_away_reason::no_reason) {
   ...
   bool p2p_net_plugin_impl::start_session( connection_ptr con ) {
   ...
   void p2p_net_plugin_impl::start_listen_loop( ) {
   ...
   void p2p_net_plugin_impl::start_read_message( connection_ptr conn ) {
            ...
            [this,weak_conn]( boost::system::error_code ec, std::size_t bytes_transferred ) {
               ...
               try {
                  if( !ec ) {
                     ...
                     DATX_ASSERT(bytes_transferred <= conn->pending_message_buffer.bytes_to_write(), plugin_exception, "");
   ...
   size_t p2p_net_plugin_impl::count_open_sockets() const
   {
      size_t count = 0;
   ...
   template<typename VerifierFunc>
   void p2p_net_plugin_impl::send_all( const net_message &msg, VerifierFunc verify) {
      for( auto &c : connections) {
   ...
   bool p2p_net_plugin_impl::is_valid( const handshake_message &msg) {
   ...
   void p2p_net_plugin_impl::handle_message( connection_ptr c, const chain_size_message &msg) {
      peer_ilog(c, "received chain_size_message");

   }

   void p2p_net_plugin_impl::handle_message( connection_ptr c, const handshake_message &msg) {
   ...
   void p2p_net_plugin_impl::handle_message( connection_ptr c, const go_away_message &msg ) {
   ...
   void p2p_net_plugin_impl::handle_message(connection_ptr c, const time_message &msg) {
   ...
   void p2p_net_plugin_impl::handle_message( connection_ptr c, const notice_message &msg) {
   ...
   void p2p_net_plugin_impl::handle_message( connection_ptr c, const request_message &msg) {
   ...
   void p2p_net_plugin_impl::handle_message( connection_ptr c, const sync_request_message &msg) {
      if( msg.end_block == 0) {
   ...
   void p2p_net_plugin_impl::handle_message( connection_ptr c, const packed_transaction &msg) {
      fc_dlog(logger, "got a packed transaction, cancel wait");
      peer_ilog(c, "received packed_transaction");
      controller& cc = my_impl->chain_plug->chain();
      if( cc.get_read_mode() == datxio::db_read_mode::READ_ONLY ) {
   ...
   void p2p_net_plugin_impl::handle_message( connection_ptr c, const signed_block &msg) {
      controller &cc = chain_plug->chain();
   ...
   void p2p_net_plugin_impl::start_conn_timer(boost::asio::steady_timer::duration du, std::weak_ptr<connection> from_connection) {
   ...
   void p2p_net_plugin_impl::start_txn_timer() {
   ...
   void p2p_net_plugin_impl::ticker() {
   ...
   void p2p_net_plugin_impl::start_monitors() {
   ...
   void p2p_net_plugin_impl::expire_txns() {
      start_txn_timer( );
   ...
   void p2p_net_plugin_impl::connection_monitor(std::weak_ptr<connection> from_connection) {
   ...
   void p2p_net_plugin_impl::close( connection_ptr c ) {
   ...
   void p2p_net_plugin_impl::accepted_block_header(const block_state_ptr& block) {
      fc_dlog(logger,"signaled, id = ${id}",("id", block->id));
   }

   void p2p_net_plugin_impl::accepted_block(const block_state_ptr& block) {
      fc_dlog(logger,"signaled, id = ${id}",("id", block->id));
      dispatcher->bcast_block(*block->block);
   }

   void p2p_net_plugin_impl::irreversible_block(const block_state_ptr&block) {
      fc_dlog(logger,"signaled, id = ${id}",("id", block->id));
   }

   void p2p_net_plugin_impl::accepted_transaction(const transaction_metadata_ptr& md) {
      fc_dlog(logger,"signaled, id = ${id}",("id", md->id));
//      dispatcher->bcast_transaction(md->packed_trx);
   }

   void p2p_net_plugin_impl::applied_transaction(const transaction_trace_ptr& txn) {
      fc_dlog(logger,"signaled, id = ${id}",("id", txn->id));
   }

   void p2p_net_plugin_impl::accepted_confirmation(const header_confirmation& head) {
      fc_dlog(logger,"signaled, id = ${id}",("id", head.block_id));
   }

   void p2p_net_plugin_impl::transaction_ack(const std::pair<fc::exception_ptr, packed_transaction_ptr>& results) {
   ...
   bool p2p_net_plugin_impl::authenticate_peer(const handshake_message& msg) const {
   ...
   chain::public_key_type p2p_net_plugin_impl::get_authentication_key() const {
   ...
   chain::signature_type p2p_net_plugin_impl::sign_compact(const chain::public_key_type& signer, const fc::sha256& digest) const
   {
   ...
   p2p_net_plugin::p2p_net_plugin()
      :my( new p2p_net_plugin_impl ) {
      my_impl = my.get();
   }
   ...
   void p2p_net_plugin::set_program_options( options_description& /*cli*/, options_description& cfg )
   {
      cfg.add_options()
   ...
   void p2p_net_plugin::plugin_initialize( const variables_map& options ) {
      ...
      try {
         ...
         if( options.count( "allowed-connection" )) {
            ...
            for( const std::string& allowed_remote : allowed_remotes ) {
               if( allowed_remote == "any" )
                  my->allowed_connections |= p2p_net_plugin_impl::Any;
               else if( allowed_remote == "producers" )
                  my->allowed_connections |= p2p_net_plugin_impl::Producers;
               else if( allowed_remote == "specified" )
                  my->allowed_connections |= p2p_net_plugin_impl::Specified;
               else if( allowed_remote == "none" )
                  my->allowed_connections = p2p_net_plugin_impl::None;
            }
         }

         if( my->allowed_connections & p2p_net_plugin_impl::Specified )
         ...
         my->chain_id = app().get_plugin<core_plugin>().get_chain_id();
   ...
   void p2p_net_plugin::plugin_startup() {
      if( my->acceptor ) {
         ...
         try {
           ...
           ilog("p2p_net_plugin::plugin_startup failed to bind to port ${port}",
      ...
      {
         cc.accepted_block_header.connect( boost::bind(&p2p_net_plugin_impl::accepted_block_header, my.get(), _1));
         cc.accepted_block.connect(  boost::bind(&p2p_net_plugin_impl::accepted_block, my.get(), _1));
         cc.irreversible_block.connect( boost::bind(&p2p_net_plugin_impl::irreversible_block, my.get(), _1));
         cc.accepted_transaction.connect( boost::bind(&p2p_net_plugin_impl::accepted_transaction, my.get(), _1));
         cc.applied_transaction.connect( boost::bind(&p2p_net_plugin_impl::applied_transaction, my.get(), _1));
         cc.accepted_confirmation.connect( boost::bind(&p2p_net_plugin_impl::accepted_confirmation, my.get(), _1));
      }

      my->incoming_transaction_ack_subscription = app().get_channel<channels::transaction_ack>().subscribe(boost::bind(&p2p_net_plugin_impl::transaction_ack, my.get(), _1));
   ...
   void p2p_net_plugin::plugin_shutdown() {
      try {
         ilog( "shutdown.." );
         my->done = true;
         if( my->acceptor ) {
   ...
   size_t p2p_net_plugin::num_peers() const {
    ...
    */
   string p2p_net_plugin::connect( const string& host ) {
   ...
   string p2p_net_plugin::disconnect( const string& host ) {
   ...
   optional<connection_status> p2p_net_plugin::status( const string& host )const {
   ...
   vector<connection_status> p2p_net_plugin::connections()const {
   ...
   connection_ptr p2p_net_plugin_impl::find_connection( string host )const {
      for( const auto& c : connections )
         if( c->peer_addr == host ) return c;
      return connection_ptr();
   }

   uint16_t p2p_net_plugin_impl::to_protocol_version (uint16_t v) {
