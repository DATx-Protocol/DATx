 ...
 */
#include <datxio/mongo_db_plugin/mongo_db_plugin.hpp>
         ...
         if(account) {
            ...
            if( view.find( "abi" ) != view.end()) {
               ...
               if( n == chain::config::system_account_name ) {
                  // redefine datxio setabi.abi from bytes to abi_def
...
void mongo_db_plugin::set_program_options(options_description& cli, options_description& cfg)
{
   cfg.add_options()
         ...
         "The target queue size between noddatx and MongoDB plugin thread.")
         ...
         "MongoDB URI connection string, see: https://docs.mongodb.com/master/reference/connection-string/."
               " If not specified then plugin is disabled. Default database 'DATX' is used if not specified in URI."
      ...
      if( options.count( "mongodb-uri" )) {
         ...
         if( options.at( "replay-blockchain" ).as<bool>() || options.at( "hard-replay-blockchain" ).as<bool>() || options.at( "delete-all-blocks" ).as<bool>() ) {
            if( options.at( "mongodb-wipe" ).as<bool>()) {
               ...
               DATX_ASSERT( false, chain::plugin_config_exception, "--mongodb-wipe required with --replay-blockchain, --hard-replay-blockchain, or --delete-all-blocks"
         ...
         if( options.count( "mongodb-abi-cache-size" )) {
            ...
            DATX_ASSERT( my->abi_cache_size > 0, chain::plugin_config_exception, "mongodb-abi-cache-size > 0 required" );
         }
         ...
         wlog( "datxio::mongo_db_plugin configured, but no --mongodb-uri specified." );
