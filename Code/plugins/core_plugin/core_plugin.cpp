...
#include <datxio/chain/fork_database.hpp>
#include <datxio/chain/block_log.hpp>
#include <datxio/chain/exceptions.hpp>
#include <datxio/chain/authorization_manager.hpp>
#include <datxio/chain/producer_object.hpp>
#include <datxio/chain/config.hpp>
#include <datxio/chain/wasm_interface.hpp>
#include <datxio/chain/resource_limits.hpp>
#include <datxio/chain/reversible_block_object.hpp>
#include <datxio/chain/controller.hpp>
#include <datxio/chain/generated_transaction_object.hpp>

#include <datxio/chain/datxio_contract.hpp>
...
std::ostream& operator<<(std::ostream& osm, datxio::chain::db_read_mode m) {
   if ( m == datxio::chain::db_read_mode::SPECULATIVE ) {
      osm << "speculative";
   } else if ( m == datxio::chain::db_read_mode::HEAD ) {
      osm << "head";
   } else if ( m == datxio::chain::db_read_mode::READ_ONLY ) {
      osm << "read-only";
   } else if ( m == datxio::chain::db_read_mode::IRREVERSIBLE ) {
      osm << "irreversible";
   }
...
void validate(boost::any& v,
              ...
              datxio::chain::db_read_mode* /* target_type */,
              int)
{
  ...
  if ( s == "speculative" ) {
     v = boost::any(datxio::chain::db_read_mode::SPECULATIVE);
  } else if ( s == "head" ) {
     v = boost::any(datxio::chain::db_read_mode::HEAD);
  } else if ( s == "read-only" ) {
     v = boost::any(datxio::chain::db_read_mode::READ_ONLY);
  } else if ( s == "irreversible" ) {
     v = boost::any(datxio::chain::db_read_mode::IRREVERSIBLE);
  } else {
     throw validation_error(validation_error::invalid_option_value);
  }
}

std::ostream& operator<<(std::ostream& osm, datxio::chain::validation_mode m) {
   if ( m == datxio::chain::validation_mode::FULL ) {
      osm << "full";
   } else if ( m == datxio::chain::validation_mode::LIGHT ) {
      osm << "light";
   }
...
void validate(boost::any& v,
              const std::vector<std::string>& values,
              datxio::chain::validation_mode* /* target_type */,
              int)
{
  using namespace boost::program_options;
  ...
  if ( s == "full" ) {
     v = boost::any(datxio::chain::validation_mode::FULL);
  } else if ( s == "light" ) {
     v = boost::any(datxio::chain::validation_mode::LIGHT);
  } else {
     throw validation_error(validation_error::invalid_option_value);
  }
}
...
using namespace datxio::chain::config;
using namespace datxio::chain::plugin_interface;
...
:my(new core_plugin_impl()) {
}

core_plugin::~core_plugin(){}

void core_plugin::set_program_options(options_description& cli, options_description& cfg)
{
   cfg.add_options()
         ("blocks-dir", bpo::value<bfs::path>()->default_value("blocks"),
          "the location of the blocks directory (absolute path or relative to application data dir)")
         ("checkpoint", bpo::value<vector<string>>()->composing(), "Pairs of [BLOCK_NUM,BLOCK_ID] that should be enforced as checkpoints.")
         ("wasm-runtime", bpo::value<datxio::chain::wasm_interface::vm_type>()->value_name("wavm/binaryen"), "Override default WASM runtime")
         ...
         ("read-mode", boost::program_options::value<datxio::chain::db_read_mode>()->default_value(datxio::chain::db_read_mode::SPECULATIVE),
         ...
         ("validation-mode", boost::program_options::value<datxio::chain::validation_mode>()->default_value(datxio::chain::validation_mode::FULL),
...
void core_plugin::plugin_initialize(const variables_map& options) {
   ilog("initializing chain plugin");

   try {
      try {
         genesis_state gs; // Check if DATXIO_ROOT_KEY is bad
      } catch ( const fc::exception& ) {
         elog( "DATXIO_ROOT_KEY ('${root_key}') is invalid. Recompile with a valid public key.",
               ("root_key", genesis_state::datxio_root_key));
         throw;
      }
      ...
      if( options.count( "action-blacklist" )) {
         ...
         for( const auto& a : acts ) {
            auto pos = a.find( "::" );
            DATX_ASSERT( pos != std::string::npos, plugin_config_exception, "Invalid entry in action-blacklist: '${a}'", ("a", a));
      ...
      if ( options.count("read-mode") ) {
         ...
         DATX_ASSERT( my->chain_config->read_mode != db_read_mode::IRREVERSIBLE, plugin_config_exception, "irreversible mode not currently supported." );
      }
...
void core_plugin::plugin_startup()
{ try {
   try {
      my->chain->startup();
...
void core_plugin::plugin_shutdown() {
...
void chain_apis::read_write::validate() const {
   DATX_ASSERT( db.get_read_mode() != chain::db_read_mode::READ_ONLY, missing_core_api_plugin_exception, "Not allowed, node in read-only mode" );
}

chain_apis::read_write core_plugin::get_read_write_api() {
   return chain_apis::read_write(chain(), get_abi_serializer_max_time());
}

void core_plugin::accept_block(const signed_block_ptr& block ) {
   my->incoming_block_sync_method(block);
}

void core_plugin::accept_transaction(const chain::packed_transaction& trx, next_function<chain::transaction_trace_ptr> next) {
   my->incoming_transaction_async_method(std::make_shared<packed_transaction>(trx), false, std::forward<decltype(next)>(next));
}

bool core_plugin::block_is_on_preferred_chain(const block_id_type& block_id) {
   auto b = chain().fetch_block_by_number( block_header::num_from_id(block_id) );
   return b && b->id() == block_id;
}

bool core_plugin::recover_reversible_blocks( const fc::path& db_dir, uint32_t cache_size,
...
bool core_plugin::import_reversible_blocks( const fc::path& reversible_dir,
                                             uint32_t cache_size,
...
bool core_plugin::export_reversible_blocks( const fc::path& reversible_dir,
                                             const fc::path& reversible_blocks_file ) {
...
controller::config& core_plugin::chain_config() {
   // will trigger optional assert if called before/after plugin_initialize()
   return *my->chain_config;
}

controller& core_plugin::chain() { return *my->chain; }
const controller& core_plugin::chain() const { return *my->chain; }

chain::chain_id_type core_plugin::get_chain_id()const {
   DATX_ASSERT( my->chain_id.valid(), chain_id_type_exception, "chain ID has not been initialized yet" );
   return *my->chain_id;
}

fc::microseconds core_plugin::get_abi_serializer_max_time() const {
   return my->abi_serializer_max_time_ms;
}

void core_plugin::log_guard_exception(const chain::guard_exception&e ) const {
...
void core_plugin::handle_guard_exception(const chain::guard_exception& e) const {
   log_guard_exception(e);
...
read_only::get_info_results read_only::get_info(const read_only::get_info_params&) const {
   ...
   return {
      datxio::utilities::common::itoh(static_cast<uint32_t>(app().version())),
      db.get_chain_id(),
...
uint64_t convert_to_type(const string& str, const string& desc) {
   ...
   try {
      ...
      try {
         ...
         try {
            auto symb = datxio::chain::symbol::from_string(str);
            value = symb.value();
         } catch( ... ) {
            try {
               value = ( datxio::chain::string_to_symbol( 0, str.c_str() ) >> 8 );
            } catch( ... ) {
...
read_only::get_table_rows_result read_only::get_table_rows( const read_only::get_table_rows_params& p )const {
   const abi_def abi = datxio::chain_apis::get_abi( db, p.code );
...
fc::variant read_only::get_currency_stats( const read_only::get_currency_stats_params& p )const {
   ...
   uint64_t scope = ( datxio::chain::string_to_symbol( 0, boost::algorithm::to_upper_copy(p.symbol).c_str() ) >> 8 );
...
read_only::get_producers_result read_only::get_producers( const read_only::get_producers_params& p ) const {
   const abi_def abi = datxio::chain_apis::get_abi(db, config::system_account_name);
...
read_only::get_account_results read_only::get_account( const get_account_params& params )const {
   ...
   if( abi_serializer::to_abi(code_account.abi, abi) ) {
      ...
      const auto token_code = N(datxio.token);
