...
#include <datxio/chain/block_state.hpp>
#include <datxio/chain/transaction_metadata.hpp>
#include <datxio/chain/trace.hpp>

namespace datxio { namespace chain { namespace plugin_interface {
   ...
   namespace methods {
      using get_block_by_number    = method_decl<core_plugin_interface, signed_block_ptr(uint32_t block_num)>;
      using get_block_by_id        = method_decl<core_plugin_interface, signed_block_ptr(const block_id_type& block_id)>;
      using get_head_block_id      = method_decl<core_plugin_interface, block_id_type ()>;
      using get_lib_block_id       = method_decl<core_plugin_interface, block_id_type ()>;

      using get_last_irreversible_block_number = method_decl<core_plugin_interface, uint32_t ()>;
   }
   ...
   namespace incoming {
      ...
      namespace methods {
         ...
         using block_sync            = method_decl<core_plugin_interface, void(const signed_block_ptr&), first_provider_policy>;
         using transaction_async     = method_decl<core_plugin_interface, void(const packed_transaction_ptr&, bool, next_function<transaction_trace_ptr>), first_provider_policy>;
      }
   }
