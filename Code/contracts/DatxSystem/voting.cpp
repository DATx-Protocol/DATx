...
#include <DatxioLib/privileged.hpp>
#include <DatxioLib/singleton.hpp>
...
namespace datxiosystem {
   ...
   using datxio::bytes;
   using datxio::print;
   using datxio::singleton;
   using datxio::transaction;
   ...
   void system_contract::update_elected_producers( block_timestamp block_time ) {
      ...
      std::vector< std::pair<datxio::producer_key,uint16_t> > top_producers;
      top_producers.reserve(_maxbp);

      for ( auto it = idx.cbegin(); it != idx.cend() && top_producers.size() < _maxbp && 0 < it->total_votes && it->active(); ++it ) {
         top_producers.emplace_back( std::pair<datxio::producer_key,uint16_t>({{it->owner, it->producer_key}, it->location}) );
      }
      ...
      std::vector<datxio::producer_key> producers;
...
//      }//end if time()
   ...
   void system_contract::update_votes( const account_name voter_name, const account_name proxy, const std::vector<account_name>& producers, bool voting ) {
      //validate input
      if ( proxy ) {
         datxio_assert( producers.size() == 0, "cannot vote for producers and proxy at same time" );
         datxio_assert( voter_name != proxy, "cannot proxy to self" );
         require_recipient( proxy );
      } else {
         datxio_assert( producers.size() <= 30, "attempt to vote for too many producers" );
         for( size_t i = 1; i < producers.size(); ++i ) {
            datxio_assert( producers[i-1] < producers[i], "producer votes must be unique and sorted" );
         }
      }

      auto voter = _voters.find(voter_name);
      datxio_assert( voter != _voters.end(), "user must stake before they can vote" ); /// staking creates voter object
      datxio_assert( !proxy || !voter->is_proxy, "account registered as a proxy is not allowed to use a proxy" );
      ...
      if ( voter->last_vote_weight > 0 ) {
         if( voter->proxy ) {
            ...
            datxio_assert( old_proxy != _voters.end(), "old proxy not found" ); //data corruption
      ...
      if( proxy ) {
         ...
         datxio_assert( new_proxy != _voters.end(), "invalid proxy specified" ); //if ( !voting ) { data corruption } else { wrong vote }
         datxio_assert( !voting || new_proxy->is_proxy, "proxy not found" );
      ...
      for( const auto& pd : producer_deltas ) {
         ...
         if( pitr != _producers.end() ) {
            datxio_assert( !voting || pitr->active() || !pd.second.second /* not from new set */, "producer is not currently registered" );
            _producers.modify( pitr, 0, [&]( auto& p ) {
               ...
               //datxio_assert( p.total_votes >= 0, "something bad happened" );
            });
         } else {
            datxio_assert( !pd.second.second /* not from new set */, "producer is not registered" ); //data corruption
         }
      }
   ...
   void system_contract::regproxy( const account_name proxy, bool isproxy ) {
      ...
      if ( pitr != _voters.end() ) {
         datxio_assert( isproxy != pitr->is_proxy, "action has no effect" );
         datxio_assert( !isproxy || !pitr->proxy, "account that uses a proxy is not allowed to become a proxy" );
   ...
   void system_contract::propagate_weight_change( const voter_info& voter ) {
      datxio_assert( voter.proxy == 0 || !voter.is_proxy, "account registered as a proxy is not allowed to use a proxy" );
