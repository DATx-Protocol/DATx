#pragma once
#include <DatxExchange/exchange_state.hpp>
#include <DatxExchange/exchange_accounts.hpp>
...
namespace datxio {
   ...
   struct margin_position {
      ...
      DATXLIB_SERIALIZE( margin_position, (owner)(borrowed)(collateral)(call_price) )
   };

   typedef datxio::multi_index<N(margins), margin_position,
           indexed_by<N(callprice), datxio::const_mem_fun<margin_position, uint64_t, &margin_position::get_call> >
   > margins;
   ...
   struct loan_position {
      ...
      DATXLIB_SERIALIZE( loan_position, (owner)(interest_shares) )
   };

   typedef datxio::multi_index<N(loans), loan_position> loans;
