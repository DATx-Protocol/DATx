...
namespace datxio {
   ...
   void exchange::withdraw( account_name from, extended_asset quantity ) {
      require_auth( from );
      datxio_assert( quantity.is_valid(), "invalid quantity" );
      datxio_assert( quantity.amount >= 0, "cannot withdraw negative balance" ); // Redundant? inline_transfer will fail if quantity is not positive.
   ...
   void exchange::on( const trade& t ) {
      ...
      datxio_assert( t.sell.is_valid(), "invalid sell amount" );
      datxio_assert( t.sell.amount > 0, "sell amount must be positive" );
      datxio_assert( t.min_receive.is_valid(), "invalid min receive amount" );
      datxio_assert( t.min_receive.amount >= 0, "min receive amount cannot be negative" );

      auto receive_symbol = t.min_receive.get_extended_symbol();
      datxio_assert( t.sell.get_extended_symbol() != receive_symbol, "invalid conversion" );
      ...
      if( t.min_receive.amount != 0 ) {
         datxio_assert( t.min_receive.amount <= output.amount, "unable to fill" );
      }
   ...
   void exchange::on( const upmargin& b ) {
      ...
      datxio_assert( b.delta_borrow.is_valid(), "invalid borrow delta" );
      datxio_assert( b.delta_collateral.is_valid(), "invalid collateral delta" );

      market_state market( _this_contract, b.market, _accounts );

      datxio_assert( b.delta_borrow.amount != 0 || b.delta_collateral.amount != 0, "no effect" );
      datxio_assert( b.delta_borrow.get_extended_symbol() != b.delta_collateral.get_extended_symbol(), "invalid args" );
      datxio_assert( market.exstate.base.balance.get_extended_symbol() == b.delta_borrow.get_extended_symbol() ||
                    market.exstate.quote.balance.get_extended_symbol() == b.delta_borrow.get_extended_symbol(),
                    "invalid asset for market" );
      datxio_assert( market.exstate.base.balance.get_extended_symbol() == b.delta_collateral.get_extended_symbol() ||
   ...
   void exchange::on( const covermargin& c ) {
      ...
      datxio_assert( c.cover_amount.is_valid(), "invalid cover amount" );
      datxio_assert( c.cover_amount.amount > 0, "cover amount must be positive" );
   ...
   void exchange::createx( account_name    creator,
      ...
      datxio_assert( initial_supply.is_valid(), "invalid initial supply" );
      datxio_assert( initial_supply.amount > 0, "initial supply must be positive" );
      datxio_assert( base_deposit.is_valid(), "invalid base deposit" );
      datxio_assert( base_deposit.amount > 0, "base deposit must be positive" );
      datxio_assert( quote_deposit.is_valid(), "invalid quote deposit" );
      datxio_assert( quote_deposit.amount > 0, "quote deposit must be positive" );
      datxio_assert( base_deposit.get_extended_symbol() != quote_deposit.get_extended_symbol(),
      ...
      datxio_assert( existing == exstates.end(), "market already exists" );
   ...
   void exchange::lend( account_name lender, symbol_type market, extended_asset quantity ) {
      require_auth( lender );
      datxio_assert( quantity.is_valid(), "invalid quantity" );
      datxio_assert( quantity.amount > 0, "must lend a positive amount" );
   ...
   void exchange::unlend( account_name lender, symbol_type market, double interest_shares, extended_symbol interest_symbol ) {
      require_auth( lender );
      datxio_assert( interest_shares > 0, "must unlend a positive amount" );
   ...
   void exchange::on( const currency::transfer& t, account_name code ) {
      ...
      if( t.to == _this_contract ) {
         ...
         datxio_assert( a.is_valid(), "invalid quantity in transfer" );
         datxio_assert( a.amount != 0, "zero quantity is disallowed in transfer");
         datxio_assert( a.amount > 0 || t.memo == "withdraw", "withdrew tokens without withdraw in memo");
         datxio_assert( a.amount < 0 || t.memo == "deposit", "received tokens without deposit in memo" );
   ...
   void exchange::apply( account_name contract, account_name act ) {
      ...
      switch( act ) {
         DATXIO_API( exchange, (createx)(deposit)(withdraw)(lend)(unlend) )
      };
