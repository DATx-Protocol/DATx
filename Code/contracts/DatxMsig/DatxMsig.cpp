#include <DatxMsig/DatxMsig.hpp>
#include <DatxioLib/action.hpp>
#include <DatxioLib/permission.hpp>
...
void multisig::propose() {
   ...
   datxio_assert( trx_header.expiration >= datxio::time_point_sec(now()), "transaction expired" );
   //datxio_assert( trx_header.actions.size() > 0, "transaction must have at least one action" );

   proposals proptable( _self, proposer );
   datxio_assert( proptable.find( proposal_name ) == proptable.end(), "proposal with the same name exists" );
   ...
   datxio_assert( res > 0, "transaction authorization failed" );
...
void multisig::approve( account_name proposer, name proposal_name, permission_level level ) {
   ...
   datxio_assert( itr != apps.requested_approvals.end(), "approval is not on the list of requested approvals" );
...
void multisig::unapprove( account_name proposer, name proposal_name, permission_level level ) {
   ...
   datxio_assert( itr != apps.provided_approvals.end(), "no approval previously granted" );
...
void multisig::cancel( account_name proposer, name proposal_name, account_name canceler ) {
   ...
   if( canceler != proposer ) {
      datxio_assert( unpack<transaction_header>( prop.packed_transaction ).expiration < datxio::time_point_sec(now()), "cannot cancel until expiration" );
   }
...
DATXIO_ABI( datxio::multisig, (propose)(approve)(unapprove)(cancel)(exec) )
