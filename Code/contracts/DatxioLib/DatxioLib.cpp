...
namespace datxio {
   ...
   void set_blockchain_parameters(const datxio::blockchain_parameters& params) {
      char buf[sizeof(datxio::blockchain_parameters)];
      datxio::datastream<char *> ds( buf, sizeof(buf) );
      ds << params;
      set_blockchain_parameters_packed( buf, ds.tellp() );
   }

   void get_blockchain_parameters(datxio::blockchain_parameters& params) {
      char buf[sizeof(datxio::blockchain_parameters)];
      size_t size = get_blockchain_parameters_packed( buf, sizeof(buf) );
      datxio_assert( size <= sizeof(buf), "buffer is too small" );
      datxio::datastream<const char*> ds( buf, size_t(size) );
      ds >> params;
   }
   ...
   private:
      ...
      public:
         ...
         {
            datxio_assert(_offset == _heap_size, "malloc_from_freed was designed to only be called after _heap was completely allocated");
...
{
   return datxio::memory_heap.malloc(size);
}

void* calloc(size_t count, size_t size)
{
   void* ptr = datxio::memory_heap.malloc(count*size);
...
{
   return datxio::memory_heap.realloc(ptr, size);
}

void free(void* ptr)
{
   return datxio::memory_heap.free(ptr);
}
