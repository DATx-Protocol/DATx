...
#include <DatxioLib/system.h>
#include <DatxioLib/symbol.hpp>
#include <tuple>
#include <limits>
...
namespace datxio {
   ...
   */
   ...
   struct asset {
      ...
      :amount(a),symbol{s}
      {
         datxio_assert( is_amount_within_range(), "magnitude of asset amount must be less than 2^62" );
         datxio_assert( symbol.is_valid(),        "invalid symbol name" );
      }
      ...
      asset& operator-=( const asset& a ) {
         datxio_assert( a.symbol == symbol, "attempt to subtract asset with different symbol" );
         amount -= a.amount;
         datxio_assert( -max_amount <= amount, "subtraction underflow" );
         datxio_assert( amount <= max_amount,  "subtraction overflow" );
         return *this;
      }
       ...
       */
      asset& operator+=( const asset& a ) {
         datxio_assert( a.symbol == symbol, "attempt to add asset with different symbol" );
         amount += a.amount;
         datxio_assert( -max_amount <= amount, "addition underflow" );
         datxio_assert( amount <= max_amount,  "addition overflow" );
         return *this;
      }
      ...
      asset& operator*=( int64_t a ) {
         ...
         datxio_assert( tmp <= max_amount, "multiplication overflow" );
         datxio_assert( tmp >= -max_amount, "multiplication underflow" );
         amount = (int64_t)tmp;
         return *this;
      }
       ...
       */
      asset& operator/=( int64_t a ) {
         datxio_assert( a != 0, "divide by zero" );
         datxio_assert( !(amount == std::numeric_limits<int64_t>::min() && a == -1), "signed division overflow" );
         amount /= a;
         return *this;
      }
      ...
      friend int64_t operator/( const asset& a, const asset& b ) {
         datxio_assert( a.symbol == b.symbol, "comparison of assets with different symbols is not allowed" );
      ...
      DATXLIB_SERIALIZE( asset, (amount)(symbol) )
   };
   ...
   */
   struct extended_asset : public asset {
      ...
      friend extended_asset operator - ( const extended_asset& a, const extended_asset& b ) {
         datxio_assert( a.contract == b.contract, "type mismatch" );
      ...
      DATXLIB_SERIALIZE( extended_asset, (amount)(symbol)(contract) )
   };
