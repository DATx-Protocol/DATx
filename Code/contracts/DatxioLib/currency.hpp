...
namespace datxio {
    ...
    *  @deprecated This class is deprecated in favor of datxio.token in Dawn 3.0
    */
   class currency {
      public:
         ...
         typedef datxio::multi_index<N(accounts), account> accounts;
         typedef datxio::multi_index<N(stat), currency_stats> stats;
         ...
         bool apply( account_name contract, action_name act ) {
          ...
          void issue_currency( const issue& i ) {
             ...
             statstable.modify( st, 0, [&]( auto& s ) {
                ...
                datxio_assert( s.supply.amount >= 0, "underflow" );
             });
          ...
          void on( const issue& i ) {
             ...
             datxio_assert( i.quantity.is_valid(), "invalid quantity" );
             datxio_assert( i.quantity.amount > 0, "must issue positive quantity" );
          ...
          void on( const transfer& t ) {
             ...
             datxio_assert( t.quantity.is_valid(), "invalid quantity" );
             datxio_assert( t.quantity.amount > 0, "must transfer positive quantity" );
      ...
      private:
          void sub_balance( account_name owner, asset value, const currency_stats& st ) {
             ...
             if( has_auth( owner ) ) {
                datxio_assert( !st.can_freeze || !from.frozen, "account is frozen by issuer" );
                datxio_assert( !st.can_freeze || !st.is_frozen, "all transfers are frozen by issuer" );
                datxio_assert( !st.enforce_whitelist || from.whitelist, "account is not white listed" );
             } else if( has_auth( st.issuer ) ) {
                datxio_assert( st.can_recall, "issuer may not recall token" );
             } else {
                datxio_assert( false, "insufficient authority" );
             }
          ...
          void add_balance( account_name owner, asset value, const currency_stats& st, account_name ram_payer )
          {
             ...
             if( to == to_acnts.end() ) {
                datxio_assert( !st.enforce_whitelist, "can only transfer to white listed accounts" );
                to_acnts.emplace( ram_payer, [&]( auto& a ){
                  a.balance = value;
                });
             } else {
                datxio_assert( !st.enforce_whitelist || to->whitelist, "receiver requires whitelist by issuer" );
