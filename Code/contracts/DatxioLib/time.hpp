...
namespace datxio {
  ...
  class time_point_sec
  {
    public:
        ...
        operator time_point()const { return time_point( datxio::seconds( utc_seconds) ); }
        uint32_t sec_since_epoch()const { return utc_seconds; }

        time_point_sec operator = ( const datxio::time_point& t )
        {
          utc_seconds = uint32_t(t.time_since_epoch().count() / 1000000ll);
          return *this;
        }
   ...
   class block_timestamp {
      public:
         ...
         block_timestamp next() const {
            datxio_assert( std::numeric_limits<uint32_t>::max() - slot >= 1, "block timestamp overflow" );
