...
#include <DatxioLib/action.h>
#include <DatxioLib/types.hpp>
#include <DatxioLib/serialize.hpp>
#include <DatxioLib/datastream.hpp>
#include <DatxioLib/db.h>
#include <DatxioLib/fixed_key.hpp>
...
/**
 *  The indexed_by struct is used to instantiate the indices for the Multi-Index table. In DATXIO, up to 16 secondary indices can be specified.
 *  @brief The indexed_by struct is used to instantiate the indices for the Multi-Index table. In DATXIO, up to 16 secondary indices can be specified.
 *
 *  @tparam IndexName - is the name of the index. The name must be provided as an DATXIO base32 encoded 64-bit integer and must conform to the DATXIO naming requirements of a maximum of 13 characters, the first twelve from the lowercase characters a-z, digits 0-5, and ".", and if there is a 13th character, it is restricted to lowercase characters a-p and ".".
 *  @tparam Extractor - is a function call operator that takes a const reference to the table object type and returns either a secondary key type or a reference to a secondary key type. It is recommended to use the `datxio::const_mem_fun` template, which is a type alias to the `boost::multi_index::const_mem_fun`. See the documentation for the Boost `const_mem_fun` key extractor for more details.
 *
 *  Example:
       *
*
 *  @code
 *  #include <DatxioLib/datxio.hpp>
 *  using namespace datxio;
 *  class mycontract: datxio::contract {
 *    struct record {
 *       uint64_t    primary;
 *       uint128_t   secondary;
 ...
 *      }
 *  }
 *  DATXIO_ABI( mycontract, (myaction) )
 *  @endcode
 */
 ...
 *  @brief Defines DATXIO Multi Index Table
 *  @ingroup databasecpp
 *
 *
 *
 *  DATXIO Multi-Index API provides a C++ interface to the DATXIO database. It is patterned after Boost Multi Index Container.
 *  DATXIO Multi-Index table requires exactly a uint64_t primary key. For the table to be able to retrieve the primary key,
 *  the object stored inside the table is required to have a const member function called primary_key() that returns uint64_t.
 *  DATXIO Multi-Index table also supports up to 16 secondary indices. The type of the secondary indices could be any of:
 *  - uint64_t
 *  - uint128_t
 *  - uint256_t
 *  - double
...
class multi_index
{
   private:
      ...
      struct index {
         public:
            ...
            struct const_iterator : public std::iterator<std::bidirectional_iterator_tag, const T> {
               public:
                  ...
                  const_iterator& operator++() {
                     ...
                     datxio_assert( _item != nullptr, "cannot increment end iterator" );
                  ...
                  const_iterator& operator--() {
                     ...
                     if( !_item ) {
                        ...
                        datxio_assert( ei != -1, "cannot decrement end iterator when the index is empty" );
                        prev_itr = secondary_index_db_functions<secondary_key_type>::db_idx_previous( ei , &prev_pk );
                        datxio_assert( prev_itr >= 0, "cannot decrement end iterator when the index is empty" );
                     } else {
                        if( _item->__iters[Number] == -1 ) {
                           secondary_key_type temp_secondary_key;
                           auto idxitr = secondary_index_db_functions<secondary_key_type>::db_idx_find_primary(_idx->get_code(), _idx->get_scope(), _idx->name(), _item->primary_key(), temp_secondary_key);
                        ...
                        datxio_assert( prev_itr >= 0, "cannot decrement iterator at beginning of index" );
                     }
            ...
            const_iterator require_find( const secondary_key_type& secondary, const char* error_msg = "unable to find secondary key" )const {
               auto lb = lower_bound( secondary );
               datxio_assert( lb != cend(), error_msg );
               datxio_assert( secondary == secondary_extractor_type()(*lb), error_msg );
               return lb;
            }
            ...
            const T& get( const secondary_key_type& secondary, const char* error_msg = "unable to find secondary key" )const {
               ...
               datxio_assert( result != cend(), error_msg );
               return *result;
            }
            ...
            const_iterator iterator_to( const T& obj ) {
               ...
               datxio_assert( objitem.__idx == _multidx, "object passed to iterator_to is not in multi_index" );
            ...
            void modify( const_iterator itr, uint64_t payer, Lambda&& updater ) {
               datxio_assert( itr != cend(), "cannot pass end iterator to modify" );

               _multidx->modify( *itr, payer, std::forward<Lambda&&>(updater) );
            }

            const_iterator erase( const_iterator itr ) {
               datxio_assert( itr != cend(), "cannot pass end iterator to erase" );
      ...
      const item& load_object_by_primary_iterator( int32_t itr )const {
         ...
         datxio_assert( size >= 0, "error reading iterator" );
   ...
   public:
       ...
       *
       *  Notes
       *  The `datxio::multi_index` template has template parameters `<uint64_t TableName, typename T, typename... Indices>`, where:
       ...
       *  - It is recommended to use the datxio::const_mem_fun template, which is a type alias to the boost::multi_index::const_mem_fun.  See the documentation for the Boost const_mem_fun key extractor for more details.
       *
       *  Example:
       *
       *  @code
       ...
       *      typedef datxio::multi_index< N(address), address > address_index;
       *      void myaction() {
       *        address_index addresses(_self, _self); // code, scope
       *      }
       *  }
       *  DATXIO_ABI( addressbook, (myaction) )
       *  @endcode
       */
       ...
       *        datxio_assert(addresses.get_code() == N(dan), "Codes don't match.");
       *      }
       *  }
       *  DATXIO_ABI( addressbook, (myaction) )
       *  @endcode
       ...
       *      void myaction() {
       *        address_index addresses(N(dan), N(dan)); // code, scope
       *        datxio_assert(addresses.get_code() == N(dan), "Scopes don't match");
       *      }
       *  }
       *  DATXIO_ABI( addressbook, (myaction) )
       *  @endcode
      ...
      struct const_iterator : public std::iterator<std::bidirectional_iterator_tag, const T> {
         ...
         const_iterator& operator--() {
            ...
            if( !_item ) {
               ...
               datxio_assert( ei != -1, "cannot decrement end iterator when the table is empty" );
               prev_itr = db_previous_i64( ei , &prev_pk );
               datxio_assert( prev_itr >= 0, "cannot decrement end iterator when the table is empty" );
            } else {
               prev_itr = db_previous_i64( _item->__primary_itr, &prev_pk );
               datxio_assert( prev_itr >= 0, "cannot decrement iterator at beginning of table" );
            }
       ...
       *        datxio_assert(itr == addresses.cbegin(), "Only address is not at front.");
       *      }
       *  }
       *  DATXIO_ABI( addressbook, (myaction) )
       *  @endcode
       ...
       *        });
       *        auto itr = addresses.find(N(dan));
       *        datxio_assert(itr == addresses.begin(), "Only address is not at front.");
       *      }
       *  }
       *  DATXIO_ABI( addressbook, (myaction) )
       *  @endcode
       ...
       *        });
       *        auto itr = addresses.find(N(dan));
       *        datxio_assert(itr != addresses.cend(), "Address for account doesn't exist");
       *      }
       *  }
       *  DATXIO_ABI( addressbook, (myaction) )
       *  @endcode
       ...
       *        });
       *        auto itr = addresses.find(N(dan));
       *        datxio_assert(itr != addresses.end(), "Address for account doesn't exist");
       *      }
       *  }
       *  DATXIO_ABI( addressbook, (myaction) )
       *  @endcode
       ...
       *        datxio_assert(itr->account_name == N(dan), "Incorrect Last Record ");
       *        itr++;
       *        datxio_assert(itr->account_name == N(brendan), "Incorrect Second Last Record");
       *      }
       *  }
       *  DATXIO_ABI( addressbook, (myaction) )
       *  @endcode
       ...
       *        itr--;
       *        datxio_assert(itr->account_name == N(brendan), "Incorrect First Record ");
       *        itr--;
       *        datxio_assert(itr->account_name == N(dan), "Incorrect Second Record");
       *      }
       *  }
       *  DATXIO_ABI( addressbook, (myaction) )
       *  @endcode
       ...
       *    public:
       *      addressbook(account_name self):contract(self) {}
       *      typedef datxio::multi_index< N(address), address, indexed_by< N(zip), const_mem_fun<address, uint64_t, &address::by_zip> > address_index;
       *      void myaction() {
       *        address_index addresses(_self, _self);  // code, scope
       *        // add to table, first argument is account to bill for storage
       *        addresses.emplace(payer, [&](auto& address) {
       ...
       *        datxio_assert(itr->account_name == N(brendan), "Incorrect First Lower Bound Record ");
       *        itr++;
       *        datxio_assert(itr->account_name == N(dan), "Incorrect Second Lower Bound Record");
       *        itr++;
       *        datxio_assert(itr == zip_index.end(), "Incorrect End of Iterator");
       *      }
       *  }
       *  DATXIO_ABI( addressbook, (myaction) )
       *  @endcode
       ...
       *        datxio_assert(itr->account_name == N(dan), "Incorrect First Upper Bound Record ");
       *        itr++;
       *        datxio_assert(itr == zip_index.end(), "Incorrect End of Iterator");
       *      }
       *  }
      ...
      uint64_t available_primary_key()const {
         ...
         datxio_assert( _next_primary_key < no_available_primary_key, "next primary key in table is at autoincrement limit");
       ...
       *        datxio_assert(itr->account_name == N(dan), "Incorrect Record ");
       *      }
       *  }
       *  DATXIO_ABI( addressbook, (myaction) )
       *  @endcode
       ...
       *        datxio_assert(iterator_to(user) == itr, "Invalid iterator");
       *      }
       *  }
       *  DATXIO_ABI( addressbook, (myaction) )
       *  @endcode
      ...
      const_iterator iterator_to( const T& obj )const {
         const auto& objitem = static_cast<const item&>(obj);
         datxio_assert( objitem.__idx == this, "object passed to iterator_to is not in multi_index" );
         return {this, &objitem};
      }
      /**
      ...
      const_iterator emplace( uint64_t payer, Lambda&& constructor ) {
         ...
         datxio_assert( _code == current_receiver(), "cannot create objects in table of another contract" ); // Quick fix for mutating db using multi_index that shouldn't allow mutation. Real fix can come in RC2.
       ...
       *        });
       *        auto itr = addresses.find(N(dan));
       *        datxio_assert(itr != addresses.end(), "Address for account not found");
      ...
      template<typename Lambda>
      void modify( const_iterator itr, uint64_t payer, Lambda&& updater ) {
         datxio_assert( itr != end(), "cannot pass end iterator to modify" );
       ...
       *          address.state = "CA";
       *        });
       *        datxio_assert(itr->city == "San Luis Obispo", "Address not modified");
       *      }
       *  }
       *  DATXIO_ABI( addressbook, (myaction) )
       *  @endcode
      ...
      void modify( const T& obj, uint64_t payer, Lambda&& updater ) {
         ...
         const auto& objitem = static_cast<const item&>(obj);
         datxio_assert( objitem.__idx == this, "object passed to modify is not in multi_index" );
         auto& mutableitem = const_cast<item&>(objitem);
         datxio_assert( _code == current_receiver(), "cannot modify objects in table of another contract" ); // Quick fix for mutating db using multi_index that shouldn't allow mutation. Real fix can come in RC2.
         ...
         datxio_assert( pk == obj.primary_key(), "updater cannot change primary key when modifying an object" );
       ...
       *        });
       *        auto user = addresses.get(N(dan));
       *        datxio_assert(user.first_name == "Daniel", "Couldn't get him.");
       *      }
       *  }
       *  DATXIO_ABI( addressbook, (myaction) )
       *  @endcode
       ...
       *        });
       *        auto itr = addresses.find(N(dan));
       *        datxio_assert(itr != addresses.end(), "Couldn't get him.");
       *      }
       *  }
       *  DATXIO_ABI( addressbook, (myaction) )
       *  @endcode
       ...
       *        datxio_assert(itr != addresses.end(), "Address for account not found");
       *        addresses.erase( itr );
       *        datxio_assert(itr != addresses.end(), "Address not erased properly");
       *      }
       *  }
       *  DATXIO_ABI( addressbook, (myaction) )
       *  @endcode
       */
      const_iterator erase( const_iterator itr ) {
         datxio_assert( itr != end(), "cannot pass end iterator to erase" );
       ...
       *        });
       *        auto itr = addresses.find(N(dan));
       *        datxio_assert(itr != addresses.end(), "Record is not found");
       *        addresses.erase(*itr);
       *        itr = addresses.find(N(dan));
       *        datxio_assert(itr == addresses.end(), "Record is not deleted");
       *      }
       *  }
       *  DATXIO_ABI( addressbook, (myaction) )
       *  @endcode
      ...
      void erase( const T& obj ) {
         ...
         const auto& objitem = static_cast<const item&>(obj);
         datxio_assert( objitem.__idx == this, "object passed to erase is not in multi_index" );
         datxio_assert( _code == current_receiver(), "cannot erase objects in table of another contract" ); // Quick fix for mutating db using multi_index that shouldn't allow mutation. Real fix can come in RC2.
         ...
         datxio_assert( itr2 != _items_vector.rend(), "attempt to remove object that was not in multi_index" );
