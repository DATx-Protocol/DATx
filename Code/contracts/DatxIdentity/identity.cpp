...
#include <DatxioLib/vector.hpp>

namespace identity {
   using datxio::action_meta;
      ...
      public:
        ...
        using identity_base::identity_base;
         ...
         void create( const account_name creator, const uint64_t identity ) {
            ...
            datxio_assert( itr == t.end(), "identity already exists" );
            datxio_assert( identity != 0, "identity=0 is not allowed" );
                               ...
                               const vector<certvalue>& values )
         {
            ...
            idents_table t( _self, _self );
            datxio_assert( t.find( identity ) != t.end(), "identity does not exist" );
            ...
            for( const auto& value : values ) {
               ...
               if (value.confidence) {
                  datxio_assert(value.type.size() <= 32, "certrow::type should be not longer than 32 bytes");
                  ...
                  if (value.property == N(owner)) {
                     datxio_assert(sizeof(account_name) == value.data.size(), "data size doesn't match account_name size");
...
DATXIO_ABI( identity::contract, (create)(certprop)(settrust) );
